-- phpMyAdmin SQL Dump
-- version 5.0.4
-- https://www.phpmyadmin.net/
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 23 Sep 2021 pada 01.19
-- Versi server: 10.4.17-MariaDB
-- Versi PHP: 7.4.15

SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";
START TRANSACTION;
SET time_zone = "+00:00";


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8mb4 */;

--
-- Database: `notif_basic1`
--

-- --------------------------------------------------------

--
-- Struktur dari tabel `business_type`
--

CREATE TABLE `business_type` (
  `business_type_id` TINYINT(4) NOT NULL,
  `business_type` VARCHAR(100) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `business_type`
--

INSERT INTO `business_type` (`business_type_id`, `business_type`) VALUES
(1, 'university'),
(2, 'school'),
(3, 'hotel'),
(4, 'restaurant'),
(5, 'toko online');

-- --------------------------------------------------------

--
-- Struktur dari tabel `channel`
--

CREATE TABLE `channel` (
  `channel_id` TINYINT(4) NOT NULL,
  `channel_name` VARCHAR(100) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `channel`
--

INSERT INTO `channel` (`channel_id`, `channel_name`) VALUES
(1, 'telegram'),
(2, 'email'),
(3, 'whatsapp'),
(4, 'line');

-- --------------------------------------------------------

--
-- Struktur dari tabel `country`
--

CREATE TABLE `country` (
  `country_id` BIGINT(20) NOT NULL,
  `country_name` VARCHAR(100) DEFAULT NULL,
  `sortname` VARCHAR(150) DEFAULT NULL,
  `phonecode` INT(11) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8mb4;

--
-- Dumping data untuk tabel `country`
--

INSERT INTO `country` (`country_id`, `country_name`, `sortname`, `phonecode`) VALUES
(1, 'Afghanistan', 'AF', 93),
(2, 'Albania', 'AL', 355),
(3, 'Algeria', 'DZ', 213),
(4, 'American Samoa', 'AS', 1684),
(5, 'Andorra', 'AD', 376),
(6, 'Angola', 'AO', 244),
(7, 'Anguilla', 'AI', 1264),
(8, 'Antarctica', 'AQ', 0),
(9, 'Antigua And Barbuda', 'AG', 1268),
(10, 'Argentina', 'AR', 54),
(11, 'Armenia', 'AM', 374),
(12, 'Aruba', 'AW', 297),
(13, 'Australia', 'AU', 61),
(14, 'Austria', 'AT', 43),
(15, 'Azerbaijan', 'AZ', 994),
(16, 'Bahamas The', 'BS', 1242),
(17, 'Bahrain', 'BH', 973),
(18, 'Bangladesh', 'BD', 880),
(19, 'Barbados', 'BB', 1246),
(20, 'Belarus', 'BY', 375),
(21, 'Belgium', 'BE', 32),
(22, 'Belize', 'BZ', 501),
(23, 'Benin', 'BJ', 229),
(24, 'Bermuda', 'BM', 1441),
(25, 'Bhutan', 'BT', 975),
(26, 'Bolivia', 'BO', 591),
(27, 'Bosnia and Herzegovina', 'BA', 387),
(28, 'Botswana', 'BW', 267),
(29, 'Bouvet Island', 'BV', 0),
(30, 'Brazil', 'BR', 55),
(31, 'British Indian Ocean Territory', 'IO', 246),
(32, 'Brunei', 'BN', 673),
(33, 'Bulgaria', 'BG', 359),
(34, 'Burkina Faso', 'BF', 226),
(35, 'Burundi', 'BI', 257),
(36, 'Cambodia', 'KH', 855),
(37, 'Cameroon', 'CM', 237),
(38, 'Canada', 'CA', 1),
(39, 'Cape Verde', 'CV', 238),
(40, 'Cayman Islands', 'KY', 1345),
(41, 'Central African Republic', 'CF', 236),
(42, 'Chad', 'TD', 235),
(43, 'Chile', 'CL', 56),
(44, 'China', 'CN', 86),
(45, 'Christmas Island', 'CX', 61),
(46, 'Cocos (Keeling) Islands', 'CC', 672),
(47, 'Colombia', 'CO', 57),
(48, 'Comoros', 'KM', 269),
(49, 'Republic Of The Congo', 'CG', 242),
(50, 'Democratic Republic Of The Congo', 'CD', 242),
(51, 'Cook Islands', 'CK', 682),
(52, 'Costa Rica', 'CR', 506),
(53, 'Cote D\'Ivoire (Ivory Coast)', 'CI', 225),
(54, 'Croatia (Hrvatska)', 'HR', 385),
(55, 'Cuba', 'CU', 53),
(56, 'Cyprus', 'CY', 357),
(57, 'Czech Republic', 'CZ', 420),
(58, 'Denmark', 'DK', 45),
(59, 'Djibouti', 'DJ', 253),
(60, 'Dominica', 'DM', 1767),
(61, 'Dominican Republic', 'DO', 1809),
(62, 'East Timor', 'TP', 670),
(63, 'Ecuador', 'EC', 593),
(64, 'Egypt', 'EG', 20),
(65, 'El Salvador', 'SV', 503),
(66, 'Equatorial Guinea', 'GQ', 240),
(67, 'Eritrea', 'ER', 291),
(68, 'Estonia', 'EE', 372),
(69, 'Ethiopia', 'ET', 251),
(70, 'External Territories of Australia', 'XA', 61),
(71, 'Falkland Islands', 'FK', 500),
(72, 'Faroe Islands', 'FO', 298),
(73, 'Fiji Islands', 'FJ', 679),
(74, 'Finland', 'FI', 358),
(75, 'France', 'FR', 33),
(76, 'French Guiana', 'GF', 594),
(77, 'French Polynesia', 'PF', 689),
(78, 'French Southern Territories', 'TF', 0),
(79, 'Gabon', 'GA', 241),
(80, 'Gambia The', 'GM', 220),
(81, 'Georgia', 'GE', 995),
(82, 'Germany', 'DE', 49),
(83, 'Ghana', 'GH', 233),
(84, 'Gibraltar', 'GI', 350),
(85, 'Greece', 'GR', 30),
(86, 'Greenland', 'GL', 299),
(87, 'Grenada', 'GD', 1473),
(88, 'Guadeloupe', 'GP', 590),
(89, 'Guam', 'GU', 1671),
(90, 'Guatemala', 'GT', 502),
(91, 'Guernsey and Alderney', 'XU', 44),
(92, 'Guinea', 'GN', 224),
(93, 'Guinea-Bissau', 'GW', 245),
(94, 'Guyana', 'GY', 592),
(95, 'Haiti', 'HT', 509),
(96, 'Heard and McDonald Islands', 'HM', 0),
(97, 'Honduras', 'HN', 504),
(98, 'Hong Kong S.A.R.', 'HK', 852),
(99, 'Hungary', 'HU', 36),
(100, 'Iceland', 'IS', 354),
(101, 'India', 'IN', 91),
(102, 'Indonesia', 'ID', 62),
(103, 'Iran', 'IR', 98),
(104, 'Iraq', 'IQ', 964),
(105, 'Ireland', 'IE', 353),
(106, 'Israel', 'IL', 972),
(107, 'Italy', 'IT', 39),
(108, 'Jamaica', 'JM', 1876),
(109, 'Japan', 'JP', 81),
(110, 'Jersey', 'XJ', 44),
(111, 'Jordan', 'JO', 962),
(112, 'Kazakhstan', 'KZ', 7),
(113, 'Kenya', 'KE', 254),
(114, 'Kiribati', 'KI', 686),
(115, 'Korea North', 'KP', 850),
(116, 'Korea South', 'KR', 82),
(117, 'Kuwait', 'KW', 965),
(118, 'Kyrgyzstan', 'KG', 996),
(119, 'Laos', 'LA', 856),
(120, 'Latvia', 'LV', 371),
(121, 'Lebanon', 'LB', 961),
(122, 'Lesotho', 'LS', 266),
(123, 'Liberia', 'LR', 231),
(124, 'Libya', 'LY', 218),
(125, 'Liechtenstein', 'LI', 423),
(126, 'Lithuania', 'LT', 370),
(127, 'Luxembourg', 'LU', 352),
(128, 'Macau S.A.R.', 'MO', 853),
(129, 'Macedonia', 'MK', 389),
(130, 'Madagascar', 'MG', 261),
(131, 'Malawi', 'MW', 265),
(132, 'Malaysia', 'MY', 60),
(133, 'Maldives', 'MV', 960),
(134, 'Mali', 'ML', 223),
(135, 'Malta', 'MT', 356),
(136, 'Man (Isle of)', 'XM', 44),
(137, 'Marshall Islands', 'MH', 692),
(138, 'Martinique', 'MQ', 596),
(139, 'Mauritania', 'MR', 222),
(140, 'Mauritius', 'MU', 230),
(141, 'Mayotte', 'YT', 269),
(142, 'Mexico', 'MX', 52),
(143, 'Micronesia', 'FM', 691),
(144, 'Moldova', 'MD', 373),
(145, 'Monaco', 'MC', 377),
(146, 'Mongolia', 'MN', 976),
(147, 'Montserrat', 'MS', 1664),
(148, 'Morocco', 'MA', 212),
(149, 'Mozambique', 'MZ', 258),
(150, 'Myanmar', 'MM', 95),
(151, 'Namibia', 'NA', 264),
(152, 'Nauru', 'NR', 674),
(153, 'Nepal', 'NP', 977),
(154, 'Netherlands Antilles', 'AN', 599),
(155, 'Netherlands The', 'NL', 31),
(156, 'New Caledonia', 'NC', 687),
(157, 'New Zealand', 'NZ', 64),
(158, 'Nicaragua', 'NI', 505),
(159, 'Niger', 'NE', 227),
(160, 'Nigeria', 'NG', 234),
(161, 'Niue', 'NU', 683),
(162, 'Norfolk Island', 'NF', 672),
(163, 'Northern Mariana Islands', 'MP', 1670),
(164, 'Norway', 'NO', 47),
(165, 'Oman', 'OM', 968),
(166, 'Pakistan', 'PK', 92),
(167, 'Palau', 'PW', 680),
(168, 'Palestinian Territory Occupied', 'PS', 970),
(169, 'Panama', 'PA', 507),
(170, 'Papua new Guinea', 'PG', 675),
(171, 'Paraguay', 'PY', 595),
(172, 'Peru', 'PE', 51),
(173, 'Philippines', 'PH', 63),
(174, 'Pitcairn Island', 'PN', 0),
(175, 'Poland', 'PL', 48),
(176, 'Portugal', 'PT', 351),
(177, 'Puerto Rico', 'PR', 1787),
(178, 'Qatar', 'QA', 974),
(179, 'Reunion', 'RE', 262),
(180, 'Romania', 'RO', 40),
(181, 'Russia', 'RU', 70),
(182, 'Rwanda', 'RW', 250),
(183, 'Saint Helena', 'SH', 290),
(184, 'Saint Kitts And Nevis', 'KN', 1869),
(185, 'Saint Lucia', 'LC', 1758),
(186, 'Saint Pierre and Miquelon', 'PM', 508),
(187, 'Saint Vincent And The Grenadines', 'VC', 1784),
(188, 'Samoa', 'WS', 684),
(189, 'San Marino', 'SM', 378),
(190, 'Sao Tome and Principe', 'ST', 239),
(191, 'Saudi Arabia', 'SA', 966),
(192, 'Senegal', 'SN', 221),
(193, 'Serbia', 'RS', 381),
(194, 'Seychelles', 'SC', 248),
(195, 'Sierra Leone', 'SL', 232),
(196, 'Singapore', 'SG', 65),
(197, 'Slovakia', 'SK', 421),
(198, 'Slovenia', 'SI', 386),
(199, 'Smaller Territories of the UK', 'XG', 44),
(200, 'Solomon Islands', 'SB', 677),
(201, 'Somalia', 'SO', 252),
(202, 'South Africa', 'ZA', 27),
(203, 'South Georgia', 'GS', 0),
(204, 'South Sudan', 'SS', 211),
(205, 'Spain', 'ES', 34),
(206, 'Sri Lanka', 'LK', 94),
(207, 'Sudan', 'SD', 249),
(208, 'Suriname', 'SR', 597),
(209, 'Svalbard And Jan Mayen Islands', 'SJ', 47),
(210, 'Swaziland', 'SZ', 268),
(211, 'Sweden', 'SE', 46),
(212, 'Switzerland', 'CH', 41),
(213, 'Syria', 'SY', 963),
(214, 'Taiwan', 'TW', 886),
(215, 'Tajikistan', 'TJ', 992),
(216, 'Tanzania', 'TZ', 255),
(217, 'Thailand', 'TH', 66),
(218, 'Togo', 'TG', 228),
(219, 'Tokelau', 'TK', 690),
(220, 'Tonga', 'TO', 676),
(221, 'Trinidad And Tobago', 'TT', 1868),
(222, 'Tunisia', 'TN', 216),
(223, 'Turkey', 'TR', 90),
(224, 'Turkmenistan', 'TM', 7370),
(225, 'Turks And Caicos Islands', 'TC', 1649),
(226, 'Tuvalu', 'TV', 688),
(227, 'Uganda', 'UG', 256),
(228, 'Ukraine', 'UA', 380),
(229, 'United Arab Emirates', 'AE', 971),
(230, 'United Kingdom', 'GB', 44),
(231, 'United States', 'US', 1),
(232, 'United States Minor Outlying Islands', 'UM', 1),
(233, 'Uruguay', 'UY', 598),
(234, 'Uzbekistan', 'UZ', 998),
(235, 'Vanuatu', 'VU', 678),
(236, 'Vatican City State (Holy See)', 'VA', 39),
(237, 'Venezuela', 'VE', 58),
(238, 'Vietnam', 'VN', 84),
(239, 'Virgin Islands (British)', 'VG', 1284),
(240, 'Virgin Islands (US)', 'VI', 1340),
(241, 'Wallis And Futuna Islands', 'WF', 681),
(242, 'Western Sahara', 'EH', 212),
(243, 'Yemen', 'YE', 967),
(244, 'Yugoslavia', 'YU', 38),
(245, 'Zambia', 'ZM', 260),
(246, 'Zimbabwe', 'ZW', 263);

-- --------------------------------------------------------

--
-- Struktur dari tabel `group`
--

CREATE TABLE `group` (
  `group_id` BIGINT(20) NOT NULL,
  `group_name` VARCHAR(200) DEFAULT NULL,
  `description` TEXT DEFAULT NULL,
  `logo` VARCHAR(500) DEFAULT NULL,
  `token_type` ENUM('public','token','lookup','API') DEFAULT NULL COMMENT '0=public/tanpa token, 1 =private/dengan token 3. api token dr sistem eksternal',
  `token_group` VARCHAR(50) DEFAULT NULL,
  `membership_type` TINYINT(4) DEFAULT NULL,
  `business_type` TINYINT(4) DEFAULT NULL,
  `open_time` TIME DEFAULT NULL,
  `close_time` TIME DEFAULT NULL,
  `off_day` VARCHAR(50) DEFAULT NULL COMMENT 'holiday and sunday',
  `valid` ENUM('valid','not valid') DEFAULT 'not valid'
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `group`
--

INSERT INTO `group` (`group_id`, `group_name`, `description`, `logo`, `token_type`, `token_group`, `membership_type`, `business_type`, `open_time`, `close_time`, `off_day`, `valid`) VALUES
(1, 'HMTI Udayana', 'Himpunan Mahasiswa Teknologi Informasi Universitas Udayana', '[{\"name\":\"files\\/hmti-colored_2naewlzz.png\",\"usrName\":\"hmti-colored.png\",\"size\":34043,\"type\":\"image\\/png\",\"thumbnail\":\"files\\/thhmti-colored_xwbrh1ew.png\",\"thumbnail_type\":\"image\\/png\",\"thumbnail_size\":10067,\"searchStr\":\"hmti-colored.png,!:sStrEnd\"}]', 'public', '', 3, 1, '08:00:00', '18:00:00', '', 'valid'),
(2, 'Tes', 'tes', '[{\"name\":\"files\\/hmti-colored_py05anfd.png\",\"usrName\":\"hmti-colored.png\",\"size\":34043,\"type\":\"image\\/png\",\"thumbnail\":\"files\\/thhmti-colored_w0hn3pih.png\",\"thumbnail_type\":\"image\\/png\",\"thumbnail_size\":10067,\"searchStr\":\"hmti-colored.png,!:sStrEnd\"}]', 'public', '', 3, 1, '00:00:00', '22:00:00', '', NULL),
(3, 'RERE', 'RERE', '', 'public', '', 3, 1, '08:00:00', '22:00:00', '', 'valid');

-- --------------------------------------------------------

--
-- Struktur dari tabel `group_agenda`
--

CREATE TABLE `group_agenda` (
  `group_agenda_id` BIGINT(20) NOT NULL,
  `group_id` BIGINT(20) DEFAULT NULL,
  `agenda_type_id` BIGINT(20) DEFAULT NULL,
  `message_general` VARCHAR(10) DEFAULT NULL COMMENT '1: general (sama utk semua member) 2: private(tiap member punya pesan beda2 spt info utang)',
  `agenda_name` VARCHAR(225) DEFAULT NULL COMMENT 'title',
  `message_content` TEXT DEFAULT NULL COMMENT 'kalo pesan general maka pesannya ditulis disini',
  `attachment` VARCHAR(255) DEFAULT NULL,
  `loop_id` INT(11) DEFAULT NULL,
  `loop_value` DATETIME DEFAULT NULL COMMENT 'berlaku utk semua pesan general, jika private dicek dr field  date_send',
  `repeat` TINYINT(1) DEFAULT NULL COMMENT '1 (yes), 2(no',
  `repeat_type` ENUM('daily','weekly','monthly','yearly','bydate') DEFAULT NULL COMMENT 'jika bydate cek ke tabel detail group agenda_repeat_date',
  `send_date` DATETIME DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `group_agenda`
--

INSERT INTO `group_agenda` (`group_agenda_id`, `group_id`, `agenda_type_id`, `message_general`, `agenda_name`, `message_content`, `attachment`, `loop_id`, `loop_value`, `repeat`, `repeat_type`, `send_date`) VALUES
(1, 1, 1, 'General', 'Ini Agenda Group', 'Ini Agenda Group', '', 2, '2021-08-31 08:25:41', NULL, '', NULL),
(2, 1, 1, 'General', 'Ini Agenda Group 1', 'Ini Agenda Group 1', '', 1, '2021-09-01 16:31:03', 3, 'daily', NULL),
(3, 1, 1, 'General', 'Ini Agenda Group 2', 'Ini Agenda Group 2', '', 1, '2021-09-01 16:32:01', 2, 'weekly', NULL),
(4, 1, 1, 'General', 'Ini Agenda grup 7', 'Ini Agenda grup 7', '', 1, '2021-09-01 16:32:45', 2, 'bydate', NULL),
(5, 1, 1, 'Private', 'Ini Agenda Private', '', '', NULL, NULL, NULL, '', '2021-10-07 00:25:00'),
(6, 1, 1, 'Private', 'Ini Private', '', '', NULL, NULL, NULL, '', '2021-09-01 00:00:30'),
(7, 1, NULL, 'Private', 'iNI aGENDA private', '', '', NULL, NULL, NULL, '', '2021-09-02 18:00:00'),
(52, 3, NULL, 'Private', 'agenda private', '', '[{\"name\":\"files\\/group_member (11)_ch4xlgqr.xlsx\",\"usrName\":\"group_member (11).xlsx\",\"size\":8057,\"type\":\"application\\/vnd.openxmlformats-officedocument.spreadsheetml.sheet\",\"searchStr\":\"group_member (11).xlsx,!:sStrEnd\"}]', NULL, NULL, NULL, '', '2021-09-23 16:00:00'),
(53, 3, NULL, 'Private', 'ini private lagi', '', '', NULL, NULL, NULL, '', '2021-09-30 16:38:43'),
(54, 3, 2, 'General', 'tes', 'ini tes', '[{\"name\":\"files\\/TBD I - 1905551092_pr7edbqr.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 2, '2021-09-22 22:26:08', NULL, '', NULL),
(55, 3, 2, 'General', 'Pemberitahuan', 'Akan diadakan job fair', '[{\"name\":\"files\\/TBD I - 1905551092_7cgspt16.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 1, '2021-09-24 06:41:24', 1, 'bydate', NULL),
(56, 3, NULL, 'Private', 'Nilai UAS', '', '', NULL, NULL, NULL, '', '2021-09-24 06:52:10');

-- --------------------------------------------------------

--
-- Struktur dari tabel `group_agenda_general_logs`
--

CREATE TABLE `group_agenda_general_logs` (
  `id_logs` INT(11) NOT NULL,
  `member_id` BIGINT(20) NOT NULL DEFAULT 0,
  `id_group_agenda` BIGINT(20) DEFAULT NULL,
  `group_id` BIGINT(20) DEFAULT NULL,
  `date_send` DATETIME DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `group_agenda_general_logs`
--

INSERT INTO `group_agenda_general_logs` (`id_logs`, `member_id`, `id_group_agenda`, `group_id`, `date_send`) VALUES
(1, 2, 1, NULL, NULL),
(2, 3, 1, NULL, NULL),
(3, 2, 2, NULL, '2021-09-01 16:31:03'),
(4, 2, 2, NULL, '2021-09-02 16:31:03'),
(5, 2, 2, NULL, '2021-09-03 16:31:03'),
(6, 2, 2, NULL, '2021-09-04 16:31:03'),
(7, 3, 2, NULL, '2021-09-01 16:31:03'),
(8, 3, 2, NULL, '2021-09-02 16:31:03'),
(9, 3, 2, NULL, '2021-09-03 16:31:03'),
(10, 3, 2, NULL, '2021-09-04 16:31:03'),
(11, 2, 3, NULL, '2021-09-01 16:32:01'),
(12, 2, 3, NULL, '2021-09-08 16:32:01'),
(13, 2, 3, NULL, '2021-09-15 16:32:01'),
(14, 3, 3, NULL, '2021-09-01 16:32:01'),
(15, 3, 3, NULL, '2021-09-08 16:32:01'),
(16, 3, 3, NULL, '2021-09-15 16:32:01'),
(17, 2, 4, NULL, '2021-09-01 16:32:45'),
(18, 3, 4, NULL, '2021-09-01 16:32:45'),
(92, 4, 54, NULL, NULL),
(93, 4, 55, NULL, '2021-09-24 06:41:24');

-- --------------------------------------------------------

--
-- Struktur dari tabel `group_agenda_private`
--

CREATE TABLE `group_agenda_private` (
  `group_agenda_private_id` BIGINT(20) NOT NULL,
  `id_group_agenda` BIGINT(20) DEFAULT NULL,
  `id_member` BIGINT(20) DEFAULT NULL,
  `group_id` BIGINT(20) DEFAULT NULL,
  `message_content` TEXT DEFAULT NULL COMMENT 'bisa berbeda beda antara member',
  `attachment` VARCHAR(255) DEFAULT NULL COMMENT 'bisa berbeda beda antara member',
  `date_send` DATETIME DEFAULT NULL COMMENT 'jika kososng akan diambil nilainya dari loop value di tabel group agenda'
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `group_agenda_private`
--

INSERT INTO `group_agenda_private` (`group_agenda_private_id`, `id_group_agenda`, `id_member`, `group_id`, `message_content`, `attachment`, `date_send`) VALUES
(17, 53, 3, 3, 'nilai anda 80', '', '2021-09-30 16:38:43'),
(18, 53, 4, 3, 'nilai anda 90', '', '2021-09-30 16:38:43');

-- --------------------------------------------------------

--
-- Struktur dari tabel `group_agenda_repeat_date`
--

CREATE TABLE `group_agenda_repeat_date` (
  `id_group_repeat` BIGINT(20) NOT NULL,
  `group_id` BIGINT(20) DEFAULT NULL,
  `group_agenda_id` BIGINT(20) DEFAULT NULL,
  `dodate` DATETIME DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `group_agenda_repeat_date`
--

INSERT INTO `group_agenda_repeat_date` (`id_group_repeat`, `group_id`, `group_agenda_id`, `dodate`) VALUES
(1, NULL, 4, '2021-09-02 22:00:00'),
(2, NULL, 4, '2021-09-03 16:00:55'),
(10, NULL, 55, '2021-09-24 06:43:26');

-- --------------------------------------------------------

--
-- Struktur dari tabel `group_agenda_type`
--

CREATE TABLE `group_agenda_type` (
  `agenda_type_id` BIGINT(20) NOT NULL,
  `group_id` BIGINT(20) DEFAULT NULL,
  `agenda_type` VARCHAR(225) DEFAULT NULL,
  `description` VARCHAR(255) DEFAULT NULL,
  `group_requested` BIGINT(20) DEFAULT NULL COMMENT 'siapa yg request',
  `valid` TINYINT(1) DEFAULT NULL COMMENT 'jika belum divalid belum bisa dipakai'
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `group_agenda_type`
--

INSERT INTO `group_agenda_type` (`agenda_type_id`, `group_id`, `agenda_type`, `description`, `group_requested`, `valid`) VALUES
(1, 1, 'Info Beasiswa', 'Info Beasiswa', NULL, NULL),
(2, 3, 'apayaa', 'ya apaa', NULL, NULL);

-- --------------------------------------------------------

--
-- Struktur dari tabel `group_member`
--

CREATE TABLE `group_member` (
  `group_member_id` BIGINT(20) NOT NULL,
  `member_id` BIGINT(20) DEFAULT NULL,
  `group_id` BIGINT(20) DEFAULT NULL,
  `user_type` TINYINT(4) DEFAULT NULL COMMENT 'sa(pemilik/pembuat group), admin, user (bisa request atau didaftarkan oleh admin, user berhak nolak)',
  `token_group` VARCHAR(30) DEFAULT NULL COMMENT 'dipakai saat register utk memastikan orangnya punya hak join utk private group. bisa dilookup dr token di tabel group atau  by api dgn pemilik saham',
  `identifier_group` VARCHAR(30) DEFAULT NULL COMMENT 'bisa berupa nip, nim atau no anggota lainnya di aorganisasi tersebut khusus untuk jenis member internal(lookup dan API)',
  `token_personal` VARCHAR(30) DEFAULT NULL COMMENT 'token personal pasangan NIP (lookup dan API)',
  `membership_type` TINYINT(4) DEFAULT NULL,
  `valid` TINYINT(1) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `group_member`
--

INSERT INTO `group_member` (`group_member_id`, `member_id`, `group_id`, `user_type`, `token_group`, `identifier_group`, `token_personal`, `membership_type`, `valid`) VALUES
(4, 2, 2, 2, NULL, NULL, NULL, 3, 1),
(5, 2, 3, 2, NULL, NULL, NULL, 3, 1),
(7, 4, 3, 3, '', '', '', 3, 1),
(8, 3, 1, 3, '', '', '', 3, 1);

-- --------------------------------------------------------

--
-- Struktur dari tabel `group_member_agenda`
--

CREATE TABLE `group_member_agenda` (
  `group_member_agenda_id` BIGINT(20) NOT NULL,
  `group_member_id` BIGINT(20) DEFAULT NULL,
  `member_id` BIGINT(20) DEFAULT NULL,
  `group_agenda_id` BIGINT(20) DEFAULT NULL COMMENT 'mencatat seorang member langganan apa di group tsbt',
  `group_id` BIGINT(20) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `group_member_agenda`
--

INSERT INTO `group_member_agenda` (`group_member_agenda_id`, `group_member_id`, `member_id`, `group_agenda_id`, `group_id`) VALUES
(49, 7, 4, 54, NULL),
(50, 7, 4, 55, NULL);

-- --------------------------------------------------------

--
-- Struktur dari tabel `group_member_agenda_subscriber`
--

CREATE TABLE `group_member_agenda_subscriber` (
  `group_member_id` BIGINT(20) NOT NULL,
  `member_id` BIGINT(20) DEFAULT NULL,
  `agenda_type_id` BIGINT(20) DEFAULT NULL,
  `group_id` BIGINT(20) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `group_member_agenda_subscriber`
--

INSERT INTO `group_member_agenda_subscriber` (`group_member_id`, `member_id`, `agenda_type_id`, `group_id`) VALUES
(4, 3, 2, 3),
(5, 4, 2, 3);

-- --------------------------------------------------------

--
-- Struktur dari tabel `group_member_checkin`
--

CREATE TABLE `group_member_checkin` (
  `group_member_checkin_id` BIGINT(20) NOT NULL,
  `group_member_id` BIGINT(20) DEFAULT NULL,
  `member_id` BIGINT(20) DEFAULT NULL,
  `group_id` BIGINT(20) DEFAULT NULL COMMENT 'mencatat seorang member langganan apa di group tsbt',
  `checkin_date` DATETIME DEFAULT NULL,
  `checkout_date` DATETIME DEFAULT NULL,
  `id_room` VARCHAR(10) DEFAULT NULL COMMENT 'np meja. kamar',
  `valid` ENUM('not valid','valid') DEFAULT NULL,
  `review_member` TEXT DEFAULT NULL,
  `rating_member` TINYINT(4) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

-- --------------------------------------------------------

--
-- Struktur dari tabel `group_member_order`
--

CREATE TABLE `group_member_order` (
  `group_member_order` BIGINT(20) NOT NULL,
  `group_member_id` BIGINT(20) DEFAULT NULL,
  `member_id` BIGINT(20) DEFAULT NULL,
  `group_id` BIGINT(20) DEFAULT NULL COMMENT 'mencatat seorang member langganan apa di group tsbt',
  `order_date` DATETIME DEFAULT NULL,
  `valid` ENUM('valid','not valid') DEFAULT NULL,
  `total` DOUBLE DEFAULT NULL,
  `currency` CHAR(5) DEFAULT NULL,
  `review_member` TEXT DEFAULT NULL,
  `rating_member` TINYINT(4) DEFAULT NULL,
  `payment_status` ENUM('paid','not_paid') DEFAULT NULL,
  `money_received` DOUBLE DEFAULT NULL,
  `change_money` DOUBLE DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `group_member_order`
--

INSERT INTO `group_member_order` (`group_member_order`, `group_member_id`, `member_id`, `group_id`, `order_date`, `valid`, `total`, `currency`, `review_member`, `rating_member`, `payment_status`, `money_received`, `change_money`) VALUES
(1, 8, 3, 1, '2021-09-22 22:13:54', 'not valid', NULL, 'IDR', NULL, NULL, 'not_paid', NULL, NULL);

-- --------------------------------------------------------

--
-- Struktur dari tabel `group_member_order_detail`
--

CREATE TABLE `group_member_order_detail` (
  `group_member_order_detail_id` BIGINT(20) NOT NULL,
  `group_member_order_id` BIGINT(20) DEFAULT NULL,
  `group_member_id` BIGINT(20) DEFAULT NULL,
  `member_id` BIGINT(20) DEFAULT NULL,
  `group_id` BIGINT(20) DEFAULT NULL COMMENT 'mencatat seorang member langganan apa di group tsbt',
  `group_product_id` BIGINT(20) DEFAULT NULL,
  `nominal` DOUBLE DEFAULT NULL,
  `quantity` TINYINT(4) DEFAULT NULL,
  `total` DOUBLE DEFAULT NULL,
  `progress` ENUM('order','on_progress','ready') DEFAULT NULL COMMENT 'order,on progress, ready'
) ENGINE=INNODB DEFAULT CHARSET=utf8;

-- --------------------------------------------------------

--
-- Struktur dari tabel `group_operation_off`
--

CREATE TABLE `group_operation_off` (
  `operation_id` BIGINT(20) NOT NULL,
  `group_id` BIGINT(20) DEFAULT NULL,
  `day_off` DATE DEFAULT NULL,
  `desc` VARCHAR(150) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

-- --------------------------------------------------------

--
-- Struktur dari tabel `group_product`
--

CREATE TABLE `group_product` (
  `group_product_id` BIGINT(20) NOT NULL,
  `group_id` BIGINT(20) DEFAULT NULL,
  `product_type` TINYINT(4) DEFAULT NULL,
  `product_name` VARCHAR(100) DEFAULT NULL,
  `stock_type` TINYINT(4) DEFAULT NULL,
  `stock` INT(11) DEFAULT NULL,
  `order_type` TINYINT(4) DEFAULT NULL,
  `price` DOUBLE DEFAULT NULL,
  `curency` CHAR(5) DEFAULT NULL,
  `image` VARCHAR(500) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

-- --------------------------------------------------------

--
-- Struktur dari tabel `group_user_type`
--

CREATE TABLE `group_user_type` (
  `id_user_type` TINYINT(4) NOT NULL,
  `user_type` VARCHAR(100) DEFAULT NULL COMMENT 'admin bisa kelola group spt add member, reject, agenda'
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `group_user_type`
--

INSERT INTO `group_user_type` (`id_user_type`, `user_type`) VALUES
(1, 'super admin/owner'),
(2, 'admin group'),
(3, 'user');

-- --------------------------------------------------------

--
-- Struktur dari tabel `lookup_stok`
--

CREATE TABLE `lookup_stok` (
  `lookup_stok_id` BIGINT(20) NOT NULL,
  `group_id` BIGINT(20) DEFAULT NULL,
  `product_name` VARCHAR(30) DEFAULT NULL,
  `stock` INT(11) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8mb4;

-- --------------------------------------------------------

--
-- Struktur dari tabel `lookup_token`
--

CREATE TABLE `lookup_token` (
  `id_lookup` BIGINT(20) NOT NULL,
  `id_group` BIGINT(20) DEFAULT NULL,
  `group_identifier` VARCHAR(30) DEFAULT NULL COMMENT 'nip',
  `token` VARCHAR(30) DEFAULT NULL COMMENT 'token pasangan NIP'
) ENGINE=INNODB DEFAULT CHARSET=utf8;

-- --------------------------------------------------------

--
-- Struktur dari tabel `loop_type`
--

CREATE TABLE `loop_type` (
  `id_loop` INT(11) NOT NULL,
  `loop_type` VARCHAR(20) DEFAULT NULL,
  `desc` VARCHAR(200) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `loop_type`
--

INSERT INTO `loop_type` (`id_loop`, `loop_type`, `desc`) VALUES
(1, 'on date', 'dikirim sesuai tanggal  dan jam yang di set di loop value '),
(2, 'real time', 'dikirim saat disimpan, loop value gak perlu');

-- --------------------------------------------------------

--
-- Struktur dari tabel `membership_type`
--

CREATE TABLE `membership_type` (
  `membership_id` TINYINT(4) NOT NULL,
  `membership_type` VARCHAR(100) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `membership_type`
--

INSERT INTO `membership_type` (`membership_id`, `membership_type`) VALUES
(1, 'internal (punya No anggota spt perusahaan/kampus)'),
(2, 'eksternal (seperti simpatisan,misal calon mhs baru, calon pelanggan indomaret(internal pegawai)'),
(3, 'internal & external');

-- --------------------------------------------------------

--
-- Struktur dari tabel `order_type`
--

CREATE TABLE `order_type` (
  `order_type_id` TINYINT(4) NOT NULL,
  `order_type` VARCHAR(100) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `order_type`
--

INSERT INTO `order_type` (`order_type_id`, `order_type`) VALUES
(1, 'public'),
(2, 'member'),
(3, 'member check in');

-- --------------------------------------------------------

--
-- Struktur dari tabel `outbox_line`
--

CREATE TABLE `outbox_line` (
  `id_outbox` BIGINT(20) NOT NULL,
  `chat_id` VARCHAR(100) DEFAULT NULL,
  `out_msg` TEXT DEFAULT NULL,
  `type` ENUM('msg','file','loc','img') DEFAULT 'msg',
  `flag` ENUM('1','2') DEFAULT NULL,
  `tgl` DATETIME DEFAULT NULL,
  `group_agenda_id` BIGINT(20) DEFAULT NULL,
  `group_agenda_private_id` BIGINT(20) DEFAULT NULL,
  `member_agenda_id` BIGINT(20) DEFAULT NULL,
  `group_id` BIGINT(20) DEFAULT NULL,
  `group_member_id` BIGINT(20) DEFAULT NULL,
  `member_id` BIGINT(20) DEFAULT NULL,
  `id_group_repeat` BIGINT(20) DEFAULT NULL,
  `id_personal_repeat` BIGINT(20) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=latin1;

--
-- Dumping data untuk tabel `outbox_line`
--

INSERT INTO `outbox_line` (`id_outbox`, `chat_id`, `out_msg`, `type`, `flag`, `tgl`, `group_agenda_id`, `group_agenda_private_id`, `member_agenda_id`, `group_id`, `group_member_id`, `member_id`, `id_group_repeat`, `id_personal_repeat`) VALUES
(101, 'jyo', 'Ini agenda pribadi realtime', 'msg', '1', '2021-09-09 08:25:30', NULL, NULL, 74, NULL, NULL, 3, NULL, NULL),
(102, 'jyo', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_1buxl9ql.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-09 08:25:30', NULL, NULL, 74, NULL, NULL, 3, NULL, NULL),
(103, 'jyo', 'agenda ondate', 'msg', '1', '2021-09-10 20:00:00', NULL, NULL, 75, NULL, NULL, 3, NULL, NULL),
(111, 'jyo', 'agenda repeat edit', 'msg', '1', '2021-11-04 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(112, 'jyo', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-04 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(113, 'jyo', 'agenda repeat edit', 'msg', '1', '2021-11-11 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(114, 'jyo', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-11 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(115, 'jyo', 'agenda repeat edit', 'msg', '1', '2021-11-18 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(116, 'jyo', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-18 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(117, 'jyo', 'agenda repeat edit', 'msg', '1', '2021-11-25 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(118, 'jyo', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-25 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(119, 'jyo', 'agenda by date', 'msg', '1', '2021-09-10 16:30:01', NULL, NULL, 77, NULL, NULL, 3, NULL, NULL),
(120, 'jyo', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_o57440xl.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-10 16:30:01', NULL, NULL, 77, NULL, NULL, 3, NULL, NULL),
(124, 'jyo', 'Peringatan', 'msg', '1', '2021-09-24 06:12:11', NULL, NULL, 78, NULL, NULL, 3, NULL, NULL),
(125, 'jyo', '[{\"name\":\"files\\/TBD I - 1905551092_1jvr34bh.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:12:11', NULL, NULL, 78, NULL, NULL, 3, NULL, NULL),
(126, 'jyo', 'Peringatan', 'msg', '1', '2021-10-01 06:12:19', NULL, NULL, 78, NULL, NULL, 3, NULL, 18),
(127, 'jyo', '[{\"name\":\"files\\/TBD I - 1905551092_1jvr34bh.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-10-01 06:12:19', NULL, NULL, 78, NULL, NULL, 3, NULL, 18),
(128, 'jyo', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, NULL, 3, 10, NULL),
(129, 'jyo', '[{\"name\":\"files\\/TBD I - 1905551092_7cgspt16.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, NULL, 3, 10, NULL);

-- --------------------------------------------------------

--
-- Struktur dari tabel `outbox_mail_alibaba`
--

CREATE TABLE `outbox_mail_alibaba` (
  `id_outbox` BIGINT(20) NOT NULL,
  `chat_id` VARCHAR(100) DEFAULT NULL,
  `out_msg` TEXT DEFAULT NULL,
  `type` ENUM('msg','file','loc','img') DEFAULT 'msg',
  `flag` ENUM('1','2') DEFAULT NULL,
  `tgl` DATETIME DEFAULT NULL,
  `group_agenda_id` BIGINT(20) DEFAULT NULL,
  `group_agenda_private_id` BIGINT(20) DEFAULT NULL,
  `member_agenda_id` BIGINT(20) DEFAULT NULL,
  `group_id` BIGINT(20) DEFAULT NULL,
  `group_member_id` BIGINT(20) DEFAULT NULL,
  `member_id` BIGINT(20) DEFAULT NULL,
  `id_group_repeat` BIGINT(20) DEFAULT NULL,
  `id_personal_repeat` BIGINT(20) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=latin1;

--
-- Dumping data untuk tabel `outbox_mail_alibaba`
--

INSERT INTO `outbox_mail_alibaba` (`id_outbox`, `chat_id`, `out_msg`, `type`, `flag`, `tgl`, `group_agenda_id`, `group_agenda_private_id`, `member_agenda_id`, `group_id`, `group_member_id`, `member_id`, `id_group_repeat`, `id_personal_repeat`) VALUES
(170, 'jyotisabali@gmail.com', 'Ini agenda pribadi realtime', 'msg', '1', '2021-09-09 08:25:30', NULL, NULL, 74, NULL, NULL, 3, NULL, NULL),
(171, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_1buxl9ql.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-09 08:25:30', NULL, NULL, 74, NULL, NULL, 3, NULL, NULL),
(172, 'jyotisabali@gmail.com', 'agenda ondate', 'msg', '1', '2021-09-10 20:00:00', NULL, NULL, 75, NULL, NULL, 3, NULL, NULL),
(180, 'jyotisabali@gmail.com', 'agenda repeat edit', 'msg', '1', '2021-11-04 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(181, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-04 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(182, 'jyotisabali@gmail.com', 'agenda repeat edit', 'msg', '1', '2021-11-11 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(183, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-11 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(184, 'jyotisabali@gmail.com', 'agenda repeat edit', 'msg', '1', '2021-11-18 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(185, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-18 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(186, 'jyotisabali@gmail.com', 'agenda repeat edit', 'msg', '1', '2021-11-25 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(187, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-25 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(188, 'jyotisabali@gmail.com', 'agenda by date', 'msg', '1', '2021-09-10 16:30:01', NULL, NULL, 77, NULL, NULL, 3, NULL, NULL),
(189, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_o57440xl.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-10 16:30:01', NULL, NULL, 77, NULL, NULL, 3, NULL, NULL),
(193, 'nyomanjyotisa@student.unud.ac.id', 'nilai anda 90', 'msg', '1', '2021-09-30 16:38:43', 53, NULL, NULL, 3, 7, 4, NULL, NULL),
(194, 'jyotisabali@gmail.com', 'Peringatan', 'msg', '1', '2021-09-24 06:12:11', NULL, NULL, 78, NULL, NULL, 3, NULL, NULL),
(195, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/TBD I - 1905551092_1jvr34bh.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:12:11', NULL, NULL, 78, NULL, NULL, 3, NULL, NULL),
(196, 'jyotisabali@gmail.com', 'Peringatan', 'msg', '1', '2021-10-01 06:12:19', NULL, NULL, 78, NULL, NULL, 3, NULL, 18),
(197, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/TBD I - 1905551092_1jvr34bh.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-10-01 06:12:19', NULL, NULL, 78, NULL, NULL, 3, NULL, 18),
(198, 'nyomanjyotisa@student.unud.ac.id', 'ini tes', 'msg', '1', '2021-09-22 22:26:08', 54, NULL, NULL, 3, 7, 4, NULL, NULL),
(199, 'nyomanjyotisa@student.unud.ac.id', '[{\"name\":\"files\\/TBD I - 1905551092_pr7edbqr.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-22 22:26:08', 54, NULL, NULL, 3, 7, 4, NULL, NULL),
(200, 'nyomanjyotisa@student.unud.ac.id', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:41:24', 55, NULL, NULL, 3, 7, 4, NULL, NULL),
(201, 'nyomanjyotisa@student.unud.ac.id', '[{\"name\":\"files\\/TBD I - 1905551092_7cgspt16.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:41:24', 55, NULL, NULL, 3, 7, 4, NULL, NULL),
(202, 'jyotisabali@gmail.com', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, NULL, 3, 10, NULL),
(203, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/TBD I - 1905551092_7cgspt16.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, NULL, 3, 10, NULL),
(204, 'nyomanjyotisa@student.unud.ac.id', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, 7, 4, 10, NULL),
(205, 'nyomanjyotisa@student.unud.ac.id', '[{\"name\":\"files\\/TBD I - 1905551092_7cgspt16.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, 7, 4, 10, NULL);

-- --------------------------------------------------------

--
-- Struktur dari tabel `outbox_mail_aws`
--

CREATE TABLE `outbox_mail_aws` (
  `id_outbox` BIGINT(20) NOT NULL,
  `chat_id` VARCHAR(100) DEFAULT NULL,
  `out_msg` TEXT DEFAULT NULL,
  `type` ENUM('msg','file','loc','img') DEFAULT 'msg',
  `flag` ENUM('1','2') DEFAULT NULL,
  `tgl` DATETIME DEFAULT NULL,
  `group_agenda_id` BIGINT(20) DEFAULT NULL,
  `group_agenda_private_id` BIGINT(20) DEFAULT NULL,
  `member_agenda_id` BIGINT(20) DEFAULT NULL,
  `group_id` BIGINT(20) DEFAULT NULL,
  `group_member_id` BIGINT(20) DEFAULT NULL,
  `member_id` BIGINT(20) DEFAULT NULL,
  `id_group_repeat` BIGINT(20) DEFAULT NULL,
  `id_personal_repeat` BIGINT(20) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=latin1;

--
-- Dumping data untuk tabel `outbox_mail_aws`
--

INSERT INTO `outbox_mail_aws` (`id_outbox`, `chat_id`, `out_msg`, `type`, `flag`, `tgl`, `group_agenda_id`, `group_agenda_private_id`, `member_agenda_id`, `group_id`, `group_member_id`, `member_id`, `id_group_repeat`, `id_personal_repeat`) VALUES
(173, 'jyotisabali@gmail.com', 'Ini agenda pribadi realtime', 'msg', '1', '2021-09-09 08:25:30', NULL, NULL, 74, NULL, NULL, 3, NULL, NULL),
(174, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_1buxl9ql.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-09 08:25:30', NULL, NULL, 74, NULL, NULL, 3, NULL, NULL),
(175, 'jyotisabali@gmail.com', 'agenda ondate', 'msg', '1', '2021-09-10 20:00:00', NULL, NULL, 75, NULL, NULL, 3, NULL, NULL),
(183, 'jyotisabali@gmail.com', 'agenda repeat edit', 'msg', '1', '2021-11-04 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(184, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-04 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(185, 'jyotisabali@gmail.com', 'agenda repeat edit', 'msg', '1', '2021-11-11 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(186, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-11 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(187, 'jyotisabali@gmail.com', 'agenda repeat edit', 'msg', '1', '2021-11-18 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(188, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-18 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(189, 'jyotisabali@gmail.com', 'agenda repeat edit', 'msg', '1', '2021-11-25 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(190, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-25 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(191, 'jyotisabali@gmail.com', 'agenda by date', 'msg', '1', '2021-09-10 16:30:01', NULL, NULL, 77, NULL, NULL, 3, NULL, NULL),
(192, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_o57440xl.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-10 16:30:01', NULL, NULL, 77, NULL, NULL, 3, NULL, NULL),
(196, 'nyomanjyotisa@student.unud.ac.id', 'nilai anda 90', 'msg', '1', '2021-09-30 16:38:43', 53, NULL, NULL, 3, 7, 4, NULL, NULL),
(197, 'jyotisabali@gmail.com', 'Anda telah bergabung dengan group HMTI Udayana', 'msg', '1', '2021-09-22 22:03:24', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
(198, 'jyotisabali@gmail.com', 'Peringatan', 'msg', '1', '2021-09-24 06:12:11', NULL, NULL, 78, NULL, NULL, 3, NULL, NULL),
(199, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/TBD I - 1905551092_1jvr34bh.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:12:11', NULL, NULL, 78, NULL, NULL, 3, NULL, NULL),
(200, 'jyotisabali@gmail.com', 'Peringatan', 'msg', '1', '2021-10-01 06:12:19', NULL, NULL, 78, NULL, NULL, 3, NULL, 18),
(201, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/TBD I - 1905551092_1jvr34bh.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-10-01 06:12:19', NULL, NULL, 78, NULL, NULL, 3, NULL, 18),
(202, 'jyotisabali@gmail.com', 'Anda telah membuat pesanan baru di HMTI Udayana dengan id 1', 'msg', '1', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
(203, 'nyomanjyotisa@student.unud.ac.id', 'ini tes', 'msg', '1', '2021-09-22 22:26:08', 54, NULL, NULL, 3, 7, 4, NULL, NULL),
(204, 'nyomanjyotisa@student.unud.ac.id', '[{\"name\":\"files\\/TBD I - 1905551092_pr7edbqr.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-22 22:26:08', 54, NULL, NULL, 3, 7, 4, NULL, NULL),
(205, 'nyomanjyotisa@student.unud.ac.id', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:41:24', 55, NULL, NULL, 3, 7, 4, NULL, NULL),
(206, 'nyomanjyotisa@student.unud.ac.id', '[{\"name\":\"files\\/TBD I - 1905551092_7cgspt16.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:41:24', 55, NULL, NULL, 3, 7, 4, NULL, NULL),
(207, 'jyotisabali@gmail.com', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, NULL, 3, 10, NULL),
(208, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/TBD I - 1905551092_7cgspt16.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, NULL, 3, 10, NULL),
(209, 'nyomanjyotisa@student.unud.ac.id', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, 7, 4, 10, NULL),
(210, 'nyomanjyotisa@student.unud.ac.id', '[{\"name\":\"files\\/TBD I - 1905551092_7cgspt16.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, 7, 4, 10, NULL);

-- --------------------------------------------------------

--
-- Struktur dari tabel `outbox_mail_mailchimp`
--

CREATE TABLE `outbox_mail_mailchimp` (
  `id_outbox` BIGINT(20) NOT NULL,
  `chat_id` VARCHAR(100) DEFAULT NULL,
  `out_msg` TEXT DEFAULT NULL,
  `type` ENUM('msg','file','loc','img') DEFAULT 'msg',
  `flag` ENUM('1','2') DEFAULT NULL,
  `tgl` DATETIME DEFAULT NULL,
  `group_agenda_id` BIGINT(20) DEFAULT NULL,
  `group_agenda_private_id` BIGINT(20) DEFAULT NULL,
  `member_agenda_id` BIGINT(20) DEFAULT NULL,
  `group_id` BIGINT(20) DEFAULT NULL,
  `group_member_id` BIGINT(20) DEFAULT NULL,
  `member_id` BIGINT(20) DEFAULT NULL,
  `id_group_repeat` BIGINT(20) DEFAULT NULL,
  `id_personal_repeat` BIGINT(20) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=latin1;

--
-- Dumping data untuk tabel `outbox_mail_mailchimp`
--

INSERT INTO `outbox_mail_mailchimp` (`id_outbox`, `chat_id`, `out_msg`, `type`, `flag`, `tgl`, `group_agenda_id`, `group_agenda_private_id`, `member_agenda_id`, `group_id`, `group_member_id`, `member_id`, `id_group_repeat`, `id_personal_repeat`) VALUES
(170, 'jyotisabali@gmail.com', 'Ini agenda pribadi realtime', 'msg', '1', '2021-09-09 08:25:30', NULL, NULL, 74, NULL, NULL, 3, NULL, NULL),
(171, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_1buxl9ql.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-09 08:25:30', NULL, NULL, 74, NULL, NULL, 3, NULL, NULL),
(172, 'jyotisabali@gmail.com', 'agenda ondate', 'msg', '1', '2021-09-10 20:00:00', NULL, NULL, 75, NULL, NULL, 3, NULL, NULL),
(180, 'jyotisabali@gmail.com', 'agenda repeat edit', 'msg', '1', '2021-11-04 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(181, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-04 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(182, 'jyotisabali@gmail.com', 'agenda repeat edit', 'msg', '1', '2021-11-11 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(183, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-11 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(184, 'jyotisabali@gmail.com', 'agenda repeat edit', 'msg', '1', '2021-11-18 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(185, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-18 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(186, 'jyotisabali@gmail.com', 'agenda repeat edit', 'msg', '1', '2021-11-25 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(187, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-25 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(188, 'jyotisabali@gmail.com', 'agenda by date', 'msg', '1', '2021-09-10 16:30:01', NULL, NULL, 77, NULL, NULL, 3, NULL, NULL),
(189, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_o57440xl.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-10 16:30:01', NULL, NULL, 77, NULL, NULL, 3, NULL, NULL),
(193, 'nyomanjyotisa@student.unud.ac.id', 'nilai anda 90', 'msg', '1', '2021-09-30 16:38:43', 53, NULL, NULL, 3, 7, 4, NULL, NULL),
(194, 'jyotisabali@gmail.com', 'Peringatan', 'msg', '1', '2021-09-24 06:12:11', NULL, NULL, 78, NULL, NULL, 3, NULL, NULL),
(195, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/TBD I - 1905551092_1jvr34bh.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:12:11', NULL, NULL, 78, NULL, NULL, 3, NULL, NULL),
(196, 'jyotisabali@gmail.com', 'Peringatan', 'msg', '1', '2021-10-01 06:12:19', NULL, NULL, 78, NULL, NULL, 3, NULL, 18),
(197, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/TBD I - 1905551092_1jvr34bh.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-10-01 06:12:19', NULL, NULL, 78, NULL, NULL, 3, NULL, 18),
(198, 'nyomanjyotisa@student.unud.ac.id', 'ini tes', 'msg', '1', '2021-09-22 22:26:08', 54, NULL, NULL, 3, 7, 4, NULL, NULL),
(199, 'nyomanjyotisa@student.unud.ac.id', '[{\"name\":\"files\\/TBD I - 1905551092_pr7edbqr.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-22 22:26:08', 54, NULL, NULL, 3, 7, 4, NULL, NULL),
(200, 'nyomanjyotisa@student.unud.ac.id', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:41:24', 55, NULL, NULL, 3, 7, 4, NULL, NULL),
(201, 'nyomanjyotisa@student.unud.ac.id', '[{\"name\":\"files\\/TBD I - 1905551092_7cgspt16.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:41:24', 55, NULL, NULL, 3, 7, 4, NULL, NULL),
(202, 'jyotisabali@gmail.com', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, NULL, 3, 10, NULL),
(203, 'jyotisabali@gmail.com', '[{\"name\":\"files\\/TBD I - 1905551092_7cgspt16.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, NULL, 3, 10, NULL),
(204, 'nyomanjyotisa@student.unud.ac.id', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, 7, 4, 10, NULL),
(205, 'nyomanjyotisa@student.unud.ac.id', '[{\"name\":\"files\\/TBD I - 1905551092_7cgspt16.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, 7, 4, 10, NULL);

-- --------------------------------------------------------

--
-- Struktur dari tabel `outbox_telegram`
--

CREATE TABLE `outbox_telegram` (
  `id_outbox` BIGINT(20) NOT NULL,
  `chat_id` VARCHAR(100) DEFAULT NULL,
  `out_msg` TEXT DEFAULT NULL,
  `type` ENUM('msg','file','loc','img') DEFAULT 'msg',
  `flag` ENUM('1','2') DEFAULT NULL,
  `tgl` DATETIME DEFAULT NULL,
  `group_agenda_id` BIGINT(20) DEFAULT NULL,
  `group_agenda_private_id` BIGINT(20) DEFAULT NULL,
  `member_agenda_id` BIGINT(20) DEFAULT NULL,
  `group_id` BIGINT(20) DEFAULT NULL,
  `group_member_id` BIGINT(20) DEFAULT NULL,
  `member_id` BIGINT(20) DEFAULT NULL,
  `id_group_repeat` BIGINT(20) DEFAULT NULL,
  `id_personal_repeat` BIGINT(20) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=latin1;

--
-- Dumping data untuk tabel `outbox_telegram`
--

INSERT INTO `outbox_telegram` (`id_outbox`, `chat_id`, `out_msg`, `type`, `flag`, `tgl`, `group_agenda_id`, `group_agenda_private_id`, `member_agenda_id`, `group_id`, `group_member_id`, `member_id`, `id_group_repeat`, `id_personal_repeat`) VALUES
(159, '@jyotisa', 'Ini agenda pribadi realtime', 'msg', '1', '2021-09-09 08:25:30', NULL, NULL, 74, NULL, NULL, 3, NULL, NULL),
(160, '@jyotisa', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_1buxl9ql.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-09 08:25:30', NULL, NULL, 74, NULL, NULL, 3, NULL, NULL),
(161, '@jyotisa', 'agenda ondate', 'msg', '1', '2021-09-10 20:00:00', NULL, NULL, 75, NULL, NULL, 3, NULL, NULL),
(169, '@jyotisa', 'agenda repeat edit', 'msg', '1', '2021-11-04 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(170, '@jyotisa', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-04 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(171, '@jyotisa', 'agenda repeat edit', 'msg', '1', '2021-11-11 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(172, '@jyotisa', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-11 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(173, '@jyotisa', 'agenda repeat edit', 'msg', '1', '2021-11-18 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(174, '@jyotisa', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-18 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(175, '@jyotisa', 'agenda repeat edit', 'msg', '1', '2021-11-25 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(176, '@jyotisa', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-25 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(177, '@jyotisa', 'agenda by date', 'msg', '1', '2021-09-10 16:30:01', NULL, NULL, 77, NULL, NULL, 3, NULL, NULL),
(178, '@jyotisa', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_o57440xl.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-10 16:30:01', NULL, NULL, 77, NULL, NULL, 3, NULL, NULL),
(182, 'oyoy', 'nilai anda 90', 'msg', '1', '2021-09-30 16:38:43', 53, NULL, NULL, 3, 7, 4, NULL, NULL),
(183, '@jyotisa', 'Anda telah bergabung dengan group HMTI Udayana', 'msg', '1', '2021-09-22 22:03:24', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
(184, '@jyotisa', 'Peringatan', 'msg', '1', '2021-09-24 06:12:11', NULL, NULL, 78, NULL, NULL, 3, NULL, NULL),
(185, '@jyotisa', '[{\"name\":\"files\\/TBD I - 1905551092_1jvr34bh.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:12:11', NULL, NULL, 78, NULL, NULL, 3, NULL, NULL),
(186, '@jyotisa', 'Peringatan', 'msg', '1', '2021-10-01 06:12:19', NULL, NULL, 78, NULL, NULL, 3, NULL, 18),
(187, '@jyotisa', '[{\"name\":\"files\\/TBD I - 1905551092_1jvr34bh.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-10-01 06:12:19', NULL, NULL, 78, NULL, NULL, 3, NULL, 18),
(188, '@jyotisa', 'Anda telah membuat pesanan baru di HMTI Udayana dengan id 1', 'msg', '1', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
(189, 'oyoy', 'ini tes', 'msg', '1', '2021-09-22 22:26:08', 54, NULL, NULL, 3, 7, 4, NULL, NULL),
(190, 'oyoy', '[{\"name\":\"files\\/TBD I - 1905551092_pr7edbqr.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-22 22:26:08', 54, NULL, NULL, 3, 7, 4, NULL, NULL),
(191, 'oyoy', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:41:24', 55, NULL, NULL, 3, 7, 4, NULL, NULL),
(192, 'oyoy', '[{\"name\":\"files\\/TBD I - 1905551092_7cgspt16.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:41:24', 55, NULL, NULL, 3, 7, 4, NULL, NULL),
(193, 'jyotisabali@gmail.com', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, NULL, 3, 10, NULL),
(194, '@jyotisa', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, NULL, 3, 10, NULL),
(195, '@jyotisa', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, NULL, 3, 10, NULL),
(196, '@jyotisa', '[{\"name\":\"files\\/TBD I - 1905551092_7cgspt16.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, NULL, 3, 10, NULL),
(197, '081547655988', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, NULL, 3, 10, NULL),
(198, 'jyo', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, NULL, 3, 10, NULL),
(199, 'nyomanjyotisa@student.unud.ac.id', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, 7, 4, 10, NULL),
(200, 'oyoy', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, 7, 4, 10, NULL),
(201, 'oyoy', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, 7, 4, 10, NULL),
(202, 'oyoy', '[{\"name\":\"files\\/TBD I - 1905551092_7cgspt16.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, 7, 4, 10, NULL);

-- --------------------------------------------------------

--
-- Struktur dari tabel `outbox_whatsapp`
--

CREATE TABLE `outbox_whatsapp` (
  `id_outbox` BIGINT(20) NOT NULL,
  `chat_id` VARCHAR(100) DEFAULT NULL,
  `out_msg` TEXT DEFAULT NULL,
  `type` ENUM('msg','file','loc','img') DEFAULT 'msg',
  `flag` ENUM('1','2') DEFAULT NULL,
  `tgl` DATETIME DEFAULT NULL,
  `group_agenda_id` BIGINT(20) DEFAULT NULL,
  `group_agenda_private_id` BIGINT(20) DEFAULT NULL,
  `member_agenda_id` BIGINT(20) DEFAULT NULL,
  `group_id` BIGINT(20) DEFAULT NULL,
  `group_member_id` BIGINT(20) DEFAULT NULL,
  `member_id` BIGINT(20) DEFAULT NULL,
  `id_group_repeat` BIGINT(20) DEFAULT NULL,
  `id_personal_repeat` BIGINT(20) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=latin1;

--
-- Dumping data untuk tabel `outbox_whatsapp`
--

INSERT INTO `outbox_whatsapp` (`id_outbox`, `chat_id`, `out_msg`, `type`, `flag`, `tgl`, `group_agenda_id`, `group_agenda_private_id`, `member_agenda_id`, `group_id`, `group_member_id`, `member_id`, `id_group_repeat`, `id_personal_repeat`) VALUES
(101, '081547655988', 'Ini agenda pribadi realtime', 'msg', '1', '2021-09-09 08:25:30', NULL, NULL, 74, NULL, NULL, 3, NULL, NULL),
(102, '081547655988', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_1buxl9ql.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-09 08:25:30', NULL, NULL, 74, NULL, NULL, 3, NULL, NULL),
(103, '081547655988', 'agenda ondate', 'msg', '1', '2021-09-10 20:00:00', NULL, NULL, 75, NULL, NULL, 3, NULL, NULL),
(111, '081547655988', 'agenda repeat edit', 'msg', '1', '2021-11-04 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(112, '081547655988', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-04 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(113, '081547655988', 'agenda repeat edit', 'msg', '1', '2021-11-11 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(114, '081547655988', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-11 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(115, '081547655988', 'agenda repeat edit', 'msg', '1', '2021-11-18 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(116, '081547655988', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-18 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(117, '081547655988', 'agenda repeat edit', 'msg', '1', '2021-11-25 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(118, '081547655988', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-11-25 18:00:00', NULL, NULL, 76, NULL, NULL, 3, NULL, NULL),
(119, '081547655988', 'agenda by date', 'msg', '1', '2021-09-10 16:30:01', NULL, NULL, 77, NULL, NULL, 3, NULL, NULL),
(120, '081547655988', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_o57440xl.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-10 16:30:01', NULL, NULL, 77, NULL, NULL, 3, NULL, NULL),
(124, '081547655988', 'Peringatan', 'msg', '1', '2021-09-24 06:12:11', NULL, NULL, 78, NULL, NULL, 3, NULL, NULL),
(125, '081547655988', '[{\"name\":\"files\\/TBD I - 1905551092_1jvr34bh.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:12:11', NULL, NULL, 78, NULL, NULL, 3, NULL, NULL),
(126, '081547655988', 'Peringatan', 'msg', '1', '2021-10-01 06:12:19', NULL, NULL, 78, NULL, NULL, 3, NULL, 18),
(127, '081547655988', '[{\"name\":\"files\\/TBD I - 1905551092_1jvr34bh.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-10-01 06:12:19', NULL, NULL, 78, NULL, NULL, 3, NULL, 18),
(128, '081547655988', 'Akan diadakan job fair', 'msg', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, NULL, 3, 10, NULL),
(129, '081547655988', '[{\"name\":\"files\\/TBD I - 1905551092_7cgspt16.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 'file', '1', '2021-09-24 06:43:26', 55, NULL, NULL, 3, NULL, 3, 10, NULL);

-- --------------------------------------------------------

--
-- Struktur dari tabel `personal`
--

CREATE TABLE `personal` (
  `member_id` BIGINT(20) NOT NULL,
  `name` VARCHAR(100) DEFAULT NULL,
  `mail` VARCHAR(40) DEFAULT NULL,
  `password` VARCHAR(255) DEFAULT NULL,
  `country_id` BIGINT(20) DEFAULT NULL,
  `province_id` BIGINT(20) DEFAULT NULL,
  `valid` TINYINT(4) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `personal`
--

INSERT INTO `personal` (`member_id`, `name`, `mail`, `password`, `country_id`, `province_id`, `valid`) VALUES
(1, 'Hyper Admin', 'hyper@admin.com', '$2y$10$uPQto9Ff7QrcUwX8WfqJZuZgDYpNgdLgLe458UzH/rmAu4rR82Kyu', 102, 1667, 1),
(2, 'Nyoman Jyotisa', 'jyotisa1616@gmail.com', '$2y$10$NjEH7qpt7jm9J7FW/TLMc.hfESqCyq5RtmXxsBG0MnGicXs2S2YUe', 102, 1667, 1),
(3, 'Jyotisa', 'nyomanjyotisabali@gmail.com', '$2y$10$d79GpDMOcZ65Xlon0Kdjvug49E5h98YGXr5ygeNTkHcJNJNc9wns2', 102, 1667, 1),
(4, 'Nyoman', 'nyomanjyotisa@student.unud.ac.id', '$2y$10$2g0W4gSWNur/pOSTcl1.vuG9CIFU1M35TtbXo8o02ujr8DM.9GgE2', 102, 1667, 1);

-- --------------------------------------------------------

--
-- Struktur dari tabel `personal_agenda`
--

CREATE TABLE `personal_agenda` (
  `member_agenda_id` BIGINT(20) NOT NULL,
  `member_id` BIGINT(20) DEFAULT NULL,
  `loop_type` INT(11) DEFAULT NULL COMMENT 'real time, on date',
  `loop_value` DATETIME DEFAULT NULL,
  `message_content` VARCHAR(225) DEFAULT NULL COMMENT 'pesan yang akan dikirim',
  `attachment` VARCHAR(255) DEFAULT NULL,
  `repeat` INT(3) DEFAULT NULL,
  `repeat_type` ENUM('daily','weekly','monthly','yearly','bydate') DEFAULT NULL COMMENT 'jika bydate cek tabel personal_agenda_repeatdate'
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `personal_agenda`
--

INSERT INTO `personal_agenda` (`member_agenda_id`, `member_id`, `loop_type`, `loop_value`, `message_content`, `attachment`, `repeat`, `repeat_type`) VALUES
(1, 4, 2, '2021-08-31 08:02:29', 'Ini Agenda Pribade', '[{\"name\":\"files\\/group_member (7)_wofre6jy.xlsx\",\"usrName\":\"group_member (7).xlsx\",\"size\":8073,\"type\":\"application\\/vnd.openxmlformats-officedocument.spreadsheetml.sheet\",\"searchStr\":\"group_member (7).xlsx,!:sStrEnd\"}]', NULL, ''),
(2, 4, 1, '2021-09-01 17:00:00', 'Ini Agenda Pribadi 2', '', 0, ''),
(3, 4, 1, '2021-09-01 16:00:00', 'Ini Agenda Pribadi 3', '', 2, 'daily'),
(4, 4, 1, '2021-09-01 16:05:33', 'Ini Agenda Pribadi 4', '', 2, 'weekly'),
(5, 4, 1, '2021-09-01 16:06:12', 'Ini Agenda Pribadi 5', '', 2, 'monthly'),
(6, 4, 1, '2021-10-01 16:06:47', 'Ini Agenda Pribadi 6', '', 2, 'yearly'),
(7, 4, 1, '2021-09-01 16:08:49', 'Ini PErsonal Agenda 7', '', 2, 'bydate'),
(74, 3, 2, '2021-09-09 08:25:30', 'Ini agenda pribadi realtime', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_1buxl9ql.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', NULL, ''),
(75, 3, 1, '2021-09-10 20:00:00', 'agenda ondate', '', 0, ''),
(76, 3, 1, '2021-11-04 18:00:00', 'agenda repeat edit', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_ktjtybmi.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 3, 'weekly'),
(77, 3, 1, '2021-09-10 16:30:01', 'agenda by date', '[{\"name\":\"files\\/1905551092_Ringkasan Jurnal_ERP A_o57440xl.pdf\",\"usrName\":\"1905551092_Ringkasan Jurnal_ERP A.pdf\",\"size\":167626,\"type\":\"application\\/pdf\",\"searchStr\":\"1905551092_Ringkasan Jurnal_ERP A.pdf,!:sStrEnd\"}]', 2, 'bydate'),
(78, 3, 1, '2021-09-24 06:12:11', 'Peringatan', '[{\"name\":\"files\\/TBD I - 1905551092_1jvr34bh.pdf\",\"usrName\":\"TBD I - 1905551092.pdf\",\"size\":171995,\"type\":\"application\\/pdf\",\"searchStr\":\"TBD I - 1905551092.pdf,!:sStrEnd\"}]', 1, 'bydate');

-- --------------------------------------------------------

--
-- Struktur dari tabel `personal_agenda_repeat_date`
--

CREATE TABLE `personal_agenda_repeat_date` (
  `id_group_repeat` BIGINT(20) NOT NULL,
  `member_agenda_id` BIGINT(20) DEFAULT NULL,
  `dodate` DATETIME DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `personal_agenda_repeat_date`
--

INSERT INTO `personal_agenda_repeat_date` (`id_group_repeat`, `member_agenda_id`, `dodate`) VALUES
(1, 7, '2021-09-02 16:08:55'),
(2, 7, '2021-09-10 17:00:00'),
(12, NULL, '2021-09-29 18:55:11'),
(14, NULL, '2021-09-24 12:00:00'),
(15, NULL, '2021-09-17 15:00:00'),
(18, 78, '2021-10-01 06:12:19');

-- --------------------------------------------------------

--
-- Struktur dari tabel `personal_channel`
--

CREATE TABLE `personal_channel` (
  `member_ch_id` BIGINT(20) NOT NULL,
  `member_id` BIGINT(20) DEFAULT NULL,
  `channel_id` TINYINT(4) DEFAULT NULL,
  `chat_id` VARCHAR(50) DEFAULT NULL COMMENT 'username, email, idline'
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `personal_channel`
--

INSERT INTO `personal_channel` (`member_ch_id`, `member_id`, `channel_id`, `chat_id`) VALUES
(1, 2, 2, 'jyotisa1616@gmail.com'),
(2, 3, 2, 'jyotisabali@gmail.com'),
(3, 4, 2, 'nyomanjyotisa@student.unud.ac.id'),
(4, 3, 1, '@jyotisa'),
(5, 3, 3, '081547655988'),
(9, 3, 4, 'jyo'),
(10, 4, 1, 'oyoy');

-- --------------------------------------------------------

--
-- Struktur dari tabel `product_type`
--

CREATE TABLE `product_type` (
  `id_product_type` TINYINT(4) NOT NULL,
  `product_type` VARCHAR(100) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `product_type`
--

INSERT INTO `product_type` (`id_product_type`, `product_type`) VALUES
(1, 'food'),
(2, 'drink'),
(3, 'tailor'),
(4, 'motorcycle service'),
(5, 'car service'),
(6, 'room'),
(7, 'table');

-- --------------------------------------------------------

--
-- Struktur dari tabel `project5_uggroups`
--

CREATE TABLE `project5_uggroups` (
  `GroupID` INT(11) NOT NULL,
  `Label` VARCHAR(300) DEFAULT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `project5_uggroups`
--

INSERT INTO `project5_uggroups` (`GroupID`, `Label`) VALUES
(1, 'Personal'),
(2, 'Admin Group');

-- --------------------------------------------------------

--
-- Struktur dari tabel `project5_ugmembers`
--

CREATE TABLE `project5_ugmembers` (
  `UserName` VARCHAR(300) NOT NULL,
  `GroupID` INT(11) NOT NULL
) ENGINE=INNODB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `project5_ugmembers`
--

INSERT INTO `project5_ugmembers` (`UserName`, `GroupID`) VALUES
('a', -1),
('aa', 2),
('hyper@admin.com', -1),
('jyotisa1616@gmail.com', 2);

-- --------------------------------------------------------

--
-- Struktur dari tabel `project5_ugrights`
--

CREATE TABLE `project5_ugrights` (
  `TableName` varchar(300) NOT NULL,
  `GroupID` int(11) NOT NULL,
  `AccessMask` varchar(10) DEFAULT NULL,
  `Page` mediumtext DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `project5_ugrights`
--

INSERT INTO `project5_ugrights` (`TableName`, `GroupID`, `AccessMask`, `Page`) VALUES
('Add New Order', -1, 'ADESPIM', NULL),
('Add Order Review', -1, 'ADESPIM', NULL),
('add_checkin_review', -1, 'ADESPIM', NULL),
('add_new_checkin', -1, 'ADESPIM', NULL),
('admin_members', -1, 'ADESPIM', NULL),
('admin_rights', -1, 'ADESPIM', NULL),
('admin_users', -1, 'ADESPIM', NULL),
('business_type', -1, 'ADESPIM', NULL),
('channel', -1, 'AEDSPI', ''),
('country', -1, 'ADESPIM', NULL),
('group', -3, 'S', ''),
('group', -2, 'AS', ''),
('group', -1, 'AEDSPI', ''),
('group', 1, 'S', ''),
('group', 2, 'D', ''),
('Group as AdminView', -1, 'ADESPIM', NULL),
('group1', -1, 'M', ''),
('group11', -1, 'M', ''),
('group2', -1, 'M', ''),
('group21', -1, 'M', ''),
('group211', -1, 'M', ''),
('group212', -1, 'M', ''),
('group2121', -1, 'M', ''),
('group3', -1, 'M', ''),
('group4', -1, 'M', ''),
('group_agenda', -1, 'M', ''),
('group_agenda', 2, 'AEDSPI', ''),
('group_agenda1', -1, 'M', ''),
('group_agenda_general_logs', -1, 'M', ''),
('group_agenda_general_logs', 2, 'ESPI', ''),
('group_agenda_private', -1, 'M', ''),
('group_agenda_private', 2, 'AEDSPI', ''),
('group_agenda_repeat_date', -1, 'M', ''),
('group_agenda_repeat_date', 2, 'AEDSPI', ''),
('group_agenda_type', -1, 'M', ''),
('group_agenda_type', 2, 'AEDSPI', ''),
('group_agenda_type1', -1, 'M', ''),
('group_agenda_type11', -1, 'M', ''),
('group_as_admin', 2, 'ESPI', ''),
('group_joined_view', -1, 'ADESPIM', NULL),
('group_member', -2, 'AEDSPI', ''),
('group_member', -1, 'M', ''),
('group_member', 1, 'AEDSPI', ''),
('group_member', 2, 'DESPI', ''),
('group_member1', -1, 'M', ''),
('group_member11', -1, 'M', ''),
('group_member_agenda', 2, 'AEDSPI', ''),
('group_member_agenda_subscriber', -2, 'AEDSPI', ''),
('group_member_agenda_subscriber', -1, 'M', ''),
('group_member_checkin', -1, 'DE', ''),
('group_member_checkin', 2, 'AEDSPI', ''),
('group_member_order', -2, 'AESPIM', ''),
('group_member_order', -1, 'D', ''),
('group_member_order', 1, 'ASPIM', ''),
('group_member_order', 2, 'DSPIM', ''),
('group_member_order_detail', -2, 'ADSPI', ''),
('group_member_order_detail', -1, 'DE', ''),
('group_member_order_detail', 1, 'ASPI', ''),
('group_member_order_detail', 2, 'DESPI', ''),
('group_operation_off', -1, 'M', ''),
('group_operation_off', 2, 'AEDSPI', ''),
('group_product', -1, 'M', ''),
('group_product', 2, 'AEDSPI', ''),
('group_product1', -1, 'M', ''),
('group_product_view', -1, 'ADESPIM', NULL),
('group_room_view', -1, 'ADESPIM', NULL),
('group_user_type', -1, 'AEDSPI', ''),
('joined_group', -2, 'SPI', ''),
('joined_group', 2, 'ESPI', ''),
('kasir', 2, 'ESPI', ''),
('lookup_stok', 2, 'AEDSPI', ''),
('lookup_token', -1, 'M', ''),
('lookup_token', 2, 'AEDSPI', ''),
('loop_type', -1, 'AEDSPI', ''),
('member checkin', -2, 'ASPI', ''),
('member checkin', -1, 'DEM', ''),
('membership_type', -1, 'AEDSPI', ''),
('member_checkin', -2, 'ASPI', ''),
('new_group_request', -1, 'ESPI', ''),
('not_reviewed_checkin_view', -1, 'ADESPIM', NULL),
('not_reviewed_order_view', -1, 'ADESPIM', NULL),
('not_yet_valid_order', -1, 'ADESPIM', NULL),
('order_type', -1, 'AEDSPI', ''),
('payment', -1, 'A', ''),
('payment', 2, 'AESPI', ''),
('personal', -2, 'E', ''),
('personal', 1, 'ESPIM', ''),
('personal_agenda', -2, 'AEDSPI', ''),
('personal_agenda', 1, 'AEDSPIM', ''),
('personal_agenda_repeat_date', -2, 'AEDSPI', ''),
('personal_agenda_repeat_date', -1, 'M', ''),
('personal_agenda_repeat_date', 1, 'S', ''),
('personal_channel', -2, 'AEDSPI', ''),
('personal_channel', 1, 'AEDSPI', ''),
('personal_channel', 2, 'AEDSPIM', ''),
('product_for_order', -2, 'SP', ''),
('product_for_order', 2, 'SP', ''),
('product_type', -1, 'AEDSPI', ''),
('product_type', 1, 'AEDSPI', ''),
('project5_uggroups', -1, 'M', ''),
('project5_ugmembers', -1, 'M', ''),
('project5_ugrights', -1, 'M', ''),
('province', -1, 'ADESPIM', NULL),
('review_checkin', -2, 'ESPIM', ''),
('review_checkin', -1, 'AD', ''),
('review_checkin', 1, 'ESPIM', ''),
('review_order', -2, 'ESPI', ''),
('review_order', -1, 'ADM', ''),
('review_order', 1, 'ESPI', ''),
('Room View', -1, 'ADESPIM', NULL),
('stock_type', -1, 'AEDSPI', ''),
('update_order_progess', -1, 'ADM', ''),
('update_order_progess', 2, 'ESPI', ''),
('validate_checkin', -1, 'ADM', ''),
('validate_checkin', 2, 'ESPI', ''),
('validate_order', -1, 'ADM', ''),
('validate_order', 2, 'ESPI', ''),
('your_group', -2, 'ESPI', ''),
('your_group', 2, 'DESPI', '');

-- --------------------------------------------------------

--
-- Struktur dari tabel `province`
--

CREATE TABLE `province` (
  `province_id` bigint(20) NOT NULL,
  `country_id` bigint(20) DEFAULT NULL,
  `province_name` varchar(100) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

--
-- Dumping data untuk tabel `province`
--

INSERT INTO `province` (`province_id`, `country_id`, `province_name`) VALUES
(1, 101, 'Andaman and Nicobar Islands'),
(2, 101, 'Andhra Pradesh'),
(3, 101, 'Arunachal Pradesh'),
(4, 101, 'Assam'),
(5, 101, 'Bihar'),
(6, 101, 'Chandigarh'),
(7, 101, 'Chhattisgarh'),
(8, 101, 'Dadra and Nagar Haveli'),
(9, 101, 'Daman and Diu'),
(10, 101, 'Delhi'),
(11, 101, 'Goa'),
(12, 101, 'Gujarat'),
(13, 101, 'Haryana'),
(14, 101, 'Himachal Pradesh'),
(15, 101, 'Jammu and Kashmir'),
(16, 101, 'Jharkhand'),
(17, 101, 'Karnataka'),
(18, 101, 'Kenmore'),
(19, 101, 'Kerala'),
(20, 101, 'Lakshadweep'),
(21, 101, 'Madhya Pradesh'),
(22, 101, 'Maharashtra'),
(23, 101, 'Manipur'),
(24, 101, 'Meghalaya'),
(25, 101, 'Mizoram'),
(26, 101, 'Nagaland'),
(27, 101, 'Narora'),
(28, 101, 'Natwar'),
(29, 101, 'Odisha'),
(30, 101, 'Paschim Medinipur'),
(31, 101, 'Pondicherry'),
(32, 101, 'Punjab'),
(33, 101, 'Rajasthan'),
(34, 101, 'Sikkim'),
(35, 101, 'Tamil Nadu'),
(36, 101, 'Telangana'),
(37, 101, 'Tripura'),
(38, 101, 'Uttar Pradesh'),
(39, 101, 'Uttarakhand'),
(40, 101, 'Vaishali'),
(41, 101, 'West Bengal'),
(42, 1, 'Badakhshan'),
(43, 1, 'Badgis'),
(44, 1, 'Baglan'),
(45, 1, 'Balkh'),
(46, 1, 'Bamiyan'),
(47, 1, 'Farah'),
(48, 1, 'Faryab'),
(49, 1, 'Gawr'),
(50, 1, 'Gazni'),
(51, 1, 'Herat'),
(52, 1, 'Hilmand'),
(53, 1, 'Jawzjan'),
(54, 1, 'Kabul'),
(55, 1, 'Kapisa'),
(56, 1, 'Khawst'),
(57, 1, 'Kunar'),
(58, 1, 'Lagman'),
(59, 1, 'Lawghar'),
(60, 1, 'Nangarhar'),
(61, 1, 'Nimruz'),
(62, 1, 'Nuristan'),
(63, 1, 'Paktika'),
(64, 1, 'Paktiya'),
(65, 1, 'Parwan'),
(66, 1, 'Qandahar'),
(67, 1, 'Qunduz'),
(68, 1, 'Samangan'),
(69, 1, 'Sar-e Pul'),
(70, 1, 'Takhar'),
(71, 1, 'Uruzgan'),
(72, 1, 'Wardag'),
(73, 1, 'Zabul'),
(74, 2, 'Berat'),
(75, 2, 'Bulqize'),
(76, 2, 'Delvine'),
(77, 2, 'Devoll'),
(78, 2, 'Dibre'),
(79, 2, 'Durres'),
(80, 2, 'Elbasan'),
(81, 2, 'Fier'),
(82, 2, 'Gjirokaster'),
(83, 2, 'Gramsh'),
(84, 2, 'Has'),
(85, 2, 'Kavaje'),
(86, 2, 'Kolonje'),
(87, 2, 'Korce'),
(88, 2, 'Kruje'),
(89, 2, 'Kucove'),
(90, 2, 'Kukes'),
(91, 2, 'Kurbin'),
(92, 2, 'Lezhe'),
(93, 2, 'Librazhd'),
(94, 2, 'Lushnje'),
(95, 2, 'Mallakaster'),
(96, 2, 'Malsi e Madhe'),
(97, 2, 'Mat'),
(98, 2, 'Mirdite'),
(99, 2, 'Peqin'),
(100, 2, 'Permet'),
(101, 2, 'Pogradec'),
(102, 2, 'Puke'),
(103, 2, 'Sarande'),
(104, 2, 'Shkoder'),
(105, 2, 'Skrapar'),
(106, 2, 'Tepelene'),
(107, 2, 'Tirane'),
(108, 2, 'Tropoje'),
(109, 2, 'Vlore'),
(110, 3, '\'Ayn Daflah'),
(111, 3, '\'Ayn Tamushanat'),
(112, 3, 'Adrar'),
(113, 3, 'Algiers'),
(114, 3, 'Annabah'),
(115, 3, 'Bashshar'),
(116, 3, 'Batnah'),
(117, 3, 'Bijayah'),
(118, 3, 'Biskrah'),
(119, 3, 'Blidah'),
(120, 3, 'Buirah'),
(121, 3, 'Bumardas'),
(122, 3, 'Burj Bu Arririj'),
(123, 3, 'Ghalizan'),
(124, 3, 'Ghardayah'),
(125, 3, 'Ilizi'),
(126, 3, 'Jijili'),
(127, 3, 'Jilfah'),
(128, 3, 'Khanshalah'),
(129, 3, 'Masilah'),
(130, 3, 'Midyah'),
(131, 3, 'Milah'),
(132, 3, 'Muaskar'),
(133, 3, 'Mustaghanam'),
(134, 3, 'Naama'),
(135, 3, 'Oran'),
(136, 3, 'Ouargla'),
(137, 3, 'Qalmah'),
(138, 3, 'Qustantinah'),
(139, 3, 'Sakikdah'),
(140, 3, 'Satif'),
(141, 3, 'Sayda\''),
(142, 3, 'Sidi ban-al-\'Abbas'),
(143, 3, 'Suq Ahras'),
(144, 3, 'Tamanghasat'),
(145, 3, 'Tibazah'),
(146, 3, 'Tibissah'),
(147, 3, 'Tilimsan'),
(148, 3, 'Tinduf'),
(149, 3, 'Tisamsilt'),
(150, 3, 'Tiyarat'),
(151, 3, 'Tizi Wazu'),
(152, 3, 'Umm-al-Bawaghi'),
(153, 3, 'Wahran'),
(154, 3, 'Warqla'),
(155, 3, 'Wilaya d Alger'),
(156, 3, 'Wilaya de Bejaia'),
(157, 3, 'Wilaya de Constantine'),
(158, 3, 'al-Aghwat'),
(159, 3, 'al-Bayadh'),
(160, 3, 'al-Jaza\'ir'),
(161, 3, 'al-Wad'),
(162, 3, 'ash-Shalif'),
(163, 3, 'at-Tarif'),
(164, 4, 'Eastern'),
(165, 4, 'Manu\'a'),
(166, 4, 'Swains Island'),
(167, 4, 'Western'),
(168, 5, 'Andorra la Vella'),
(169, 5, 'Canillo'),
(170, 5, 'Encamp'),
(171, 5, 'La Massana'),
(172, 5, 'Les Escaldes'),
(173, 5, 'Ordino'),
(174, 5, 'Sant Julia de Loria'),
(175, 6, 'Bengo'),
(176, 6, 'Benguela'),
(177, 6, 'Bie'),
(178, 6, 'Cabinda'),
(179, 6, 'Cunene'),
(180, 6, 'Huambo'),
(181, 6, 'Huila'),
(182, 6, 'Kuando-Kubango'),
(183, 6, 'Kwanza Norte'),
(184, 6, 'Kwanza Sul'),
(185, 6, 'Luanda'),
(186, 6, 'Lunda Norte'),
(187, 6, 'Lunda Sul'),
(188, 6, 'Malanje'),
(189, 6, 'Moxico'),
(190, 6, 'Namibe'),
(191, 6, 'Uige'),
(192, 6, 'Zaire'),
(193, 7, 'Other Provinces'),
(194, 8, 'Sector claimed by Argentina/Ch'),
(195, 8, 'Sector claimed by Argentina/UK'),
(196, 8, 'Sector claimed by Australia'),
(197, 8, 'Sector claimed by France'),
(198, 8, 'Sector claimed by New Zealand'),
(199, 8, 'Sector claimed by Norway'),
(200, 8, 'Unclaimed Sector'),
(201, 9, 'Barbuda'),
(202, 9, 'Saint George'),
(203, 9, 'Saint John'),
(204, 9, 'Saint Mary'),
(205, 9, 'Saint Paul'),
(206, 9, 'Saint Peter'),
(207, 9, 'Saint Philip'),
(208, 10, 'Buenos Aires'),
(209, 10, 'Catamarca'),
(210, 10, 'Chaco'),
(211, 10, 'Chubut'),
(212, 10, 'Cordoba'),
(213, 10, 'Corrientes'),
(214, 10, 'Distrito Federal'),
(215, 10, 'Entre Rios'),
(216, 10, 'Formosa'),
(217, 10, 'Jujuy'),
(218, 10, 'La Pampa'),
(219, 10, 'La Rioja'),
(220, 10, 'Mendoza'),
(221, 10, 'Misiones'),
(222, 10, 'Neuquen'),
(223, 10, 'Rio Negro'),
(224, 10, 'Salta'),
(225, 10, 'San Juan'),
(226, 10, 'San Luis'),
(227, 10, 'Santa Cruz'),
(228, 10, 'Santa Fe'),
(229, 10, 'Santiago del Estero'),
(230, 10, 'Tierra del Fuego'),
(231, 10, 'Tucuman'),
(232, 11, 'Aragatsotn'),
(233, 11, 'Ararat'),
(234, 11, 'Armavir'),
(235, 11, 'Gegharkunik'),
(236, 11, 'Kotaik'),
(237, 11, 'Lori'),
(238, 11, 'Shirak'),
(239, 11, 'Stepanakert'),
(240, 11, 'Syunik'),
(241, 11, 'Tavush'),
(242, 11, 'Vayots Dzor'),
(243, 11, 'Yerevan'),
(244, 12, 'Aruba'),
(245, 13, 'Auckland'),
(246, 13, 'Australian Capital Territory'),
(247, 13, 'Balgowlah'),
(248, 13, 'Balmain'),
(249, 13, 'Bankstown'),
(250, 13, 'Baulkham Hills'),
(251, 13, 'Bonnet Bay'),
(252, 13, 'Camberwell'),
(253, 13, 'Carole Park'),
(254, 13, 'Castle Hill'),
(255, 13, 'Caulfield'),
(256, 13, 'Chatswood'),
(257, 13, 'Cheltenham'),
(258, 13, 'Cherrybrook'),
(259, 13, 'Clayton'),
(260, 13, 'Collingwood'),
(261, 13, 'Frenchs Forest'),
(262, 13, 'Hawthorn'),
(263, 13, 'Jannnali'),
(264, 13, 'Knoxfield'),
(265, 13, 'Melbourne'),
(266, 13, 'New South Wales'),
(267, 13, 'Northern Territory'),
(268, 13, 'Perth'),
(269, 13, 'Queensland'),
(270, 13, 'South Australia'),
(271, 13, 'Tasmania'),
(272, 13, 'Templestowe'),
(273, 13, 'Victoria'),
(274, 13, 'Werribee south'),
(275, 13, 'Western Australia'),
(276, 13, 'Wheeler'),
(277, 14, 'Bundesland Salzburg'),
(278, 14, 'Bundesland Steiermark'),
(279, 14, 'Bundesland Tirol'),
(280, 14, 'Burgenland'),
(281, 14, 'Carinthia'),
(282, 14, 'Karnten'),
(283, 14, 'Liezen'),
(284, 14, 'Lower Austria'),
(285, 14, 'Niederosterreich'),
(286, 14, 'Oberosterreich'),
(287, 14, 'Salzburg'),
(288, 14, 'Schleswig-Holstein'),
(289, 14, 'Steiermark'),
(290, 14, 'Styria'),
(291, 14, 'Tirol'),
(292, 14, 'Upper Austria'),
(293, 14, 'Vorarlberg'),
(294, 14, 'Wien'),
(295, 15, 'Abseron'),
(296, 15, 'Baki Sahari'),
(297, 15, 'Ganca'),
(298, 15, 'Ganja'),
(299, 15, 'Kalbacar'),
(300, 15, 'Lankaran'),
(301, 15, 'Mil-Qarabax'),
(302, 15, 'Mugan-Salyan'),
(303, 15, 'Nagorni-Qarabax'),
(304, 15, 'Naxcivan'),
(305, 15, 'Priaraks'),
(306, 15, 'Qazax'),
(307, 15, 'Saki'),
(308, 15, 'Sirvan'),
(309, 15, 'Xacmaz'),
(310, 16, 'Abaco'),
(311, 16, 'Acklins Island'),
(312, 16, 'Andros'),
(313, 16, 'Berry Islands'),
(314, 16, 'Biminis'),
(315, 16, 'Cat Island'),
(316, 16, 'Crooked Island'),
(317, 16, 'Eleuthera'),
(318, 16, 'Exuma and Cays'),
(319, 16, 'Grand Bahama'),
(320, 16, 'Inagua Islands'),
(321, 16, 'Long Island'),
(322, 16, 'Mayaguana'),
(323, 16, 'New Providence'),
(324, 16, 'Ragged Island'),
(325, 16, 'Rum Cay'),
(326, 16, 'San Salvador'),
(327, 17, '\'Isa'),
(328, 17, 'Badiyah'),
(329, 17, 'Hidd'),
(330, 17, 'Jidd Hafs'),
(331, 17, 'Mahama'),
(332, 17, 'Manama'),
(333, 17, 'Sitrah'),
(334, 17, 'al-Manamah'),
(335, 17, 'al-Muharraq'),
(336, 17, 'ar-Rifa\'a'),
(337, 18, 'Bagar Hat'),
(338, 18, 'Bandarban'),
(339, 18, 'Barguna'),
(340, 18, 'Barisal'),
(341, 18, 'Bhola'),
(342, 18, 'Bogora'),
(343, 18, 'Brahman Bariya'),
(344, 18, 'Chandpur'),
(345, 18, 'Chattagam'),
(346, 18, 'Chittagong Division'),
(347, 18, 'Chuadanga'),
(348, 18, 'Dhaka'),
(349, 18, 'Dinajpur'),
(350, 18, 'Faridpur'),
(351, 18, 'Feni'),
(352, 18, 'Gaybanda'),
(353, 18, 'Gazipur'),
(354, 18, 'Gopalganj'),
(355, 18, 'Habiganj'),
(356, 18, 'Jaipur Hat'),
(357, 18, 'Jamalpur'),
(358, 18, 'Jessor'),
(359, 18, 'Jhalakati'),
(360, 18, 'Jhanaydah'),
(361, 18, 'Khagrachhari'),
(362, 18, 'Khulna'),
(363, 18, 'Kishorganj'),
(364, 18, 'Koks Bazar'),
(365, 18, 'Komilla'),
(366, 18, 'Kurigram'),
(367, 18, 'Kushtiya'),
(368, 18, 'Lakshmipur'),
(369, 18, 'Lalmanir Hat'),
(370, 18, 'Madaripur'),
(371, 18, 'Magura'),
(372, 18, 'Maimansingh'),
(373, 18, 'Manikganj'),
(374, 18, 'Maulvi Bazar'),
(375, 18, 'Meherpur'),
(376, 18, 'Munshiganj'),
(377, 18, 'Naral'),
(378, 18, 'Narayanganj'),
(379, 18, 'Narsingdi'),
(380, 18, 'Nator'),
(381, 18, 'Naugaon'),
(382, 18, 'Nawabganj'),
(383, 18, 'Netrakona'),
(384, 18, 'Nilphamari'),
(385, 18, 'Noakhali'),
(386, 18, 'Pabna'),
(387, 18, 'Panchagarh'),
(388, 18, 'Patuakhali'),
(389, 18, 'Pirojpur'),
(390, 18, 'Rajbari'),
(391, 18, 'Rajshahi'),
(392, 18, 'Rangamati'),
(393, 18, 'Rangpur'),
(394, 18, 'Satkhira'),
(395, 18, 'Shariatpur'),
(396, 18, 'Sherpur'),
(397, 18, 'Silhat'),
(398, 18, 'Sirajganj'),
(399, 18, 'Sunamganj'),
(400, 18, 'Tangayal'),
(401, 18, 'Thakurgaon'),
(402, 19, 'Christ Church'),
(403, 19, 'Saint Andrew'),
(404, 19, 'Saint George'),
(405, 19, 'Saint James'),
(406, 19, 'Saint John'),
(407, 19, 'Saint Joseph'),
(408, 19, 'Saint Lucy'),
(409, 19, 'Saint Michael'),
(410, 19, 'Saint Peter'),
(411, 19, 'Saint Philip'),
(412, 19, 'Saint Thomas'),
(413, 20, 'Brest'),
(414, 20, 'Homjel\''),
(415, 20, 'Hrodna'),
(416, 20, 'Mahiljow'),
(417, 20, 'Mahilyowskaya Voblasts'),
(418, 20, 'Minsk'),
(419, 20, 'Minskaja Voblasts\''),
(420, 20, 'Petrik'),
(421, 20, 'Vicebsk'),
(422, 21, 'Antwerpen'),
(423, 21, 'Berchem'),
(424, 21, 'Brabant'),
(425, 21, 'Brabant Wallon'),
(426, 21, 'Brussel'),
(427, 21, 'East Flanders'),
(428, 21, 'Hainaut'),
(429, 21, 'Liege'),
(430, 21, 'Limburg'),
(431, 21, 'Luxembourg'),
(432, 21, 'Namur'),
(433, 21, 'Ontario'),
(434, 21, 'Oost-Vlaanderen'),
(435, 21, 'Provincie Brabant'),
(436, 21, 'Vlaams-Brabant'),
(437, 21, 'Wallonne'),
(438, 21, 'West-Vlaanderen'),
(439, 22, 'Belize'),
(440, 22, 'Cayo'),
(441, 22, 'Corozal'),
(442, 22, 'Orange Walk'),
(443, 22, 'Stann Creek'),
(444, 22, 'Toledo'),
(445, 23, 'Alibori'),
(446, 23, 'Atacora'),
(447, 23, 'Atlantique'),
(448, 23, 'Borgou'),
(449, 23, 'Collines'),
(450, 23, 'Couffo'),
(451, 23, 'Donga'),
(452, 23, 'Littoral'),
(453, 23, 'Mono'),
(454, 23, 'Oueme'),
(455, 23, 'Plateau'),
(456, 23, 'Zou'),
(457, 24, 'Hamilton'),
(458, 24, 'Saint George'),
(459, 25, 'Bumthang'),
(460, 25, 'Chhukha'),
(461, 25, 'Chirang'),
(462, 25, 'Daga'),
(463, 25, 'Geylegphug'),
(464, 25, 'Ha'),
(465, 25, 'Lhuntshi'),
(466, 25, 'Mongar'),
(467, 25, 'Pemagatsel'),
(468, 25, 'Punakha'),
(469, 25, 'Rinpung'),
(470, 25, 'Samchi'),
(471, 25, 'Samdrup Jongkhar'),
(472, 25, 'Shemgang'),
(473, 25, 'Tashigang'),
(474, 25, 'Timphu'),
(475, 25, 'Tongsa'),
(476, 25, 'Wangdiphodrang'),
(477, 26, 'Beni'),
(478, 26, 'Chuquisaca'),
(479, 26, 'Cochabamba'),
(480, 26, 'La Paz'),
(481, 26, 'Oruro'),
(482, 26, 'Pando'),
(483, 26, 'Potosi'),
(484, 26, 'Santa Cruz'),
(485, 26, 'Tarija'),
(486, 27, 'Federacija Bosna i Hercegovina'),
(487, 27, 'Republika Srpska'),
(488, 28, 'Central Bobonong'),
(489, 28, 'Central Boteti'),
(490, 28, 'Central Mahalapye'),
(491, 28, 'Central Serowe-Palapye'),
(492, 28, 'Central Tutume'),
(493, 28, 'Chobe'),
(494, 28, 'Francistown'),
(495, 28, 'Gaborone'),
(496, 28, 'Ghanzi'),
(497, 28, 'Jwaneng'),
(498, 28, 'Kgalagadi North'),
(499, 28, 'Kgalagadi South'),
(500, 28, 'Kgatleng'),
(501, 28, 'Kweneng'),
(502, 28, 'Lobatse'),
(503, 28, 'Ngamiland'),
(504, 28, 'Ngwaketse'),
(505, 28, 'North East'),
(506, 28, 'Okavango'),
(507, 28, 'Orapa'),
(508, 28, 'Selibe Phikwe'),
(509, 28, 'South East'),
(510, 28, 'Sowa'),
(511, 29, 'Bouvet Island'),
(512, 30, 'Acre'),
(513, 30, 'Alagoas'),
(514, 30, 'Amapa'),
(515, 30, 'Amazonas'),
(516, 30, 'Bahia'),
(517, 30, 'Ceara'),
(518, 30, 'Distrito Federal'),
(519, 30, 'Espirito Santo'),
(520, 30, 'Estado de Sao Paulo'),
(521, 30, 'Goias'),
(522, 30, 'Maranhao'),
(523, 30, 'Mato Grosso'),
(524, 30, 'Mato Grosso do Sul'),
(525, 30, 'Minas Gerais'),
(526, 30, 'Para'),
(527, 30, 'Paraiba'),
(528, 30, 'Parana'),
(529, 30, 'Pernambuco'),
(530, 30, 'Piaui'),
(531, 30, 'Rio Grande do Norte'),
(532, 30, 'Rio Grande do Sul'),
(533, 30, 'Rio de Janeiro'),
(534, 30, 'Rondonia'),
(535, 30, 'Roraima'),
(536, 30, 'Santa Catarina'),
(537, 30, 'Sao Paulo'),
(538, 30, 'Sergipe'),
(539, 30, 'Tocantins'),
(540, 31, 'British Indian Ocean Territory'),
(541, 32, 'Belait'),
(542, 32, 'Brunei-Muara'),
(543, 32, 'Temburong'),
(544, 32, 'Tutong'),
(545, 33, 'Blagoevgrad'),
(546, 33, 'Burgas'),
(547, 33, 'Dobrich'),
(548, 33, 'Gabrovo'),
(549, 33, 'Haskovo'),
(550, 33, 'Jambol'),
(551, 33, 'Kardzhali'),
(552, 33, 'Kjustendil'),
(553, 33, 'Lovech'),
(554, 33, 'Montana'),
(555, 33, 'Oblast Sofiya-Grad'),
(556, 33, 'Pazardzhik'),
(557, 33, 'Pernik'),
(558, 33, 'Pleven'),
(559, 33, 'Plovdiv'),
(560, 33, 'Razgrad'),
(561, 33, 'Ruse'),
(562, 33, 'Shumen'),
(563, 33, 'Silistra'),
(564, 33, 'Sliven'),
(565, 33, 'Smoljan'),
(566, 33, 'Sofija grad'),
(567, 33, 'Sofijska oblast'),
(568, 33, 'Stara Zagora'),
(569, 33, 'Targovishte'),
(570, 33, 'Varna'),
(571, 33, 'Veliko Tarnovo'),
(572, 33, 'Vidin'),
(573, 33, 'Vraca'),
(574, 33, 'Yablaniza'),
(575, 34, 'Bale'),
(576, 34, 'Bam'),
(577, 34, 'Bazega'),
(578, 34, 'Bougouriba'),
(579, 34, 'Boulgou'),
(580, 34, 'Boulkiemde'),
(581, 34, 'Comoe'),
(582, 34, 'Ganzourgou'),
(583, 34, 'Gnagna'),
(584, 34, 'Gourma'),
(585, 34, 'Houet'),
(586, 34, 'Ioba'),
(587, 34, 'Kadiogo'),
(588, 34, 'Kenedougou'),
(589, 34, 'Komandjari'),
(590, 34, 'Kompienga'),
(591, 34, 'Kossi'),
(592, 34, 'Kouritenga'),
(593, 34, 'Kourweogo'),
(594, 34, 'Leraba'),
(595, 34, 'Mouhoun'),
(596, 34, 'Nahouri'),
(597, 34, 'Namentenga'),
(598, 34, 'Noumbiel'),
(599, 34, 'Oubritenga'),
(600, 34, 'Oudalan'),
(601, 34, 'Passore'),
(602, 34, 'Poni'),
(603, 34, 'Sanguie'),
(604, 34, 'Sanmatenga'),
(605, 34, 'Seno'),
(606, 34, 'Sissili'),
(607, 34, 'Soum'),
(608, 34, 'Sourou'),
(609, 34, 'Tapoa'),
(610, 34, 'Tuy'),
(611, 34, 'Yatenga'),
(612, 34, 'Zondoma'),
(613, 34, 'Zoundweogo'),
(614, 35, 'Bubanza'),
(615, 35, 'Bujumbura'),
(616, 35, 'Bururi'),
(617, 35, 'Cankuzo'),
(618, 35, 'Cibitoke'),
(619, 35, 'Gitega'),
(620, 35, 'Karuzi'),
(621, 35, 'Kayanza'),
(622, 35, 'Kirundo'),
(623, 35, 'Makamba'),
(624, 35, 'Muramvya'),
(625, 35, 'Muyinga'),
(626, 35, 'Ngozi'),
(627, 35, 'Rutana'),
(628, 35, 'Ruyigi'),
(629, 36, 'Banteay Mean Chey'),
(630, 36, 'Bat Dambang'),
(631, 36, 'Kampong Cham'),
(632, 36, 'Kampong Chhnang'),
(633, 36, 'Kampong Spoeu'),
(634, 36, 'Kampong Thum'),
(635, 36, 'Kampot'),
(636, 36, 'Kandal'),
(637, 36, 'Kaoh Kong'),
(638, 36, 'Kracheh'),
(639, 36, 'Krong Kaeb'),
(640, 36, 'Krong Pailin'),
(641, 36, 'Krong Preah Sihanouk'),
(642, 36, 'Mondol Kiri'),
(643, 36, 'Otdar Mean Chey'),
(644, 36, 'Phnum Penh'),
(645, 36, 'Pousat'),
(646, 36, 'Preah Vihear'),
(647, 36, 'Prey Veaeng'),
(648, 36, 'Rotanak Kiri'),
(649, 36, 'Siem Reab'),
(650, 36, 'Stueng Traeng'),
(651, 36, 'Svay Rieng'),
(652, 36, 'Takaev'),
(653, 37, 'Adamaoua'),
(654, 37, 'Centre'),
(655, 37, 'Est'),
(656, 37, 'Littoral'),
(657, 37, 'Nord'),
(658, 37, 'Nord Extreme'),
(659, 37, 'Nordouest'),
(660, 37, 'Ouest'),
(661, 37, 'Sud'),
(662, 37, 'Sudouest'),
(663, 38, 'Alberta'),
(664, 38, 'British Columbia'),
(665, 38, 'Manitoba'),
(666, 38, 'New Brunswick'),
(667, 38, 'Newfoundland and Labrador'),
(668, 38, 'Northwest Territories'),
(669, 38, 'Nova Scotia'),
(670, 38, 'Nunavut'),
(671, 38, 'Ontario'),
(672, 38, 'Prince Edward Island'),
(673, 38, 'Quebec'),
(674, 38, 'Saskatchewan'),
(675, 38, 'Yukon'),
(676, 39, 'Boavista'),
(677, 39, 'Brava'),
(678, 39, 'Fogo'),
(679, 39, 'Maio'),
(680, 39, 'Sal'),
(681, 39, 'Santo Antao'),
(682, 39, 'Sao Nicolau'),
(683, 39, 'Sao Tiago'),
(684, 39, 'Sao Vicente'),
(685, 40, 'Grand Cayman'),
(686, 41, 'Bamingui-Bangoran'),
(687, 41, 'Bangui'),
(688, 41, 'Basse-Kotto'),
(689, 41, 'Haut-Mbomou'),
(690, 41, 'Haute-Kotto'),
(691, 41, 'Kemo'),
(692, 41, 'Lobaye'),
(693, 41, 'Mambere-Kadei'),
(694, 41, 'Mbomou'),
(695, 41, 'Nana-Gribizi'),
(696, 41, 'Nana-Mambere'),
(697, 41, 'Ombella Mpoko'),
(698, 41, 'Ouaka'),
(699, 41, 'Ouham'),
(700, 41, 'Ouham-Pende'),
(701, 41, 'Sangha-Mbaere'),
(702, 41, 'Vakaga'),
(703, 42, 'Batha'),
(704, 42, 'Biltine'),
(705, 42, 'Bourkou-Ennedi-Tibesti'),
(706, 42, 'Chari-Baguirmi'),
(707, 42, 'Guera'),
(708, 42, 'Kanem'),
(709, 42, 'Lac'),
(710, 42, 'Logone Occidental'),
(711, 42, 'Logone Oriental'),
(712, 42, 'Mayo-Kebbi'),
(713, 42, 'Moyen-Chari'),
(714, 42, 'Ouaddai'),
(715, 42, 'Salamat'),
(716, 42, 'Tandjile'),
(717, 43, 'Aisen'),
(718, 43, 'Antofagasta'),
(719, 43, 'Araucania'),
(720, 43, 'Atacama'),
(721, 43, 'Bio Bio'),
(722, 43, 'Coquimbo'),
(723, 43, 'Libertador General Bernardo O\''),
(724, 43, 'Los Lagos'),
(725, 43, 'Magellanes'),
(726, 43, 'Maule'),
(727, 43, 'Metropolitana'),
(728, 43, 'Metropolitana de Santiago'),
(729, 43, 'Tarapaca'),
(730, 43, 'Valparaiso'),
(731, 44, 'Anhui'),
(734, 44, 'Aomen'),
(735, 44, 'Beijing'),
(736, 44, 'Beijing Shi'),
(737, 44, 'Chongqing'),
(738, 44, 'Fujian'),
(740, 44, 'Gansu'),
(741, 44, 'Guangdong'),
(743, 44, 'Guangxi'),
(744, 44, 'Guizhou'),
(745, 44, 'Hainan'),
(746, 44, 'Hebei'),
(747, 44, 'Heilongjiang'),
(748, 44, 'Henan'),
(749, 44, 'Hubei'),
(750, 44, 'Hunan'),
(751, 44, 'Jiangsu'),
(753, 44, 'Jiangxi'),
(754, 44, 'Jilin'),
(755, 44, 'Liaoning'),
(757, 44, 'Nei Monggol'),
(758, 44, 'Ningxia Hui'),
(759, 44, 'Qinghai'),
(760, 44, 'Shaanxi'),
(761, 44, 'Shandong'),
(763, 44, 'Shanghai'),
(764, 44, 'Shanxi'),
(765, 44, 'Sichuan'),
(766, 44, 'Tianjin'),
(767, 44, 'Xianggang'),
(768, 44, 'Xinjiang'),
(769, 44, 'Xizang'),
(770, 44, 'Yunnan'),
(771, 44, 'Zhejiang'),
(773, 45, 'Christmas Island'),
(774, 46, 'Cocos (Keeling) Islands'),
(775, 47, 'Amazonas'),
(776, 47, 'Antioquia'),
(777, 47, 'Arauca'),
(778, 47, 'Atlantico'),
(779, 47, 'Bogota'),
(780, 47, 'Bolivar'),
(781, 47, 'Boyaca'),
(782, 47, 'Caldas'),
(783, 47, 'Caqueta'),
(784, 47, 'Casanare'),
(785, 47, 'Cauca'),
(786, 47, 'Cesar'),
(787, 47, 'Choco'),
(788, 47, 'Cordoba'),
(789, 47, 'Cundinamarca'),
(790, 47, 'Guainia'),
(791, 47, 'Guaviare'),
(792, 47, 'Huila'),
(793, 47, 'La Guajira'),
(794, 47, 'Magdalena'),
(795, 47, 'Meta'),
(796, 47, 'Narino'),
(797, 47, 'Norte de Santander'),
(798, 47, 'Putumayo'),
(799, 47, 'Quindio'),
(800, 47, 'Risaralda'),
(801, 47, 'San Andres y Providencia'),
(802, 47, 'Santander'),
(803, 47, 'Sucre'),
(804, 47, 'Tolima'),
(805, 47, 'Valle del Cauca'),
(806, 47, 'Vaupes'),
(807, 47, 'Vichada'),
(808, 48, 'Mwali'),
(809, 48, 'Njazidja'),
(810, 48, 'Nzwani'),
(811, 49, 'Bouenza'),
(812, 49, 'Brazzaville'),
(813, 49, 'Cuvette'),
(814, 49, 'Kouilou'),
(815, 49, 'Lekoumou'),
(816, 49, 'Likouala'),
(817, 49, 'Niari'),
(818, 49, 'Plateaux'),
(819, 49, 'Pool'),
(820, 49, 'Sangha'),
(821, 50, 'Bandundu'),
(822, 50, 'Bas-Congo'),
(823, 50, 'Equateur'),
(824, 50, 'Haut-Congo'),
(825, 50, 'Kasai-Occidental'),
(826, 50, 'Kasai-Oriental'),
(827, 50, 'Katanga'),
(828, 50, 'Kinshasa'),
(829, 50, 'Maniema'),
(830, 50, 'Nord-Kivu'),
(831, 50, 'Sud-Kivu'),
(832, 51, 'Aitutaki'),
(833, 51, 'Atiu'),
(834, 51, 'Mangaia'),
(835, 51, 'Manihiki'),
(836, 51, 'Mauke'),
(837, 51, 'Mitiaro'),
(838, 51, 'Nassau'),
(839, 51, 'Pukapuka'),
(840, 51, 'Rakahanga'),
(841, 51, 'Rarotonga'),
(842, 51, 'Tongareva'),
(843, 52, 'Alajuela'),
(844, 52, 'Cartago'),
(845, 52, 'Guanacaste'),
(846, 52, 'Heredia'),
(847, 52, 'Limon'),
(848, 52, 'Puntarenas'),
(849, 52, 'San Jose'),
(850, 53, 'Abidjan'),
(851, 53, 'Agneby'),
(852, 53, 'Bafing'),
(853, 53, 'Denguele'),
(854, 53, 'Dix-huit Montagnes'),
(855, 53, 'Fromager'),
(856, 53, 'Haut-Sassandra'),
(857, 53, 'Lacs'),
(858, 53, 'Lagunes'),
(859, 53, 'Marahoue'),
(860, 53, 'Moyen-Cavally'),
(861, 53, 'Moyen-Comoe'),
(862, 53, 'N\'zi-Comoe'),
(863, 53, 'Sassandra'),
(864, 53, 'Savanes'),
(865, 53, 'Sud-Bandama'),
(866, 53, 'Sud-Comoe'),
(867, 53, 'Vallee du Bandama'),
(868, 53, 'Worodougou'),
(869, 53, 'Zanzan'),
(870, 54, 'Bjelovar-Bilogora'),
(871, 54, 'Dubrovnik-Neretva'),
(872, 54, 'Grad Zagreb'),
(873, 54, 'Istra'),
(874, 54, 'Karlovac'),
(875, 54, 'Koprivnica-Krizhevci'),
(876, 54, 'Krapina-Zagorje'),
(877, 54, 'Lika-Senj'),
(878, 54, 'Medhimurje'),
(879, 54, 'Medimurska Zupanija'),
(880, 54, 'Osijek-Baranja'),
(881, 54, 'Osjecko-Baranjska Zupanija'),
(882, 54, 'Pozhega-Slavonija'),
(883, 54, 'Primorje-Gorski Kotar'),
(884, 54, 'Shibenik-Knin'),
(885, 54, 'Sisak-Moslavina'),
(886, 54, 'Slavonski Brod-Posavina'),
(887, 54, 'Split-Dalmacija'),
(888, 54, 'Varazhdin'),
(889, 54, 'Virovitica-Podravina'),
(890, 54, 'Vukovar-Srijem'),
(891, 54, 'Zadar'),
(892, 54, 'Zagreb'),
(893, 55, 'Camaguey'),
(894, 55, 'Ciego de Avila'),
(895, 55, 'Cienfuegos'),
(896, 55, 'Ciudad de la Habana'),
(897, 55, 'Granma'),
(898, 55, 'Guantanamo'),
(899, 55, 'Habana'),
(900, 55, 'Holguin'),
(901, 55, 'Isla de la Juventud'),
(902, 55, 'La Habana'),
(903, 55, 'Las Tunas'),
(904, 55, 'Matanzas'),
(905, 55, 'Pinar del Rio'),
(906, 55, 'Sancti Spiritus'),
(907, 55, 'Santiago de Cuba'),
(908, 55, 'Villa Clara'),
(909, 56, 'Government controlled area'),
(910, 56, 'Limassol'),
(911, 56, 'Nicosia District'),
(912, 56, 'Paphos'),
(913, 56, 'Turkish controlled area'),
(914, 57, 'Central Bohemian'),
(915, 57, 'Frycovice'),
(916, 57, 'Jihocesky Kraj'),
(917, 57, 'Jihochesky'),
(918, 57, 'Jihomoravsky'),
(919, 57, 'Karlovarsky'),
(920, 57, 'Klecany'),
(921, 57, 'Kralovehradecky'),
(922, 57, 'Liberecky'),
(923, 57, 'Lipov'),
(924, 57, 'Moravskoslezsky'),
(925, 57, 'Olomoucky'),
(926, 57, 'Olomoucky Kraj'),
(927, 57, 'Pardubicky'),
(928, 57, 'Plzensky'),
(929, 57, 'Praha'),
(930, 57, 'Rajhrad'),
(931, 57, 'Smirice'),
(932, 57, 'South Moravian'),
(933, 57, 'Straz nad Nisou'),
(934, 57, 'Stredochesky'),
(935, 57, 'Unicov'),
(936, 57, 'Ustecky'),
(937, 57, 'Valletta'),
(938, 57, 'Velesin'),
(939, 57, 'Vysochina'),
(940, 57, 'Zlinsky'),
(941, 58, 'Arhus'),
(942, 58, 'Bornholm'),
(943, 58, 'Frederiksborg'),
(944, 58, 'Fyn'),
(945, 58, 'Hovedstaden'),
(946, 58, 'Kobenhavn'),
(947, 58, 'Kobenhavns Amt'),
(948, 58, 'Kobenhavns Kommune'),
(949, 58, 'Nordjylland'),
(950, 58, 'Ribe'),
(951, 58, 'Ringkobing'),
(952, 58, 'Roervig'),
(953, 58, 'Roskilde'),
(954, 58, 'Roslev'),
(955, 58, 'Sjaelland'),
(956, 58, 'Soeborg'),
(957, 58, 'Sonderjylland'),
(958, 58, 'Storstrom'),
(959, 58, 'Syddanmark'),
(960, 58, 'Toelloese'),
(961, 58, 'Vejle'),
(962, 58, 'Vestsjalland'),
(963, 58, 'Viborg'),
(964, 59, '\'Ali Sabih'),
(965, 59, 'Dikhil'),
(966, 59, 'Jibuti'),
(967, 59, 'Tajurah'),
(968, 59, 'Ubuk'),
(969, 60, 'Saint Andrew'),
(970, 60, 'Saint David'),
(971, 60, 'Saint George'),
(972, 60, 'Saint John'),
(973, 60, 'Saint Joseph'),
(974, 60, 'Saint Luke'),
(975, 60, 'Saint Mark'),
(976, 60, 'Saint Patrick'),
(977, 60, 'Saint Paul'),
(978, 60, 'Saint Peter'),
(979, 61, 'Azua'),
(980, 61, 'Bahoruco'),
(981, 61, 'Barahona'),
(982, 61, 'Dajabon'),
(983, 61, 'Distrito Nacional'),
(984, 61, 'Duarte'),
(985, 61, 'El Seybo'),
(986, 61, 'Elias Pina'),
(987, 61, 'Espaillat'),
(988, 61, 'Hato Mayor'),
(989, 61, 'Independencia'),
(990, 61, 'La Altagracia'),
(991, 61, 'La Romana'),
(992, 61, 'La Vega'),
(993, 61, 'Maria Trinidad Sanchez'),
(994, 61, 'Monsenor Nouel'),
(995, 61, 'Monte Cristi'),
(996, 61, 'Monte Plata'),
(997, 61, 'Pedernales'),
(998, 61, 'Peravia'),
(999, 61, 'Puerto Plata'),
(1000, 61, 'Salcedo'),
(1001, 61, 'Samana'),
(1002, 61, 'San Cristobal'),
(1003, 61, 'San Juan'),
(1004, 61, 'San Pedro de Macoris'),
(1005, 61, 'Sanchez Ramirez'),
(1006, 61, 'Santiago'),
(1007, 61, 'Santiago Rodriguez'),
(1008, 61, 'Valverde'),
(1009, 62, 'Aileu'),
(1010, 62, 'Ainaro'),
(1011, 62, 'Ambeno'),
(1012, 62, 'Baucau'),
(1013, 62, 'Bobonaro'),
(1014, 62, 'Cova Lima'),
(1015, 62, 'Dili'),
(1016, 62, 'Ermera'),
(1017, 62, 'Lautem'),
(1018, 62, 'Liquica'),
(1019, 62, 'Manatuto'),
(1020, 62, 'Manufahi'),
(1021, 62, 'Viqueque'),
(1022, 63, 'Azuay'),
(1023, 63, 'Bolivar'),
(1024, 63, 'Canar'),
(1025, 63, 'Carchi'),
(1026, 63, 'Chimborazo'),
(1027, 63, 'Cotopaxi'),
(1028, 63, 'El Oro'),
(1029, 63, 'Esmeraldas'),
(1030, 63, 'Galapagos'),
(1031, 63, 'Guayas'),
(1032, 63, 'Imbabura'),
(1033, 63, 'Loja'),
(1034, 63, 'Los Rios'),
(1035, 63, 'Manabi'),
(1036, 63, 'Morona Santiago'),
(1037, 63, 'Napo'),
(1038, 63, 'Orellana'),
(1039, 63, 'Pastaza'),
(1040, 63, 'Pichincha'),
(1041, 63, 'Sucumbios'),
(1042, 63, 'Tungurahua'),
(1043, 63, 'Zamora Chinchipe'),
(1044, 64, 'Aswan'),
(1045, 64, 'Asyut'),
(1046, 64, 'Bani Suwayf'),
(1047, 64, 'Bur Sa\'id'),
(1048, 64, 'Cairo'),
(1049, 64, 'Dumyat'),
(1050, 64, 'Kafr-ash-Shaykh'),
(1051, 64, 'Matruh'),
(1052, 64, 'Muhafazat ad Daqahliyah'),
(1053, 64, 'Muhafazat al Fayyum'),
(1054, 64, 'Muhafazat al Gharbiyah'),
(1055, 64, 'Muhafazat al Iskandariyah'),
(1056, 64, 'Muhafazat al Qahirah'),
(1057, 64, 'Qina'),
(1058, 64, 'Sawhaj'),
(1059, 64, 'Sina al-Janubiyah'),
(1060, 64, 'Sina ash-Shamaliyah'),
(1061, 64, 'ad-Daqahliyah'),
(1062, 64, 'al-Bahr-al-Ahmar'),
(1063, 64, 'al-Buhayrah'),
(1064, 64, 'al-Fayyum'),
(1065, 64, 'al-Gharbiyah'),
(1066, 64, 'al-Iskandariyah'),
(1067, 64, 'al-Ismailiyah'),
(1068, 64, 'al-Jizah'),
(1069, 64, 'al-Minufiyah'),
(1070, 64, 'al-Minya'),
(1071, 64, 'al-Qahira'),
(1072, 64, 'al-Qalyubiyah'),
(1073, 64, 'al-Uqsur'),
(1074, 64, 'al-Wadi al-Jadid'),
(1075, 64, 'as-Suways'),
(1076, 64, 'ash-Sharqiyah'),
(1077, 65, 'Ahuachapan'),
(1078, 65, 'Cabanas'),
(1079, 65, 'Chalatenango'),
(1080, 65, 'Cuscatlan'),
(1081, 65, 'La Libertad'),
(1082, 65, 'La Paz'),
(1083, 65, 'La Union'),
(1084, 65, 'Morazan'),
(1085, 65, 'San Miguel'),
(1086, 65, 'San Salvador'),
(1087, 65, 'San Vicente'),
(1088, 65, 'Santa Ana'),
(1089, 65, 'Sonsonate'),
(1090, 65, 'Usulutan'),
(1091, 66, 'Annobon'),
(1092, 66, 'Bioko Norte'),
(1093, 66, 'Bioko Sur'),
(1094, 66, 'Centro Sur'),
(1095, 66, 'Kie-Ntem'),
(1096, 66, 'Litoral'),
(1097, 66, 'Wele-Nzas'),
(1098, 67, 'Anseba'),
(1099, 67, 'Debub'),
(1100, 67, 'Debub-Keih-Bahri'),
(1101, 67, 'Gash-Barka'),
(1102, 67, 'Maekel'),
(1103, 67, 'Semien-Keih-Bahri'),
(1104, 68, 'Harju'),
(1105, 68, 'Hiiu'),
(1106, 68, 'Ida-Viru'),
(1107, 68, 'Jarva'),
(1108, 68, 'Jogeva'),
(1109, 68, 'Laane'),
(1110, 68, 'Laane-Viru'),
(1111, 68, 'Parnu'),
(1112, 68, 'Polva'),
(1113, 68, 'Rapla'),
(1114, 68, 'Saare'),
(1115, 68, 'Tartu'),
(1116, 68, 'Valga'),
(1117, 68, 'Viljandi'),
(1118, 68, 'Voru'),
(1119, 69, 'Addis Abeba'),
(1120, 69, 'Afar'),
(1121, 69, 'Amhara'),
(1122, 69, 'Benishangul'),
(1123, 69, 'Diredawa'),
(1124, 69, 'Gambella'),
(1125, 69, 'Harar'),
(1126, 69, 'Jigjiga'),
(1127, 69, 'Mekele'),
(1128, 69, 'Oromia'),
(1129, 69, 'Somali'),
(1130, 69, 'Southern'),
(1131, 69, 'Tigray'),
(1132, 70, 'Christmas Island'),
(1133, 70, 'Cocos Islands'),
(1134, 70, 'Coral Sea Islands'),
(1135, 71, 'Falkland Islands'),
(1136, 71, 'South Georgia'),
(1137, 72, 'Klaksvik'),
(1138, 72, 'Nor ara Eysturoy'),
(1139, 72, 'Nor oy'),
(1140, 72, 'Sandoy'),
(1141, 72, 'Streymoy'),
(1142, 72, 'Su uroy'),
(1143, 72, 'Sy ra Eysturoy'),
(1144, 72, 'Torshavn'),
(1145, 72, 'Vaga'),
(1146, 73, 'Central'),
(1147, 73, 'Eastern'),
(1148, 73, 'Northern'),
(1149, 73, 'South Pacific'),
(1150, 73, 'Western'),
(1151, 74, 'Ahvenanmaa'),
(1152, 74, 'Etela-Karjala'),
(1153, 74, 'Etela-Pohjanmaa'),
(1154, 74, 'Etela-Savo'),
(1155, 74, 'Etela-Suomen Laani'),
(1156, 74, 'Ita-Suomen Laani'),
(1157, 74, 'Ita-Uusimaa'),
(1158, 74, 'Kainuu'),
(1159, 74, 'Kanta-Hame'),
(1160, 74, 'Keski-Pohjanmaa'),
(1161, 74, 'Keski-Suomi'),
(1162, 74, 'Kymenlaakso'),
(1163, 74, 'Lansi-Suomen Laani'),
(1164, 74, 'Lappi'),
(1165, 74, 'Northern Savonia'),
(1166, 74, 'Ostrobothnia'),
(1167, 74, 'Oulun Laani'),
(1168, 74, 'Paijat-Hame'),
(1169, 74, 'Pirkanmaa'),
(1170, 74, 'Pohjanmaa'),
(1171, 74, 'Pohjois-Karjala'),
(1172, 74, 'Pohjois-Pohjanmaa'),
(1173, 74, 'Pohjois-Savo'),
(1174, 74, 'Saarijarvi'),
(1175, 74, 'Satakunta'),
(1176, 74, 'Southern Savonia'),
(1177, 74, 'Tavastia Proper'),
(1178, 74, 'Uleaborgs Lan'),
(1179, 74, 'Uusimaa'),
(1180, 74, 'Varsinais-Suomi'),
(1181, 75, 'Ain'),
(1182, 75, 'Aisne'),
(1183, 75, 'Albi Le Sequestre'),
(1184, 75, 'Allier'),
(1185, 75, 'Alpes-Cote dAzur'),
(1186, 75, 'Alpes-Maritimes'),
(1187, 75, 'Alpes-de-Haute-Provence'),
(1188, 75, 'Alsace'),
(1189, 75, 'Aquitaine'),
(1190, 75, 'Ardeche'),
(1191, 75, 'Ardennes'),
(1192, 75, 'Ariege'),
(1193, 75, 'Aube'),
(1194, 75, 'Aude'),
(1195, 75, 'Auvergne'),
(1196, 75, 'Aveyron'),
(1197, 75, 'Bas-Rhin'),
(1198, 75, 'Basse-Normandie'),
(1199, 75, 'Bouches-du-Rhone'),
(1200, 75, 'Bourgogne'),
(1201, 75, 'Bretagne'),
(1202, 75, 'Brittany'),
(1203, 75, 'Burgundy'),
(1204, 75, 'Calvados'),
(1205, 75, 'Cantal'),
(1206, 75, 'Cedex'),
(1207, 75, 'Centre'),
(1208, 75, 'Charente'),
(1209, 75, 'Charente-Maritime'),
(1210, 75, 'Cher'),
(1211, 75, 'Correze'),
(1212, 75, 'Corse-du-Sud'),
(1213, 75, 'Cote-d\'Or'),
(1214, 75, 'Cotes-d\'Armor'),
(1215, 75, 'Creuse'),
(1216, 75, 'Crolles'),
(1217, 75, 'Deux-Sevres'),
(1218, 75, 'Dordogne'),
(1219, 75, 'Doubs'),
(1220, 75, 'Drome'),
(1221, 75, 'Essonne'),
(1222, 75, 'Eure'),
(1223, 75, 'Eure-et-Loir'),
(1224, 75, 'Feucherolles'),
(1225, 75, 'Finistere'),
(1226, 75, 'Franche-Comte'),
(1227, 75, 'Gard'),
(1228, 75, 'Gers'),
(1229, 75, 'Gironde'),
(1230, 75, 'Haut-Rhin'),
(1231, 75, 'Haute-Corse'),
(1232, 75, 'Haute-Garonne'),
(1233, 75, 'Haute-Loire'),
(1234, 75, 'Haute-Marne'),
(1235, 75, 'Haute-Saone'),
(1236, 75, 'Haute-Savoie'),
(1237, 75, 'Haute-Vienne'),
(1238, 75, 'Hautes-Alpes'),
(1239, 75, 'Hautes-Pyrenees'),
(1240, 75, 'Hauts-de-Seine'),
(1241, 75, 'Herault'),
(1242, 75, 'Ile-de-France'),
(1243, 75, 'Ille-et-Vilaine'),
(1244, 75, 'Indre'),
(1245, 75, 'Indre-et-Loire'),
(1246, 75, 'Isere'),
(1247, 75, 'Jura'),
(1248, 75, 'Klagenfurt'),
(1249, 75, 'Landes'),
(1250, 75, 'Languedoc-Roussillon'),
(1251, 75, 'Larcay'),
(1252, 75, 'Le Castellet'),
(1253, 75, 'Le Creusot'),
(1254, 75, 'Limousin'),
(1255, 75, 'Loir-et-Cher'),
(1256, 75, 'Loire'),
(1257, 75, 'Loire-Atlantique'),
(1258, 75, 'Loiret'),
(1259, 75, 'Lorraine'),
(1260, 75, 'Lot'),
(1261, 75, 'Lot-et-Garonne'),
(1262, 75, 'Lower Normandy'),
(1263, 75, 'Lozere'),
(1264, 75, 'Maine-et-Loire'),
(1265, 75, 'Manche'),
(1266, 75, 'Marne'),
(1267, 75, 'Mayenne'),
(1268, 75, 'Meurthe-et-Moselle'),
(1269, 75, 'Meuse'),
(1270, 75, 'Midi-Pyrenees'),
(1271, 75, 'Morbihan'),
(1272, 75, 'Moselle'),
(1273, 75, 'Nievre'),
(1274, 75, 'Nord'),
(1275, 75, 'Nord-Pas-de-Calais'),
(1276, 75, 'Oise'),
(1277, 75, 'Orne'),
(1278, 75, 'Paris'),
(1279, 75, 'Pas-de-Calais'),
(1280, 75, 'Pays de la Loire'),
(1281, 75, 'Pays-de-la-Loire'),
(1282, 75, 'Picardy'),
(1283, 75, 'Puy-de-Dome'),
(1284, 75, 'Pyrenees-Atlantiques'),
(1285, 75, 'Pyrenees-Orientales'),
(1286, 75, 'Quelmes'),
(1287, 75, 'Rhone'),
(1288, 75, 'Rhone-Alpes'),
(1289, 75, 'Saint Ouen'),
(1290, 75, 'Saint Viatre'),
(1291, 75, 'Saone-et-Loire'),
(1292, 75, 'Sarthe'),
(1293, 75, 'Savoie'),
(1294, 75, 'Seine-Maritime'),
(1295, 75, 'Seine-Saint-Denis'),
(1296, 75, 'Seine-et-Marne'),
(1297, 75, 'Somme'),
(1298, 75, 'Sophia Antipolis'),
(1299, 75, 'Souvans'),
(1300, 75, 'Tarn'),
(1301, 75, 'Tarn-et-Garonne'),
(1302, 75, 'Territoire de Belfort'),
(1303, 75, 'Treignac'),
(1304, 75, 'Upper Normandy'),
(1305, 75, 'Val-d\'Oise'),
(1306, 75, 'Val-de-Marne'),
(1307, 75, 'Var'),
(1308, 75, 'Vaucluse'),
(1309, 75, 'Vellise'),
(1310, 75, 'Vendee'),
(1311, 75, 'Vienne'),
(1312, 75, 'Vosges'),
(1313, 75, 'Yonne'),
(1314, 75, 'Yvelines'),
(1315, 76, 'Cayenne'),
(1316, 76, 'Saint-Laurent-du-Maroni'),
(1317, 77, 'Iles du Vent'),
(1318, 77, 'Iles sous le Vent'),
(1319, 77, 'Marquesas'),
(1320, 77, 'Tuamotu'),
(1321, 77, 'Tubuai'),
(1322, 78, 'Amsterdam'),
(1323, 78, 'Crozet Islands'),
(1324, 78, 'Kerguelen'),
(1325, 79, 'Estuaire'),
(1326, 79, 'Haut-Ogooue'),
(1327, 79, 'Moyen-Ogooue'),
(1328, 79, 'Ngounie'),
(1329, 79, 'Nyanga'),
(1330, 79, 'Ogooue-Ivindo'),
(1331, 79, 'Ogooue-Lolo'),
(1332, 79, 'Ogooue-Maritime'),
(1333, 79, 'Woleu-Ntem'),
(1334, 80, 'Banjul'),
(1335, 80, 'Basse'),
(1336, 80, 'Brikama'),
(1337, 80, 'Janjanbureh'),
(1338, 80, 'Kanifing'),
(1339, 80, 'Kerewan'),
(1340, 80, 'Kuntaur'),
(1341, 80, 'Mansakonko'),
(1342, 81, 'Abhasia'),
(1343, 81, 'Ajaria'),
(1344, 81, 'Guria'),
(1345, 81, 'Imereti'),
(1346, 81, 'Kaheti'),
(1347, 81, 'Kvemo Kartli'),
(1348, 81, 'Mcheta-Mtianeti'),
(1349, 81, 'Racha'),
(1350, 81, 'Samagrelo-Zemo Svaneti'),
(1351, 81, 'Samche-Zhavaheti'),
(1352, 81, 'Shida Kartli'),
(1353, 81, 'Tbilisi'),
(1354, 82, 'Auvergne'),
(1355, 82, 'Baden-Wurttemberg'),
(1356, 82, 'Bavaria'),
(1357, 82, 'Bayern'),
(1358, 82, 'Beilstein Wurtt'),
(1359, 82, 'Berlin'),
(1360, 82, 'Brandenburg'),
(1361, 82, 'Bremen'),
(1362, 82, 'Dreisbach'),
(1363, 82, 'Freistaat Bayern'),
(1364, 82, 'Hamburg'),
(1365, 82, 'Hannover'),
(1366, 82, 'Heroldstatt'),
(1367, 82, 'Hessen'),
(1368, 82, 'Kortenberg'),
(1369, 82, 'Laasdorf'),
(1370, 82, 'Land Baden-Wurttemberg'),
(1371, 82, 'Land Bayern'),
(1372, 82, 'Land Brandenburg'),
(1373, 82, 'Land Hessen'),
(1374, 82, 'Land Mecklenburg-Vorpommern'),
(1375, 82, 'Land Nordrhein-Westfalen'),
(1376, 82, 'Land Rheinland-Pfalz'),
(1377, 82, 'Land Sachsen'),
(1378, 82, 'Land Sachsen-Anhalt'),
(1379, 82, 'Land Thuringen'),
(1380, 82, 'Lower Saxony'),
(1381, 82, 'Mecklenburg-Vorpommern'),
(1382, 82, 'Mulfingen'),
(1383, 82, 'Munich'),
(1384, 82, 'Neubeuern'),
(1385, 82, 'Niedersachsen'),
(1386, 82, 'Noord-Holland'),
(1387, 82, 'Nordrhein-Westfalen'),
(1388, 82, 'North Rhine-Westphalia'),
(1389, 82, 'Osterode'),
(1390, 82, 'Rheinland-Pfalz'),
(1391, 82, 'Rhineland-Palatinate'),
(1392, 82, 'Saarland'),
(1393, 82, 'Sachsen'),
(1394, 82, 'Sachsen-Anhalt'),
(1395, 82, 'Saxony'),
(1396, 82, 'Schleswig-Holstein'),
(1397, 82, 'Thuringia'),
(1398, 82, 'Webling'),
(1399, 82, 'Weinstrabe'),
(1400, 82, 'schlobborn'),
(1401, 83, 'Ashanti'),
(1402, 83, 'Brong-Ahafo'),
(1403, 83, 'Central'),
(1404, 83, 'Eastern'),
(1405, 83, 'Greater Accra'),
(1406, 83, 'Northern'),
(1407, 83, 'Upper East'),
(1408, 83, 'Upper West'),
(1409, 83, 'Volta'),
(1410, 83, 'Western'),
(1411, 84, 'Gibraltar'),
(1412, 85, 'Acharnes'),
(1413, 85, 'Ahaia'),
(1414, 85, 'Aitolia kai Akarnania'),
(1415, 85, 'Argolis'),
(1416, 85, 'Arkadia'),
(1417, 85, 'Arta'),
(1418, 85, 'Attica'),
(1419, 85, 'Attiki'),
(1420, 85, 'Ayion Oros'),
(1421, 85, 'Crete'),
(1422, 85, 'Dodekanisos'),
(1423, 85, 'Drama'),
(1424, 85, 'Evia'),
(1425, 85, 'Evritania'),
(1426, 85, 'Evros'),
(1427, 85, 'Evvoia'),
(1428, 85, 'Florina'),
(1429, 85, 'Fokis'),
(1430, 85, 'Fthiotis'),
(1431, 85, 'Grevena'),
(1432, 85, 'Halandri'),
(1433, 85, 'Halkidiki'),
(1434, 85, 'Hania'),
(1435, 85, 'Heraklion'),
(1436, 85, 'Hios'),
(1437, 85, 'Ilia'),
(1438, 85, 'Imathia'),
(1439, 85, 'Ioannina'),
(1440, 85, 'Iraklion'),
(1441, 85, 'Karditsa'),
(1442, 85, 'Kastoria'),
(1443, 85, 'Kavala'),
(1444, 85, 'Kefallinia'),
(1445, 85, 'Kerkira'),
(1446, 85, 'Kiklades'),
(1447, 85, 'Kilkis'),
(1448, 85, 'Korinthia'),
(1449, 85, 'Kozani'),
(1450, 85, 'Lakonia'),
(1451, 85, 'Larisa'),
(1452, 85, 'Lasithi'),
(1453, 85, 'Lesvos'),
(1454, 85, 'Levkas'),
(1455, 85, 'Magnisia'),
(1456, 85, 'Messinia'),
(1457, 85, 'Nomos Attikis'),
(1458, 85, 'Nomos Zakynthou'),
(1459, 85, 'Pella'),
(1460, 85, 'Pieria'),
(1461, 85, 'Piraios'),
(1462, 85, 'Preveza'),
(1463, 85, 'Rethimni'),
(1464, 85, 'Rodopi'),
(1465, 85, 'Samos'),
(1466, 85, 'Serrai'),
(1467, 85, 'Thesprotia'),
(1468, 85, 'Thessaloniki'),
(1469, 85, 'Trikala'),
(1470, 85, 'Voiotia'),
(1471, 85, 'West Greece'),
(1472, 85, 'Xanthi'),
(1473, 85, 'Zakinthos'),
(1474, 86, 'Aasiaat'),
(1475, 86, 'Ammassalik'),
(1476, 86, 'Illoqqortoormiut'),
(1477, 86, 'Ilulissat'),
(1478, 86, 'Ivittuut'),
(1479, 86, 'Kangaatsiaq'),
(1480, 86, 'Maniitsoq'),
(1481, 86, 'Nanortalik'),
(1482, 86, 'Narsaq'),
(1483, 86, 'Nuuk'),
(1484, 86, 'Paamiut'),
(1485, 86, 'Qaanaaq'),
(1486, 86, 'Qaqortoq'),
(1487, 86, 'Qasigiannguit'),
(1488, 86, 'Qeqertarsuaq'),
(1489, 86, 'Sisimiut'),
(1490, 86, 'Udenfor kommunal inddeling'),
(1491, 86, 'Upernavik'),
(1492, 86, 'Uummannaq'),
(1493, 87, 'Carriacou-Petite Martinique'),
(1494, 87, 'Saint Andrew'),
(1495, 87, 'Saint Davids'),
(1496, 87, 'Saint George\'s'),
(1497, 87, 'Saint John'),
(1498, 87, 'Saint Mark'),
(1499, 87, 'Saint Patrick'),
(1500, 88, 'Basse-Terre'),
(1501, 88, 'Grande-Terre'),
(1502, 88, 'Iles des Saintes'),
(1503, 88, 'La Desirade'),
(1504, 88, 'Marie-Galante'),
(1505, 88, 'Saint Barthelemy'),
(1506, 88, 'Saint Martin'),
(1507, 89, 'Agana Heights'),
(1508, 89, 'Agat'),
(1509, 89, 'Barrigada'),
(1510, 89, 'Chalan-Pago-Ordot'),
(1511, 89, 'Dededo'),
(1512, 89, 'Hagatna'),
(1513, 89, 'Inarajan'),
(1514, 89, 'Mangilao'),
(1515, 89, 'Merizo'),
(1516, 89, 'Mongmong-Toto-Maite'),
(1517, 89, 'Santa Rita'),
(1518, 89, 'Sinajana'),
(1519, 89, 'Talofofo'),
(1520, 89, 'Tamuning'),
(1521, 89, 'Yigo'),
(1522, 89, 'Yona'),
(1523, 90, 'Alta Verapaz'),
(1524, 90, 'Baja Verapaz'),
(1525, 90, 'Chimaltenango'),
(1526, 90, 'Chiquimula'),
(1527, 90, 'El Progreso'),
(1528, 90, 'Escuintla'),
(1529, 90, 'Guatemala'),
(1530, 90, 'Huehuetenango'),
(1531, 90, 'Izabal'),
(1532, 90, 'Jalapa'),
(1533, 90, 'Jutiapa'),
(1534, 90, 'Peten'),
(1535, 90, 'Quezaltenango'),
(1536, 90, 'Quiche'),
(1537, 90, 'Retalhuleu'),
(1538, 90, 'Sacatepequez'),
(1539, 90, 'San Marcos'),
(1540, 90, 'Santa Rosa'),
(1541, 90, 'Solola'),
(1542, 90, 'Suchitepequez'),
(1543, 90, 'Totonicapan'),
(1544, 90, 'Zacapa'),
(1545, 91, 'Alderney'),
(1546, 91, 'Castel'),
(1547, 91, 'Forest'),
(1548, 91, 'Saint Andrew'),
(1549, 91, 'Saint Martin'),
(1550, 91, 'Saint Peter Port'),
(1551, 91, 'Saint Pierre du Bois'),
(1552, 91, 'Saint Sampson'),
(1553, 91, 'Saint Saviour'),
(1554, 91, 'Sark'),
(1555, 91, 'Torteval'),
(1556, 91, 'Vale'),
(1557, 92, 'Beyla'),
(1558, 92, 'Boffa'),
(1559, 92, 'Boke'),
(1560, 92, 'Conakry'),
(1561, 92, 'Coyah'),
(1562, 92, 'Dabola'),
(1563, 92, 'Dalaba'),
(1564, 92, 'Dinguiraye'),
(1565, 92, 'Faranah'),
(1566, 92, 'Forecariah'),
(1567, 92, 'Fria'),
(1568, 92, 'Gaoual'),
(1569, 92, 'Gueckedou'),
(1570, 92, 'Kankan'),
(1571, 92, 'Kerouane'),
(1572, 92, 'Kindia'),
(1573, 92, 'Kissidougou'),
(1574, 92, 'Koubia'),
(1575, 92, 'Koundara'),
(1576, 92, 'Kouroussa'),
(1577, 92, 'Labe'),
(1578, 92, 'Lola'),
(1579, 92, 'Macenta'),
(1580, 92, 'Mali'),
(1581, 92, 'Mamou'),
(1582, 92, 'Mandiana'),
(1583, 92, 'Nzerekore'),
(1584, 92, 'Pita'),
(1585, 92, 'Siguiri'),
(1586, 92, 'Telimele'),
(1587, 92, 'Tougue'),
(1588, 92, 'Yomou'),
(1589, 93, 'Bafata'),
(1590, 93, 'Bissau'),
(1591, 93, 'Bolama'),
(1592, 93, 'Cacheu'),
(1593, 93, 'Gabu'),
(1594, 93, 'Oio'),
(1595, 93, 'Quinara'),
(1596, 93, 'Tombali'),
(1597, 94, 'Barima-Waini'),
(1598, 94, 'Cuyuni-Mazaruni'),
(1599, 94, 'Demerara-Mahaica'),
(1600, 94, 'East Berbice-Corentyne'),
(1601, 94, 'Essequibo Islands-West Demerar'),
(1602, 94, 'Mahaica-Berbice'),
(1603, 94, 'Pomeroon-Supenaam'),
(1604, 94, 'Potaro-Siparuni'),
(1605, 94, 'Upper Demerara-Berbice'),
(1606, 94, 'Upper Takutu-Upper Essequibo'),
(1607, 95, 'Artibonite'),
(1608, 95, 'Centre'),
(1609, 95, 'Grand\'Anse'),
(1610, 95, 'Nord'),
(1611, 95, 'Nord-Est'),
(1612, 95, 'Nord-Ouest'),
(1613, 95, 'Ouest'),
(1614, 95, 'Sud'),
(1615, 95, 'Sud-Est'),
(1616, 96, 'Heard and McDonald Islands'),
(1617, 97, 'Atlantida'),
(1618, 97, 'Choluteca'),
(1619, 97, 'Colon'),
(1620, 97, 'Comayagua'),
(1621, 97, 'Copan'),
(1622, 97, 'Cortes'),
(1623, 97, 'Distrito Central'),
(1624, 97, 'El Paraiso'),
(1625, 97, 'Francisco Morazan'),
(1626, 97, 'Gracias a Dios'),
(1627, 97, 'Intibuca'),
(1628, 97, 'Islas de la Bahia'),
(1629, 97, 'La Paz'),
(1630, 97, 'Lempira'),
(1631, 97, 'Ocotepeque'),
(1632, 97, 'Olancho'),
(1633, 97, 'Santa Barbara'),
(1634, 97, 'Valle'),
(1635, 97, 'Yoro'),
(1636, 98, 'Hong Kong'),
(1637, 99, 'Bacs-Kiskun'),
(1638, 99, 'Baranya'),
(1639, 99, 'Bekes'),
(1640, 99, 'Borsod-Abauj-Zemplen'),
(1641, 99, 'Budapest'),
(1642, 99, 'Csongrad'),
(1643, 99, 'Fejer'),
(1644, 99, 'Gyor-Moson-Sopron'),
(1645, 99, 'Hajdu-Bihar'),
(1646, 99, 'Heves'),
(1647, 99, 'Jasz-Nagykun-Szolnok'),
(1648, 99, 'Komarom-Esztergom'),
(1649, 99, 'Nograd'),
(1650, 99, 'Pest'),
(1651, 99, 'Somogy'),
(1652, 99, 'Szabolcs-Szatmar-Bereg'),
(1653, 99, 'Tolna'),
(1654, 99, 'Vas'),
(1655, 99, 'Veszprem'),
(1656, 99, 'Zala'),
(1657, 100, 'Austurland'),
(1658, 100, 'Gullbringusysla'),
(1659, 100, 'Hofu borgarsva i'),
(1660, 100, 'Nor urland eystra'),
(1661, 100, 'Nor urland vestra'),
(1662, 100, 'Su urland'),
(1663, 100, 'Su urnes'),
(1664, 100, 'Vestfir ir'),
(1665, 100, 'Vesturland'),
(1666, 102, 'Aceh'),
(1667, 102, 'Bali'),
(1668, 102, 'Bangka-Belitung'),
(1669, 102, 'Banten'),
(1670, 102, 'Bengkulu'),
(1671, 102, 'Gandaria'),
(1672, 102, 'Gorontalo'),
(1673, 102, 'Jakarta'),
(1674, 102, 'Jambi'),
(1675, 102, 'Jawa Barat'),
(1676, 102, 'Jawa Tengah'),
(1677, 102, 'Jawa Timur'),
(1678, 102, 'Kalimantan Barat'),
(1679, 102, 'Kalimantan Selatan'),
(1680, 102, 'Kalimantan Tengah'),
(1681, 102, 'Kalimantan Timur'),
(1682, 102, 'Kendal'),
(1683, 102, 'Lampung'),
(1684, 102, 'Maluku'),
(1685, 102, 'Maluku Utara'),
(1686, 102, 'Nusa Tenggara Barat'),
(1687, 102, 'Nusa Tenggara Timur'),
(1688, 102, 'Papua'),
(1689, 102, 'Riau'),
(1690, 102, 'Riau Kepulauan'),
(1691, 102, 'Solo'),
(1692, 102, 'Sulawesi Selatan'),
(1693, 102, 'Sulawesi Tengah'),
(1694, 102, 'Sulawesi Tenggara'),
(1695, 102, 'Sulawesi Utara'),
(1696, 102, 'Sumatera Barat'),
(1697, 102, 'Sumatera Selatan'),
(1698, 102, 'Sumatera Utara'),
(1699, 102, 'Yogyakarta'),
(1700, 103, 'Ardabil'),
(1701, 103, 'Azarbayjan-e Bakhtari'),
(1702, 103, 'Azarbayjan-e Khavari'),
(1703, 103, 'Bushehr'),
(1704, 103, 'Chahar Mahal-e Bakhtiari'),
(1705, 103, 'Esfahan'),
(1706, 103, 'Fars'),
(1707, 103, 'Gilan'),
(1708, 103, 'Golestan'),
(1709, 103, 'Hamadan'),
(1710, 103, 'Hormozgan'),
(1711, 103, 'Ilam'),
(1712, 103, 'Kerman'),
(1713, 103, 'Kermanshah'),
(1714, 103, 'Khorasan'),
(1715, 103, 'Khuzestan'),
(1716, 103, 'Kohgiluyeh-e Boyerahmad'),
(1717, 103, 'Kordestan'),
(1718, 103, 'Lorestan'),
(1719, 103, 'Markazi'),
(1720, 103, 'Mazandaran'),
(1721, 103, 'Ostan-e Esfahan'),
(1722, 103, 'Qazvin'),
(1723, 103, 'Qom'),
(1724, 103, 'Semnan'),
(1725, 103, 'Sistan-e Baluchestan'),
(1726, 103, 'Tehran'),
(1727, 103, 'Yazd'),
(1728, 103, 'Zanjan'),
(1729, 104, 'Babil'),
(1730, 104, 'Baghdad'),
(1731, 104, 'Dahuk'),
(1732, 104, 'Dhi Qar'),
(1733, 104, 'Diyala'),
(1734, 104, 'Erbil'),
(1735, 104, 'Irbil'),
(1736, 104, 'Karbala'),
(1737, 104, 'Kurdistan'),
(1738, 104, 'Maysan'),
(1739, 104, 'Ninawa'),
(1740, 104, 'Salah-ad-Din'),
(1741, 104, 'Wasit'),
(1742, 104, 'al-Anbar'),
(1743, 104, 'al-Basrah'),
(1744, 104, 'al-Muthanna'),
(1745, 104, 'al-Qadisiyah'),
(1746, 104, 'an-Najaf'),
(1747, 104, 'as-Sulaymaniyah'),
(1748, 104, 'at-Ta\'mim'),
(1749, 105, 'Armagh'),
(1750, 105, 'Carlow'),
(1751, 105, 'Cavan'),
(1752, 105, 'Clare'),
(1753, 105, 'Cork'),
(1754, 105, 'Donegal'),
(1755, 105, 'Dublin'),
(1756, 105, 'Galway'),
(1757, 105, 'Kerry'),
(1758, 105, 'Kildare'),
(1759, 105, 'Kilkenny'),
(1760, 105, 'Laois'),
(1761, 105, 'Leinster'),
(1762, 105, 'Leitrim'),
(1763, 105, 'Limerick'),
(1764, 105, 'Loch Garman'),
(1765, 105, 'Longford'),
(1766, 105, 'Louth'),
(1767, 105, 'Mayo'),
(1768, 105, 'Meath'),
(1769, 105, 'Monaghan'),
(1770, 105, 'Offaly'),
(1771, 105, 'Roscommon'),
(1772, 105, 'Sligo'),
(1773, 105, 'Tipperary North Riding'),
(1774, 105, 'Tipperary South Riding'),
(1775, 105, 'Ulster'),
(1776, 105, 'Waterford'),
(1777, 105, 'Westmeath'),
(1778, 105, 'Wexford'),
(1779, 105, 'Wicklow'),
(1780, 106, 'Beit Hanania'),
(1781, 106, 'Ben Gurion Airport'),
(1782, 106, 'Bethlehem'),
(1783, 106, 'Caesarea'),
(1784, 106, 'Centre'),
(1785, 106, 'Gaza'),
(1786, 106, 'Hadaron'),
(1787, 106, 'Haifa District'),
(1788, 106, 'Hamerkaz'),
(1789, 106, 'Hazafon'),
(1790, 106, 'Hebron'),
(1791, 106, 'Jaffa'),
(1792, 106, 'Jerusalem'),
(1793, 106, 'Khefa'),
(1794, 106, 'Kiryat Yam'),
(1795, 106, 'Lower Galilee'),
(1796, 106, 'Qalqilya'),
(1797, 106, 'Talme Elazar'),
(1798, 106, 'Tel Aviv'),
(1799, 106, 'Tsafon'),
(1800, 106, 'Umm El Fahem'),
(1801, 106, 'Yerushalayim'),
(1802, 107, 'Abruzzi'),
(1803, 107, 'Abruzzo'),
(1804, 107, 'Agrigento'),
(1805, 107, 'Alessandria'),
(1806, 107, 'Ancona'),
(1807, 107, 'Arezzo'),
(1808, 107, 'Ascoli Piceno'),
(1809, 107, 'Asti'),
(1810, 107, 'Avellino'),
(1811, 107, 'Bari'),
(1812, 107, 'Basilicata'),
(1813, 107, 'Belluno'),
(1814, 107, 'Benevento'),
(1815, 107, 'Bergamo'),
(1816, 107, 'Biella'),
(1817, 107, 'Bologna'),
(1818, 107, 'Bolzano'),
(1819, 107, 'Brescia'),
(1820, 107, 'Brindisi'),
(1821, 107, 'Calabria'),
(1822, 107, 'Campania'),
(1823, 107, 'Cartoceto'),
(1824, 107, 'Caserta'),
(1825, 107, 'Catania'),
(1826, 107, 'Chieti'),
(1827, 107, 'Como'),
(1828, 107, 'Cosenza'),
(1829, 107, 'Cremona'),
(1830, 107, 'Cuneo'),
(1831, 107, 'Emilia-Romagna'),
(1832, 107, 'Ferrara'),
(1833, 107, 'Firenze'),
(1834, 107, 'Florence'),
(1835, 107, 'Forli-Cesena '),
(1836, 107, 'Friuli-Venezia Giulia'),
(1837, 107, 'Frosinone'),
(1838, 107, 'Genoa'),
(1839, 107, 'Gorizia'),
(1840, 107, 'L\'Aquila'),
(1841, 107, 'Lazio'),
(1842, 107, 'Lecce'),
(1843, 107, 'Lecco'),
(1845, 107, 'Liguria'),
(1846, 107, 'Lodi'),
(1847, 107, 'Lombardia'),
(1848, 107, 'Lombardy'),
(1849, 107, 'Macerata'),
(1850, 107, 'Mantova'),
(1851, 107, 'Marche'),
(1852, 107, 'Messina'),
(1853, 107, 'Milan'),
(1854, 107, 'Modena'),
(1855, 107, 'Molise'),
(1856, 107, 'Molteno'),
(1857, 107, 'Montenegro'),
(1858, 107, 'Monza and Brianza'),
(1859, 107, 'Naples'),
(1860, 107, 'Novara'),
(1861, 107, 'Padova'),
(1862, 107, 'Parma'),
(1863, 107, 'Pavia'),
(1864, 107, 'Perugia'),
(1865, 107, 'Pesaro-Urbino'),
(1866, 107, 'Piacenza'),
(1867, 107, 'Piedmont'),
(1868, 107, 'Piemonte'),
(1869, 107, 'Pisa'),
(1870, 107, 'Pordenone'),
(1871, 107, 'Potenza'),
(1872, 107, 'Puglia'),
(1873, 107, 'Reggio Emilia'),
(1874, 107, 'Rimini'),
(1875, 107, 'Roma'),
(1876, 107, 'Salerno'),
(1877, 107, 'Sardegna'),
(1878, 107, 'Sassari'),
(1879, 107, 'Savona'),
(1880, 107, 'Sicilia'),
(1881, 107, 'Siena'),
(1882, 107, 'Sondrio'),
(1883, 107, 'South Tyrol'),
(1884, 107, 'Taranto'),
(1885, 107, 'Teramo'),
(1886, 107, 'Torino'),
(1887, 107, 'Toscana'),
(1888, 107, 'Trapani'),
(1889, 107, 'Trentino-Alto Adige'),
(1890, 107, 'Trento'),
(1891, 107, 'Treviso'),
(1892, 107, 'Udine'),
(1893, 107, 'Umbria'),
(1894, 107, 'Valle d\'Aosta'),
(1895, 107, 'Varese'),
(1896, 107, 'Veneto'),
(1897, 107, 'Venezia'),
(1898, 107, 'Verbano-Cusio-Ossola'),
(1899, 107, 'Vercelli'),
(1900, 107, 'Verona'),
(1901, 107, 'Vicenza'),
(1902, 107, 'Viterbo'),
(1903, 108, 'Buxoro Viloyati'),
(1904, 108, 'Clarendon'),
(1905, 108, 'Hanover'),
(1906, 108, 'Kingston'),
(1907, 108, 'Manchester'),
(1908, 108, 'Portland'),
(1909, 108, 'Saint Andrews'),
(1910, 108, 'Saint Ann'),
(1911, 108, 'Saint Catherine'),
(1912, 108, 'Saint Elizabeth'),
(1913, 108, 'Saint James'),
(1914, 108, 'Saint Mary'),
(1915, 108, 'Saint Thomas'),
(1916, 108, 'Trelawney'),
(1917, 108, 'Westmoreland'),
(1918, 109, 'Aichi'),
(1919, 109, 'Akita'),
(1920, 109, 'Aomori'),
(1921, 109, 'Chiba'),
(1922, 109, 'Ehime'),
(1923, 109, 'Fukui'),
(1924, 109, 'Fukuoka'),
(1925, 109, 'Fukushima'),
(1926, 109, 'Gifu'),
(1927, 109, 'Gumma'),
(1928, 109, 'Hiroshima'),
(1929, 109, 'Hokkaido'),
(1930, 109, 'Hyogo'),
(1931, 109, 'Ibaraki'),
(1932, 109, 'Ishikawa'),
(1933, 109, 'Iwate'),
(1934, 109, 'Kagawa'),
(1935, 109, 'Kagoshima'),
(1936, 109, 'Kanagawa'),
(1937, 109, 'Kanto'),
(1938, 109, 'Kochi'),
(1939, 109, 'Kumamoto'),
(1940, 109, 'Kyoto'),
(1941, 109, 'Mie'),
(1942, 109, 'Miyagi'),
(1943, 109, 'Miyazaki'),
(1944, 109, 'Nagano'),
(1945, 109, 'Nagasaki'),
(1946, 109, 'Nara'),
(1947, 109, 'Niigata'),
(1948, 109, 'Oita'),
(1949, 109, 'Okayama'),
(1950, 109, 'Okinawa'),
(1951, 109, 'Osaka'),
(1952, 109, 'Saga'),
(1953, 109, 'Saitama'),
(1954, 109, 'Shiga'),
(1955, 109, 'Shimane'),
(1956, 109, 'Shizuoka'),
(1957, 109, 'Tochigi'),
(1958, 109, 'Tokushima'),
(1959, 109, 'Tokyo'),
(1960, 109, 'Tottori'),
(1961, 109, 'Toyama'),
(1962, 109, 'Wakayama'),
(1963, 109, 'Yamagata'),
(1964, 109, 'Yamaguchi'),
(1965, 109, 'Yamanashi'),
(1966, 110, 'Grouville'),
(1967, 110, 'Saint Brelade'),
(1968, 110, 'Saint Clement'),
(1969, 110, 'Saint Helier'),
(1970, 110, 'Saint John'),
(1971, 110, 'Saint Lawrence'),
(1972, 110, 'Saint Martin'),
(1973, 110, 'Saint Mary'),
(1974, 110, 'Saint Peter'),
(1975, 110, 'Saint Saviour'),
(1976, 110, 'Trinity'),
(1977, 111, '\'Ajlun'),
(1978, 111, 'Amman'),
(1979, 111, 'Irbid'),
(1980, 111, 'Jarash'),
(1981, 111, 'Ma\'an'),
(1982, 111, 'Madaba'),
(1983, 111, 'al-\'Aqabah'),
(1984, 111, 'al-Balqa\''),
(1985, 111, 'al-Karak'),
(1986, 111, 'al-Mafraq'),
(1987, 111, 'at-Tafilah'),
(1988, 111, 'az-Zarqa\''),
(1989, 112, 'Akmecet'),
(1990, 112, 'Akmola'),
(1991, 112, 'Aktobe'),
(1992, 112, 'Almati'),
(1993, 112, 'Atirau'),
(1994, 112, 'Batis Kazakstan'),
(1995, 112, 'Burlinsky Region'),
(1996, 112, 'Karagandi'),
(1997, 112, 'Kostanay'),
(1998, 112, 'Mankistau'),
(1999, 112, 'Ontustik Kazakstan'),
(2000, 112, 'Pavlodar'),
(2001, 112, 'Sigis Kazakstan'),
(2002, 112, 'Soltustik Kazakstan'),
(2003, 112, 'Taraz'),
(2004, 113, 'Central'),
(2005, 113, 'Coast'),
(2006, 113, 'Eastern'),
(2007, 113, 'Nairobi'),
(2008, 113, 'North Eastern'),
(2009, 113, 'Nyanza'),
(2010, 113, 'Rift Valley'),
(2011, 113, 'Western'),
(2012, 114, 'Abaiang'),
(2013, 114, 'Abemana'),
(2014, 114, 'Aranuka'),
(2015, 114, 'Arorae'),
(2016, 114, 'Banaba'),
(2017, 114, 'Beru'),
(2018, 114, 'Butaritari'),
(2019, 114, 'Kiritimati'),
(2020, 114, 'Kuria'),
(2021, 114, 'Maiana'),
(2022, 114, 'Makin'),
(2023, 114, 'Marakei'),
(2024, 114, 'Nikunau'),
(2025, 114, 'Nonouti'),
(2026, 114, 'Onotoa'),
(2027, 114, 'Phoenix Islands'),
(2028, 114, 'Tabiteuea North'),
(2029, 114, 'Tabiteuea South'),
(2030, 114, 'Tabuaeran'),
(2031, 114, 'Tamana'),
(2032, 114, 'Tarawa North'),
(2033, 114, 'Tarawa South'),
(2034, 114, 'Teraina'),
(2035, 115, 'Chagangdo'),
(2036, 115, 'Hamgyeongbukto'),
(2037, 115, 'Hamgyeongnamdo'),
(2038, 115, 'Hwanghaebukto'),
(2039, 115, 'Hwanghaenamdo'),
(2040, 115, 'Kaeseong'),
(2041, 115, 'Kangweon'),
(2042, 115, 'Nampo'),
(2043, 115, 'Pyeonganbukto'),
(2044, 115, 'Pyeongannamdo'),
(2045, 115, 'Pyeongyang'),
(2046, 115, 'Yanggang'),
(2047, 116, 'Busan'),
(2048, 116, 'Cheju'),
(2049, 116, 'Chollabuk'),
(2050, 116, 'Chollanam'),
(2051, 116, 'Chungbuk'),
(2052, 116, 'Chungcheongbuk'),
(2053, 116, 'Chungcheongnam'),
(2054, 116, 'Chungnam'),
(2055, 116, 'Daegu'),
(2056, 116, 'Gangwon-do'),
(2057, 116, 'Goyang-si'),
(2058, 116, 'Gyeonggi-do'),
(2059, 116, 'Gyeongsang '),
(2060, 116, 'Gyeongsangnam-do'),
(2061, 116, 'Incheon'),
(2062, 116, 'Jeju-Si'),
(2063, 116, 'Jeonbuk'),
(2064, 116, 'Kangweon'),
(2065, 116, 'Kwangju'),
(2066, 116, 'Kyeonggi'),
(2067, 116, 'Kyeongsangbuk'),
(2068, 116, 'Kyeongsangnam'),
(2069, 116, 'Kyonggi-do'),
(2070, 116, 'Kyungbuk-Do'),
(2071, 116, 'Kyunggi-Do'),
(2072, 116, 'Kyunggi-do'),
(2073, 116, 'Pusan'),
(2074, 116, 'Seoul'),
(2075, 116, 'Sudogwon'),
(2076, 116, 'Taegu'),
(2077, 116, 'Taejeon'),
(2078, 116, 'Taejon-gwangyoksi'),
(2079, 116, 'Ulsan'),
(2080, 116, 'Wonju'),
(2081, 116, 'gwangyoksi'),
(2082, 117, 'Al Asimah'),
(2083, 117, 'Hawalli'),
(2084, 117, 'Mishref'),
(2085, 117, 'Qadesiya'),
(2086, 117, 'Safat'),
(2087, 117, 'Salmiya'),
(2088, 117, 'al-Ahmadi'),
(2089, 117, 'al-Farwaniyah'),
(2090, 117, 'al-Jahra'),
(2091, 117, 'al-Kuwayt'),
(2092, 118, 'Batken'),
(2093, 118, 'Bishkek'),
(2094, 118, 'Chui'),
(2095, 118, 'Issyk-Kul'),
(2096, 118, 'Jalal-Abad'),
(2097, 118, 'Naryn'),
(2098, 118, 'Osh'),
(2099, 118, 'Talas'),
(2100, 119, 'Attopu'),
(2101, 119, 'Bokeo'),
(2102, 119, 'Bolikhamsay'),
(2103, 119, 'Champasak'),
(2104, 119, 'Houaphanh'),
(2105, 119, 'Khammouane'),
(2106, 119, 'Luang Nam Tha'),
(2107, 119, 'Luang Prabang'),
(2108, 119, 'Oudomxay'),
(2109, 119, 'Phongsaly'),
(2110, 119, 'Saravan'),
(2111, 119, 'Savannakhet'),
(2112, 119, 'Sekong'),
(2113, 119, 'Viangchan Prefecture'),
(2114, 119, 'Viangchan Province'),
(2115, 119, 'Xaignabury'),
(2116, 119, 'Xiang Khuang'),
(2117, 120, 'Aizkraukles'),
(2118, 120, 'Aluksnes'),
(2119, 120, 'Balvu'),
(2120, 120, 'Bauskas'),
(2121, 120, 'Cesu'),
(2122, 120, 'Daugavpils'),
(2123, 120, 'Daugavpils City'),
(2124, 120, 'Dobeles'),
(2125, 120, 'Gulbenes'),
(2126, 120, 'Jekabspils'),
(2127, 120, 'Jelgava'),
(2128, 120, 'Jelgavas'),
(2129, 120, 'Jurmala City'),
(2130, 120, 'Kraslavas'),
(2131, 120, 'Kuldigas'),
(2132, 120, 'Liepaja'),
(2133, 120, 'Liepajas'),
(2134, 120, 'Limbazhu'),
(2135, 120, 'Ludzas'),
(2136, 120, 'Madonas'),
(2137, 120, 'Ogres'),
(2138, 120, 'Preilu'),
(2139, 120, 'Rezekne'),
(2140, 120, 'Rezeknes'),
(2141, 120, 'Riga'),
(2142, 120, 'Rigas'),
(2143, 120, 'Saldus'),
(2144, 120, 'Talsu'),
(2145, 120, 'Tukuma'),
(2146, 120, 'Valkas'),
(2147, 120, 'Valmieras'),
(2148, 120, 'Ventspils'),
(2149, 120, 'Ventspils City'),
(2150, 121, 'Beirut'),
(2151, 121, 'Jabal Lubnan'),
(2152, 121, 'Mohafazat Liban-Nord'),
(2153, 121, 'Mohafazat Mont-Liban'),
(2154, 121, 'Sidon'),
(2155, 121, 'al-Biqa'),
(2156, 121, 'al-Janub'),
(2157, 121, 'an-Nabatiyah'),
(2158, 121, 'ash-Shamal'),
(2159, 122, 'Berea'),
(2160, 122, 'Butha-Buthe'),
(2161, 122, 'Leribe'),
(2162, 122, 'Mafeteng'),
(2163, 122, 'Maseru'),
(2164, 122, 'Mohale\'s Hoek'),
(2165, 122, 'Mokhotlong'),
(2166, 122, 'Qacha\'s Nek'),
(2167, 122, 'Quthing'),
(2168, 122, 'Thaba-Tseka'),
(2169, 123, 'Bomi'),
(2170, 123, 'Bong'),
(2171, 123, 'Grand Bassa'),
(2172, 123, 'Grand Cape Mount'),
(2173, 123, 'Grand Gedeh'),
(2174, 123, 'Loffa'),
(2175, 123, 'Margibi'),
(2176, 123, 'Maryland and Grand Kru'),
(2177, 123, 'Montserrado'),
(2178, 123, 'Nimba'),
(2179, 123, 'Rivercess'),
(2180, 123, 'Sinoe'),
(2181, 124, 'Ajdabiya'),
(2182, 124, 'Fezzan'),
(2183, 124, 'Banghazi'),
(2184, 124, 'Darnah'),
(2185, 124, 'Ghadamis'),
(2186, 124, 'Gharyan'),
(2187, 124, 'Misratah'),
(2188, 124, 'Murzuq'),
(2189, 124, 'Sabha'),
(2190, 124, 'Sawfajjin'),
(2191, 124, 'Surt');
INSERT INTO `province` (`province_id`, `country_id`, `province_name`) VALUES
(2192, 124, 'Tarabulus'),
(2193, 124, 'Tarhunah'),
(2194, 124, 'Tripolitania'),
(2195, 124, 'Tubruq'),
(2196, 124, 'Yafran'),
(2197, 124, 'Zlitan'),
(2198, 124, 'al-\'Aziziyah'),
(2199, 124, 'al-Fatih'),
(2200, 124, 'al-Jabal al Akhdar'),
(2201, 124, 'al-Jufrah'),
(2202, 124, 'al-Khums'),
(2203, 124, 'al-Kufrah'),
(2204, 124, 'an-Nuqat al-Khams'),
(2205, 124, 'ash-Shati\''),
(2206, 124, 'az-Zawiyah'),
(2207, 125, 'Balzers'),
(2208, 125, 'Eschen'),
(2209, 125, 'Gamprin'),
(2210, 125, 'Mauren'),
(2211, 125, 'Planken'),
(2212, 125, 'Ruggell'),
(2213, 125, 'Schaan'),
(2214, 125, 'Schellenberg'),
(2215, 125, 'Triesen'),
(2216, 125, 'Triesenberg'),
(2217, 125, 'Vaduz'),
(2218, 126, 'Alytaus'),
(2219, 126, 'Anyksciai'),
(2220, 126, 'Kauno'),
(2221, 126, 'Klaipedos'),
(2222, 126, 'Marijampoles'),
(2223, 126, 'Panevezhio'),
(2224, 126, 'Panevezys'),
(2225, 126, 'Shiauliu'),
(2226, 126, 'Taurages'),
(2227, 126, 'Telshiu'),
(2228, 126, 'Telsiai'),
(2229, 126, 'Utenos'),
(2230, 126, 'Vilniaus'),
(2231, 127, 'Capellen'),
(2232, 127, 'Clervaux'),
(2233, 127, 'Diekirch'),
(2234, 127, 'Echternach'),
(2235, 127, 'Esch-sur-Alzette'),
(2236, 127, 'Grevenmacher'),
(2237, 127, 'Luxembourg'),
(2238, 127, 'Mersch'),
(2239, 127, 'Redange'),
(2240, 127, 'Remich'),
(2241, 127, 'Vianden'),
(2242, 127, 'Wiltz'),
(2243, 128, 'Macau'),
(2244, 129, 'Berovo'),
(2245, 129, 'Bitola'),
(2246, 129, 'Brod'),
(2247, 129, 'Debar'),
(2248, 129, 'Delchevo'),
(2249, 129, 'Demir Hisar'),
(2250, 129, 'Gevgelija'),
(2251, 129, 'Gostivar'),
(2252, 129, 'Kavadarci'),
(2253, 129, 'Kichevo'),
(2254, 129, 'Kochani'),
(2255, 129, 'Kratovo'),
(2256, 129, 'Kriva Palanka'),
(2257, 129, 'Krushevo'),
(2258, 129, 'Kumanovo'),
(2259, 129, 'Negotino'),
(2260, 129, 'Ohrid'),
(2261, 129, 'Prilep'),
(2262, 129, 'Probishtip'),
(2263, 129, 'Radovish'),
(2264, 129, 'Resen'),
(2265, 129, 'Shtip'),
(2266, 129, 'Skopje'),
(2267, 129, 'Struga'),
(2268, 129, 'Strumica'),
(2269, 129, 'Sveti Nikole'),
(2270, 129, 'Tetovo'),
(2271, 129, 'Valandovo'),
(2272, 129, 'Veles'),
(2273, 129, 'Vinica'),
(2274, 130, 'Antananarivo'),
(2275, 130, 'Antsiranana'),
(2276, 130, 'Fianarantsoa'),
(2277, 130, 'Mahajanga'),
(2278, 130, 'Toamasina'),
(2279, 130, 'Toliary'),
(2280, 131, 'Balaka'),
(2281, 131, 'Blantyre City'),
(2282, 131, 'Chikwawa'),
(2283, 131, 'Chiradzulu'),
(2284, 131, 'Chitipa'),
(2285, 131, 'Dedza'),
(2286, 131, 'Dowa'),
(2287, 131, 'Karonga'),
(2288, 131, 'Kasungu'),
(2289, 131, 'Lilongwe City'),
(2290, 131, 'Machinga'),
(2291, 131, 'Mangochi'),
(2292, 131, 'Mchinji'),
(2293, 131, 'Mulanje'),
(2294, 131, 'Mwanza'),
(2295, 131, 'Mzimba'),
(2296, 131, 'Mzuzu City'),
(2297, 131, 'Nkhata Bay'),
(2298, 131, 'Nkhotakota'),
(2299, 131, 'Nsanje'),
(2300, 131, 'Ntcheu'),
(2301, 131, 'Ntchisi'),
(2302, 131, 'Phalombe'),
(2303, 131, 'Rumphi'),
(2304, 131, 'Salima'),
(2305, 131, 'Thyolo'),
(2306, 131, 'Zomba Municipality'),
(2307, 132, 'Johor'),
(2308, 132, 'Kedah'),
(2309, 132, 'Kelantan'),
(2310, 132, 'Kuala Lumpur'),
(2311, 132, 'Labuan'),
(2312, 132, 'Melaka'),
(2313, 132, 'Negeri Johor'),
(2314, 132, 'Negeri Sembilan'),
(2315, 132, 'Pahang'),
(2316, 132, 'Penang'),
(2317, 132, 'Perak'),
(2318, 132, 'Perlis'),
(2319, 132, 'Pulau Pinang'),
(2320, 132, 'Sabah'),
(2321, 132, 'Sarawak'),
(2322, 132, 'Selangor'),
(2323, 132, 'Sembilan'),
(2324, 132, 'Terengganu'),
(2325, 133, 'Alif Alif'),
(2326, 133, 'Alif Dhaal'),
(2327, 133, 'Baa'),
(2328, 133, 'Dhaal'),
(2329, 133, 'Faaf'),
(2330, 133, 'Gaaf Alif'),
(2331, 133, 'Gaaf Dhaal'),
(2332, 133, 'Ghaviyani'),
(2333, 133, 'Haa Alif'),
(2334, 133, 'Haa Dhaal'),
(2335, 133, 'Kaaf'),
(2336, 133, 'Laam'),
(2337, 133, 'Lhaviyani'),
(2338, 133, 'Male'),
(2339, 133, 'Miim'),
(2340, 133, 'Nuun'),
(2341, 133, 'Raa'),
(2342, 133, 'Shaviyani'),
(2343, 133, 'Siin'),
(2344, 133, 'Thaa'),
(2345, 133, 'Vaav'),
(2346, 134, 'Bamako'),
(2347, 134, 'Gao'),
(2348, 134, 'Kayes'),
(2349, 134, 'Kidal'),
(2350, 134, 'Koulikoro'),
(2351, 134, 'Mopti'),
(2352, 134, 'Segou'),
(2353, 134, 'Sikasso'),
(2354, 134, 'Tombouctou'),
(2355, 135, 'Gozo and Comino'),
(2356, 135, 'Inner Harbour'),
(2357, 135, 'Northern'),
(2358, 135, 'Outer Harbour'),
(2359, 135, 'South Eastern'),
(2360, 135, 'Valletta'),
(2361, 135, 'Western'),
(2362, 136, 'Castletown'),
(2363, 136, 'Douglas'),
(2364, 136, 'Laxey'),
(2365, 136, 'Onchan'),
(2366, 136, 'Peel'),
(2367, 136, 'Port Erin'),
(2368, 136, 'Port Saint Mary'),
(2369, 136, 'Ramsey'),
(2370, 137, 'Ailinlaplap'),
(2371, 137, 'Ailuk'),
(2372, 137, 'Arno'),
(2373, 137, 'Aur'),
(2374, 137, 'Bikini'),
(2375, 137, 'Ebon'),
(2376, 137, 'Enewetak'),
(2377, 137, 'Jabat'),
(2378, 137, 'Jaluit'),
(2379, 137, 'Kili'),
(2380, 137, 'Kwajalein'),
(2381, 137, 'Lae'),
(2382, 137, 'Lib'),
(2383, 137, 'Likiep'),
(2384, 137, 'Majuro'),
(2385, 137, 'Maloelap'),
(2386, 137, 'Mejit'),
(2387, 137, 'Mili'),
(2388, 137, 'Namorik'),
(2389, 137, 'Namu'),
(2390, 137, 'Rongelap'),
(2391, 137, 'Ujae'),
(2392, 137, 'Utrik'),
(2393, 137, 'Wotho'),
(2394, 137, 'Wotje'),
(2395, 138, 'Fort-de-France'),
(2396, 138, 'La Trinite'),
(2397, 138, 'Le Marin'),
(2398, 138, 'Saint-Pierre'),
(2399, 139, 'Adrar'),
(2400, 139, 'Assaba'),
(2401, 139, 'Brakna'),
(2402, 139, 'Dhakhlat Nawadibu'),
(2403, 139, 'Hudh-al-Gharbi'),
(2404, 139, 'Hudh-ash-Sharqi'),
(2405, 139, 'Inshiri'),
(2406, 139, 'Nawakshut'),
(2407, 139, 'Qidimagha'),
(2408, 139, 'Qurqul'),
(2409, 139, 'Taqant'),
(2410, 139, 'Tiris Zammur'),
(2411, 139, 'Trarza'),
(2412, 140, 'Black River'),
(2413, 140, 'Eau Coulee'),
(2414, 140, 'Flacq'),
(2415, 140, 'Floreal'),
(2416, 140, 'Grand Port'),
(2417, 140, 'Moka'),
(2418, 140, 'Pamplempousses'),
(2419, 140, 'Plaines Wilhelm'),
(2420, 140, 'Port Louis'),
(2421, 140, 'Riviere du Rempart'),
(2422, 140, 'Rodrigues'),
(2423, 140, 'Rose Hill'),
(2424, 140, 'Savanne'),
(2425, 141, 'Mayotte'),
(2426, 141, 'Pamanzi'),
(2427, 142, 'Aguascalientes'),
(2428, 142, 'Baja California'),
(2429, 142, 'Baja California Sur'),
(2430, 142, 'Campeche'),
(2431, 142, 'Chiapas'),
(2432, 142, 'Chihuahua'),
(2433, 142, 'Coahuila'),
(2434, 142, 'Colima'),
(2435, 142, 'Distrito Federal'),
(2436, 142, 'Durango'),
(2437, 142, 'Estado de Mexico'),
(2438, 142, 'Guanajuato'),
(2439, 142, 'Guerrero'),
(2440, 142, 'Hidalgo'),
(2441, 142, 'Jalisco'),
(2442, 142, 'Mexico'),
(2443, 142, 'Michoacan'),
(2444, 142, 'Morelos'),
(2445, 142, 'Nayarit'),
(2446, 142, 'Nuevo Leon'),
(2447, 142, 'Oaxaca'),
(2448, 142, 'Puebla'),
(2449, 142, 'Queretaro'),
(2450, 142, 'Quintana Roo'),
(2451, 142, 'San Luis Potosi'),
(2452, 142, 'Sinaloa'),
(2453, 142, 'Sonora'),
(2454, 142, 'Tabasco'),
(2455, 142, 'Tamaulipas'),
(2456, 142, 'Tlaxcala'),
(2457, 142, 'Veracruz'),
(2458, 142, 'Yucatan'),
(2459, 142, 'Zacatecas'),
(2460, 143, 'Chuuk'),
(2461, 143, 'Kusaie'),
(2462, 143, 'Pohnpei'),
(2463, 143, 'Yap'),
(2464, 144, 'Balti'),
(2465, 144, 'Cahul'),
(2466, 144, 'Chisinau'),
(2467, 144, 'Chisinau Oras'),
(2468, 144, 'Edinet'),
(2469, 144, 'Gagauzia'),
(2470, 144, 'Lapusna'),
(2471, 144, 'Orhei'),
(2472, 144, 'Soroca'),
(2473, 144, 'Taraclia'),
(2474, 144, 'Tighina'),
(2475, 144, 'Transnistria'),
(2476, 144, 'Ungheni'),
(2477, 145, 'Fontvieille'),
(2478, 145, 'La Condamine'),
(2479, 145, 'Monaco-Ville'),
(2480, 145, 'Monte Carlo'),
(2481, 146, 'Arhangaj'),
(2482, 146, 'Bajan-Olgij'),
(2483, 146, 'Bajanhongor'),
(2484, 146, 'Bulgan'),
(2485, 146, 'Darhan-Uul'),
(2486, 146, 'Dornod'),
(2487, 146, 'Dornogovi'),
(2488, 146, 'Dundgovi'),
(2489, 146, 'Govi-Altaj'),
(2490, 146, 'Govisumber'),
(2491, 146, 'Hentij'),
(2492, 146, 'Hovd'),
(2493, 146, 'Hovsgol'),
(2494, 146, 'Omnogovi'),
(2495, 146, 'Orhon'),
(2496, 146, 'Ovorhangaj'),
(2497, 146, 'Selenge'),
(2498, 146, 'Suhbaatar'),
(2499, 146, 'Tov'),
(2500, 146, 'Ulaanbaatar'),
(2501, 146, 'Uvs'),
(2502, 146, 'Zavhan'),
(2503, 147, 'Montserrat'),
(2504, 148, 'Agadir'),
(2505, 148, 'Casablanca'),
(2506, 148, 'Chaouia-Ouardigha'),
(2507, 148, 'Doukkala-Abda'),
(2508, 148, 'Fes-Boulemane'),
(2509, 148, 'Gharb-Chrarda-Beni Hssen'),
(2510, 148, 'Guelmim'),
(2511, 148, 'Kenitra'),
(2512, 148, 'Marrakech-Tensift-Al Haouz'),
(2513, 148, 'Meknes-Tafilalet'),
(2514, 148, 'Oriental'),
(2515, 148, 'Oujda'),
(2516, 148, 'Province de Tanger'),
(2517, 148, 'Rabat-Sale-Zammour-Zaer'),
(2518, 148, 'Sala Al Jadida'),
(2519, 148, 'Settat'),
(2520, 148, 'Souss Massa-Draa'),
(2521, 148, 'Tadla-Azilal'),
(2522, 148, 'Tangier-Tetouan'),
(2523, 148, 'Taza-Al Hoceima-Taounate'),
(2524, 148, 'Wilaya de Casablanca'),
(2525, 148, 'Wilaya de Rabat-Sale'),
(2526, 149, 'Cabo Delgado'),
(2527, 149, 'Gaza'),
(2528, 149, 'Inhambane'),
(2529, 149, 'Manica'),
(2530, 149, 'Maputo'),
(2531, 149, 'Maputo Provincia'),
(2532, 149, 'Nampula'),
(2533, 149, 'Niassa'),
(2534, 149, 'Sofala'),
(2535, 149, 'Tete'),
(2536, 149, 'Zambezia'),
(2537, 150, 'Ayeyarwady'),
(2538, 150, 'Bago'),
(2539, 150, 'Chin'),
(2540, 150, 'Kachin'),
(2541, 150, 'Kayah'),
(2542, 150, 'Kayin'),
(2543, 150, 'Magway'),
(2544, 150, 'Mandalay'),
(2545, 150, 'Mon'),
(2546, 150, 'Nay Pyi Taw'),
(2547, 150, 'Rakhine'),
(2548, 150, 'Sagaing'),
(2549, 150, 'Shan'),
(2550, 150, 'Tanintharyi'),
(2551, 150, 'Yangon'),
(2552, 151, 'Caprivi'),
(2553, 151, 'Erongo'),
(2554, 151, 'Hardap'),
(2555, 151, 'Karas'),
(2556, 151, 'Kavango'),
(2557, 151, 'Khomas'),
(2558, 151, 'Kunene'),
(2559, 151, 'Ohangwena'),
(2560, 151, 'Omaheke'),
(2561, 151, 'Omusati'),
(2562, 151, 'Oshana'),
(2563, 151, 'Oshikoto'),
(2564, 151, 'Otjozondjupa'),
(2565, 152, 'Yaren'),
(2566, 153, 'Bagmati'),
(2567, 153, 'Bheri'),
(2568, 153, 'Dhawalagiri'),
(2569, 153, 'Gandaki'),
(2570, 153, 'Janakpur'),
(2571, 153, 'Karnali'),
(2572, 153, 'Koshi'),
(2573, 153, 'Lumbini'),
(2574, 153, 'Mahakali'),
(2575, 153, 'Mechi'),
(2576, 153, 'Narayani'),
(2577, 153, 'Rapti'),
(2578, 153, 'Sagarmatha'),
(2579, 153, 'Seti'),
(2580, 154, 'Bonaire'),
(2581, 154, 'Curacao'),
(2582, 154, 'Saba'),
(2583, 154, 'Sint Eustatius'),
(2584, 154, 'Sint Maarten'),
(2585, 155, 'Amsterdam'),
(2586, 155, 'Benelux'),
(2587, 155, 'Drenthe'),
(2588, 155, 'Flevoland'),
(2589, 155, 'Friesland'),
(2590, 155, 'Gelderland'),
(2591, 155, 'Groningen'),
(2592, 155, 'Limburg'),
(2593, 155, 'Noord-Brabant'),
(2594, 155, 'Noord-Holland'),
(2595, 155, 'Overijssel'),
(2596, 155, 'South Holland'),
(2597, 155, 'Utrecht'),
(2598, 155, 'Zeeland'),
(2599, 155, 'Zuid-Holland'),
(2600, 156, 'Iles'),
(2601, 156, 'Nord'),
(2602, 156, 'Sud'),
(2603, 157, 'Area Outside Region'),
(2604, 157, 'Auckland'),
(2605, 157, 'Bay of Plenty'),
(2606, 157, 'Canterbury'),
(2607, 157, 'Christchurch'),
(2608, 157, 'Gisborne'),
(2609, 157, 'Hawke\'s Bay'),
(2610, 157, 'Manawatu-Wanganui'),
(2611, 157, 'Marlborough'),
(2612, 157, 'Nelson'),
(2613, 157, 'Northland'),
(2614, 157, 'Otago'),
(2615, 157, 'Rodney'),
(2616, 157, 'Southland'),
(2617, 157, 'Taranaki'),
(2618, 157, 'Tasman'),
(2619, 157, 'Waikato'),
(2620, 157, 'Wellington'),
(2621, 157, 'West Coast'),
(2622, 158, 'Atlantico Norte'),
(2623, 158, 'Atlantico Sur'),
(2624, 158, 'Boaco'),
(2625, 158, 'Carazo'),
(2626, 158, 'Chinandega'),
(2627, 158, 'Chontales'),
(2628, 158, 'Esteli'),
(2629, 158, 'Granada'),
(2630, 158, 'Jinotega'),
(2631, 158, 'Leon'),
(2632, 158, 'Madriz'),
(2633, 158, 'Managua'),
(2634, 158, 'Masaya'),
(2635, 158, 'Matagalpa'),
(2636, 158, 'Nueva Segovia'),
(2637, 158, 'Rio San Juan'),
(2638, 158, 'Rivas'),
(2639, 159, 'Agadez'),
(2640, 159, 'Diffa'),
(2641, 159, 'Dosso'),
(2642, 159, 'Maradi'),
(2643, 159, 'Niamey'),
(2644, 159, 'Tahoua'),
(2645, 159, 'Tillabery'),
(2646, 159, 'Zinder'),
(2647, 160, 'Abia'),
(2648, 160, 'Abuja Federal Capital Territory'),
(2649, 160, 'Adamawa'),
(2650, 160, 'Akwa Ibom'),
(2651, 160, 'Anambra'),
(2652, 160, 'Bauchi'),
(2653, 160, 'Bayelsa'),
(2654, 160, 'Benue'),
(2655, 160, 'Borno'),
(2656, 160, 'Cross River'),
(2657, 160, 'Delta'),
(2658, 160, 'Ebonyi'),
(2659, 160, 'Edo'),
(2660, 160, 'Ekiti'),
(2661, 160, 'Enugu'),
(2662, 160, 'Gombe'),
(2663, 160, 'Imo'),
(2664, 160, 'Jigawa'),
(2665, 160, 'Kaduna'),
(2666, 160, 'Kano'),
(2667, 160, 'Katsina'),
(2668, 160, 'Kebbi'),
(2669, 160, 'Kogi'),
(2670, 160, 'Kwara'),
(2671, 160, 'Lagos'),
(2672, 160, 'Nassarawa'),
(2673, 160, 'Niger'),
(2674, 160, 'Ogun'),
(2675, 160, 'Ondo'),
(2676, 160, 'Osun'),
(2677, 160, 'Oyo'),
(2678, 160, 'Plateau'),
(2679, 160, 'Rivers'),
(2680, 160, 'Sokoto'),
(2681, 160, 'Taraba'),
(2682, 160, 'Yobe'),
(2683, 160, 'Zamfara'),
(2684, 161, 'Niue'),
(2685, 162, 'Norfolk Island'),
(2686, 163, 'Northern Islands'),
(2687, 163, 'Rota'),
(2688, 163, 'Saipan'),
(2689, 163, 'Tinian'),
(2690, 164, 'Akershus'),
(2691, 164, 'Aust Agder'),
(2692, 164, 'Bergen'),
(2693, 164, 'Buskerud'),
(2694, 164, 'Finnmark'),
(2695, 164, 'Hedmark'),
(2696, 164, 'Hordaland'),
(2697, 164, 'Moere og Romsdal'),
(2698, 164, 'Nord Trondelag'),
(2699, 164, 'Nordland'),
(2700, 164, 'Oestfold'),
(2701, 164, 'Oppland'),
(2702, 164, 'Oslo'),
(2703, 164, 'Rogaland'),
(2704, 164, 'Soer Troendelag'),
(2705, 164, 'Sogn og Fjordane'),
(2706, 164, 'Stavern'),
(2707, 164, 'Sykkylven'),
(2708, 164, 'Telemark'),
(2709, 164, 'Troms'),
(2710, 164, 'Vest Agder'),
(2711, 164, 'Vestfold'),
(2712, 164, 'ÃƒÂ˜stfold'),
(2713, 165, 'Al Buraimi'),
(2714, 165, 'Dhufar'),
(2715, 165, 'Masqat'),
(2716, 165, 'Musandam'),
(2717, 165, 'Rusayl'),
(2718, 165, 'Wadi Kabir'),
(2719, 165, 'ad-Dakhiliyah'),
(2720, 165, 'adh-Dhahirah'),
(2721, 165, 'al-Batinah'),
(2722, 165, 'ash-Sharqiyah'),
(2723, 166, 'Baluchistan'),
(2724, 166, 'Federal Capital Area'),
(2725, 166, 'Federally administered Tribal '),
(2726, 166, 'North-West Frontier'),
(2727, 166, 'Northern Areas'),
(2728, 166, 'Punjab'),
(2729, 166, 'Sind'),
(2730, 167, 'Aimeliik'),
(2731, 167, 'Airai'),
(2732, 167, 'Angaur'),
(2733, 167, 'Hatobohei'),
(2734, 167, 'Kayangel'),
(2735, 167, 'Koror'),
(2736, 167, 'Melekeok'),
(2737, 167, 'Ngaraard'),
(2738, 167, 'Ngardmau'),
(2739, 167, 'Ngaremlengui'),
(2740, 167, 'Ngatpang'),
(2741, 167, 'Ngchesar'),
(2742, 167, 'Ngerchelong'),
(2743, 167, 'Ngiwal'),
(2744, 167, 'Peleliu'),
(2745, 167, 'Sonsorol'),
(2746, 168, 'Ariha'),
(2747, 168, 'Bayt Lahm'),
(2748, 168, 'Bethlehem'),
(2749, 168, 'Dayr-al-Balah'),
(2750, 168, 'Ghazzah'),
(2751, 168, 'Ghazzah ash-Shamaliyah'),
(2752, 168, 'Janin'),
(2753, 168, 'Khan Yunis'),
(2754, 168, 'Nabulus'),
(2755, 168, 'Qalqilyah'),
(2756, 168, 'Rafah'),
(2757, 168, 'Ram Allah wal-Birah'),
(2758, 168, 'Salfit'),
(2759, 168, 'Tubas'),
(2760, 168, 'Tulkarm'),
(2761, 168, 'al-Khalil'),
(2762, 168, 'al-Quds'),
(2763, 169, 'Bocas del Toro'),
(2764, 169, 'Chiriqui'),
(2765, 169, 'Cocle'),
(2766, 169, 'Colon'),
(2767, 169, 'Darien'),
(2768, 169, 'Embera'),
(2769, 169, 'Herrera'),
(2770, 169, 'Kuna Yala'),
(2771, 169, 'Los Santos'),
(2772, 169, 'Ngobe Bugle'),
(2773, 169, 'Panama'),
(2774, 169, 'Veraguas'),
(2775, 170, 'East New Britain'),
(2776, 170, 'East Sepik'),
(2777, 170, 'Eastern Highlands'),
(2778, 170, 'Enga'),
(2779, 170, 'Fly River'),
(2780, 170, 'Gulf'),
(2781, 170, 'Madang'),
(2782, 170, 'Manus'),
(2783, 170, 'Milne Bay'),
(2784, 170, 'Morobe'),
(2785, 170, 'National Capital District'),
(2786, 170, 'New Ireland'),
(2787, 170, 'North Solomons'),
(2788, 170, 'Oro'),
(2789, 170, 'Sandaun'),
(2790, 170, 'Simbu'),
(2791, 170, 'Southern Highlands'),
(2792, 170, 'West New Britain'),
(2793, 170, 'Western Highlands'),
(2794, 171, 'Alto Paraguay'),
(2795, 171, 'Alto Parana'),
(2796, 171, 'Amambay'),
(2797, 171, 'Asuncion'),
(2798, 171, 'Boqueron'),
(2799, 171, 'Caaguazu'),
(2800, 171, 'Caazapa'),
(2801, 171, 'Canendiyu'),
(2802, 171, 'Central'),
(2803, 171, 'Concepcion'),
(2804, 171, 'Cordillera'),
(2805, 171, 'Guaira'),
(2806, 171, 'Itapua'),
(2807, 171, 'Misiones'),
(2808, 171, 'Neembucu'),
(2809, 171, 'Paraguari'),
(2810, 171, 'Presidente Hayes'),
(2811, 171, 'San Pedro'),
(2812, 172, 'Amazonas'),
(2813, 172, 'Ancash'),
(2814, 172, 'Apurimac'),
(2815, 172, 'Arequipa'),
(2816, 172, 'Ayacucho'),
(2817, 172, 'Cajamarca'),
(2818, 172, 'Cusco'),
(2819, 172, 'Huancavelica'),
(2820, 172, 'Huanuco'),
(2821, 172, 'Ica'),
(2822, 172, 'Junin'),
(2823, 172, 'La Libertad'),
(2824, 172, 'Lambayeque'),
(2825, 172, 'Lima y Callao'),
(2826, 172, 'Loreto'),
(2827, 172, 'Madre de Dios'),
(2828, 172, 'Moquegua'),
(2829, 172, 'Pasco'),
(2830, 172, 'Piura'),
(2831, 172, 'Puno'),
(2832, 172, 'San Martin'),
(2833, 172, 'Tacna'),
(2834, 172, 'Tumbes'),
(2835, 172, 'Ucayali'),
(2836, 173, 'Batangas'),
(2837, 173, 'Bicol'),
(2838, 173, 'Bulacan'),
(2839, 173, 'Cagayan'),
(2840, 173, 'Caraga'),
(2841, 173, 'Central Luzon'),
(2842, 173, 'Central Mindanao'),
(2843, 173, 'Central Visayas'),
(2844, 173, 'Cordillera'),
(2845, 173, 'Davao'),
(2846, 173, 'Eastern Visayas'),
(2847, 173, 'Greater Metropolitan Area'),
(2848, 173, 'Ilocos'),
(2849, 173, 'Laguna'),
(2850, 173, 'Luzon'),
(2851, 173, 'Mactan'),
(2852, 173, 'Metropolitan Manila Area'),
(2853, 173, 'Muslim Mindanao'),
(2854, 173, 'Northern Mindanao'),
(2855, 173, 'Southern Mindanao'),
(2856, 173, 'Southern Tagalog'),
(2857, 173, 'Western Mindanao'),
(2858, 173, 'Western Visayas'),
(2859, 174, 'Pitcairn Island'),
(2860, 175, 'Biale Blota'),
(2861, 175, 'Dobroszyce'),
(2862, 175, 'Dolnoslaskie'),
(2863, 175, 'Dziekanow Lesny'),
(2864, 175, 'Hopowo'),
(2865, 175, 'Kartuzy'),
(2866, 175, 'Koscian'),
(2867, 175, 'Krakow'),
(2868, 175, 'Kujawsko-Pomorskie'),
(2869, 175, 'Lodzkie'),
(2870, 175, 'Lubelskie'),
(2871, 175, 'Lubuskie'),
(2872, 175, 'Malomice'),
(2873, 175, 'Malopolskie'),
(2874, 175, 'Mazowieckie'),
(2875, 175, 'Mirkow'),
(2876, 175, 'Opolskie'),
(2877, 175, 'Ostrowiec'),
(2878, 175, 'Podkarpackie'),
(2879, 175, 'Podlaskie'),
(2880, 175, 'Polska'),
(2881, 175, 'Pomorskie'),
(2882, 175, 'Poznan'),
(2883, 175, 'Pruszkow'),
(2884, 175, 'Rymanowska'),
(2885, 175, 'Rzeszow'),
(2886, 175, 'Slaskie'),
(2887, 175, 'Stare Pole'),
(2888, 175, 'Swietokrzyskie'),
(2889, 175, 'Warminsko-Mazurskie'),
(2890, 175, 'Warsaw'),
(2891, 175, 'Wejherowo'),
(2892, 175, 'Wielkopolskie'),
(2893, 175, 'Wroclaw'),
(2894, 175, 'Zachodnio-Pomorskie'),
(2895, 175, 'Zukowo'),
(2896, 176, 'Abrantes'),
(2897, 176, 'Acores'),
(2898, 176, 'Alentejo'),
(2899, 176, 'Algarve'),
(2900, 176, 'Braga'),
(2901, 176, 'Centro'),
(2902, 176, 'Distrito de Leiria'),
(2903, 176, 'Distrito de Viana do Castelo'),
(2904, 176, 'Distrito de Vila Real'),
(2905, 176, 'Distrito do Porto'),
(2906, 176, 'Lisboa e Vale do Tejo'),
(2907, 176, 'Madeira'),
(2908, 176, 'Norte'),
(2909, 176, 'Paivas'),
(2910, 177, 'Arecibo'),
(2911, 177, 'Bayamon'),
(2912, 177, 'Carolina'),
(2913, 177, 'Florida'),
(2914, 177, 'Guayama'),
(2915, 177, 'Humacao'),
(2916, 177, 'Mayaguez-Aguadilla'),
(2917, 177, 'Ponce'),
(2918, 177, 'Salinas'),
(2919, 177, 'San Juan'),
(2920, 178, 'Doha'),
(2921, 178, 'Jarian-al-Batnah'),
(2922, 178, 'Umm Salal'),
(2923, 178, 'ad-Dawhah'),
(2924, 178, 'al-Ghuwayriyah'),
(2925, 178, 'al-Jumayliyah'),
(2926, 178, 'al-Khawr'),
(2927, 178, 'al-Wakrah'),
(2928, 178, 'ar-Rayyan'),
(2929, 178, 'ash-Shamal'),
(2930, 179, 'Saint-Benoit'),
(2931, 179, 'Saint-Denis'),
(2932, 179, 'Saint-Paul'),
(2933, 179, 'Saint-Pierre'),
(2934, 180, 'Alba'),
(2935, 180, 'Arad'),
(2936, 180, 'Arges'),
(2937, 180, 'Bacau'),
(2938, 180, 'Bihor'),
(2939, 180, 'Bistrita-Nasaud'),
(2940, 180, 'Botosani'),
(2941, 180, 'Braila'),
(2942, 180, 'Brasov'),
(2943, 180, 'Bucuresti'),
(2944, 180, 'Buzau'),
(2945, 180, 'Calarasi'),
(2946, 180, 'Caras-Severin'),
(2947, 180, 'Cluj'),
(2948, 180, 'Constanta'),
(2949, 180, 'Covasna'),
(2950, 180, 'Dambovita'),
(2951, 180, 'Dolj'),
(2952, 180, 'Galati'),
(2953, 180, 'Giurgiu'),
(2954, 180, 'Gorj'),
(2955, 180, 'Harghita'),
(2956, 180, 'Hunedoara'),
(2957, 180, 'Ialomita'),
(2958, 180, 'Iasi'),
(2959, 180, 'Ilfov'),
(2960, 180, 'Maramures'),
(2961, 180, 'Mehedinti'),
(2962, 180, 'Mures'),
(2963, 180, 'Neamt'),
(2964, 180, 'Olt'),
(2965, 180, 'Prahova'),
(2966, 180, 'Salaj'),
(2967, 180, 'Satu Mare'),
(2968, 180, 'Sibiu'),
(2969, 180, 'Sondelor'),
(2970, 180, 'Suceava'),
(2971, 180, 'Teleorman'),
(2972, 180, 'Timis'),
(2973, 180, 'Tulcea'),
(2974, 180, 'Valcea'),
(2975, 180, 'Vaslui'),
(2976, 180, 'Vrancea'),
(2977, 181, 'Adygeja'),
(2978, 181, 'Aga'),
(2979, 181, 'Alanija'),
(2980, 181, 'Altaj'),
(2981, 181, 'Amur'),
(2982, 181, 'Arhangelsk'),
(2983, 181, 'Astrahan'),
(2984, 181, 'Bashkortostan'),
(2985, 181, 'Belgorod'),
(2986, 181, 'Brjansk'),
(2987, 181, 'Burjatija'),
(2988, 181, 'Chechenija'),
(2989, 181, 'Cheljabinsk'),
(2990, 181, 'Chita'),
(2991, 181, 'Chukotka'),
(2992, 181, 'Chuvashija'),
(2993, 181, 'Dagestan'),
(2994, 181, 'Evenkija'),
(2995, 181, 'Gorno-Altaj'),
(2996, 181, 'Habarovsk'),
(2997, 181, 'Hakasija'),
(2998, 181, 'Hanty-Mansija'),
(2999, 181, 'Ingusetija'),
(3000, 181, 'Irkutsk'),
(3001, 181, 'Ivanovo'),
(3002, 181, 'Jamalo-Nenets'),
(3003, 181, 'Jaroslavl'),
(3004, 181, 'Jevrej'),
(3005, 181, 'Kabardino-Balkarija'),
(3006, 181, 'Kaliningrad'),
(3007, 181, 'Kalmykija'),
(3008, 181, 'Kaluga'),
(3009, 181, 'Kamchatka'),
(3010, 181, 'Karachaj-Cherkessija'),
(3011, 181, 'Karelija'),
(3012, 181, 'Kemerovo'),
(3013, 181, 'Khabarovskiy Kray'),
(3014, 181, 'Kirov'),
(3015, 181, 'Komi'),
(3016, 181, 'Komi-Permjakija'),
(3017, 181, 'Korjakija'),
(3018, 181, 'Kostroma'),
(3019, 181, 'Krasnodar'),
(3020, 181, 'Krasnojarsk'),
(3021, 181, 'Krasnoyarskiy Kray'),
(3022, 181, 'Kurgan'),
(3023, 181, 'Kursk'),
(3024, 181, 'Leningrad'),
(3025, 181, 'Lipeck'),
(3026, 181, 'Magadan'),
(3027, 181, 'Marij El'),
(3028, 181, 'Mordovija'),
(3029, 181, 'Moscow'),
(3030, 181, 'Moskovskaja Oblast'),
(3031, 181, 'Moskovskaya Oblast'),
(3032, 181, 'Moskva'),
(3033, 181, 'Murmansk'),
(3034, 181, 'Nenets'),
(3035, 181, 'Nizhnij Novgorod'),
(3036, 181, 'Novgorod'),
(3037, 181, 'Novokusnezk'),
(3038, 181, 'Novosibirsk'),
(3039, 181, 'Omsk'),
(3040, 181, 'Orenburg'),
(3041, 181, 'Orjol'),
(3042, 181, 'Penza'),
(3043, 181, 'Perm'),
(3044, 181, 'Primorje'),
(3045, 181, 'Pskov'),
(3046, 181, 'Pskovskaya Oblast'),
(3047, 181, 'Rjazan'),
(3048, 181, 'Rostov'),
(3049, 181, 'Saha'),
(3050, 181, 'Sahalin'),
(3051, 181, 'Samara'),
(3052, 181, 'Samarskaya'),
(3053, 181, 'Sankt-Peterburg'),
(3054, 181, 'Saratov'),
(3055, 181, 'Smolensk'),
(3056, 181, 'Stavropol'),
(3057, 181, 'Sverdlovsk'),
(3058, 181, 'Tajmyrija'),
(3059, 181, 'Tambov'),
(3060, 181, 'Tatarstan'),
(3061, 181, 'Tjumen'),
(3062, 181, 'Tomsk'),
(3063, 181, 'Tula'),
(3064, 181, 'Tver'),
(3065, 181, 'Tyva'),
(3066, 181, 'Udmurtija'),
(3067, 181, 'Uljanovsk'),
(3068, 181, 'Ulyanovskaya Oblast'),
(3069, 181, 'Ust-Orda'),
(3070, 181, 'Vladimir'),
(3071, 181, 'Volgograd'),
(3072, 181, 'Vologda'),
(3073, 181, 'Voronezh'),
(3074, 182, 'Butare'),
(3075, 182, 'Byumba'),
(3076, 182, 'Cyangugu'),
(3077, 182, 'Gikongoro'),
(3078, 182, 'Gisenyi'),
(3079, 182, 'Gitarama'),
(3080, 182, 'Kibungo'),
(3081, 182, 'Kibuye'),
(3082, 182, 'Kigali-ngali'),
(3083, 182, 'Ruhengeri'),
(3084, 183, 'Ascension'),
(3085, 183, 'Gough Island'),
(3086, 183, 'Saint Helena'),
(3087, 183, 'Tristan da Cunha'),
(3088, 184, 'Christ Church Nichola Town'),
(3089, 184, 'Saint Anne Sandy Point'),
(3090, 184, 'Saint George Basseterre'),
(3091, 184, 'Saint George Gingerland'),
(3092, 184, 'Saint James Windward'),
(3093, 184, 'Saint John Capesterre'),
(3094, 184, 'Saint John Figtree'),
(3095, 184, 'Saint Mary Cayon'),
(3096, 184, 'Saint Paul Capesterre'),
(3097, 184, 'Saint Paul Charlestown'),
(3098, 184, 'Saint Peter Basseterre'),
(3099, 184, 'Saint Thomas Lowland'),
(3100, 184, 'Saint Thomas Middle Island'),
(3101, 184, 'Trinity Palmetto Point'),
(3102, 185, 'Anse-la-Raye'),
(3103, 185, 'Canaries'),
(3104, 185, 'Castries'),
(3105, 185, 'Choiseul'),
(3106, 185, 'Dennery'),
(3107, 185, 'Gros Inlet'),
(3108, 185, 'Laborie'),
(3109, 185, 'Micoud'),
(3110, 185, 'Soufriere'),
(3111, 185, 'Vieux Fort'),
(3112, 186, 'Miquelon-Langlade'),
(3113, 186, 'Saint-Pierre'),
(3114, 187, 'Charlotte'),
(3115, 187, 'Grenadines'),
(3116, 187, 'Saint Andrew'),
(3117, 187, 'Saint David'),
(3118, 187, 'Saint George'),
(3119, 187, 'Saint Patrick'),
(3120, 188, 'A\'ana'),
(3121, 188, 'Aiga-i-le-Tai'),
(3122, 188, 'Atua'),
(3123, 188, 'Fa\'asaleleaga'),
(3124, 188, 'Gaga\'emauga'),
(3125, 188, 'Gagaifomauga'),
(3126, 188, 'Palauli'),
(3127, 188, 'Satupa\'itea'),
(3128, 188, 'Tuamasaga'),
(3129, 188, 'Va\'a-o-Fonoti'),
(3130, 188, 'Vaisigano'),
(3131, 189, 'Acquaviva'),
(3132, 189, 'Borgo Maggiore'),
(3133, 189, 'Chiesanuova'),
(3134, 189, 'Domagnano'),
(3135, 189, 'Faetano'),
(3136, 189, 'Fiorentino'),
(3137, 189, 'Montegiardino'),
(3138, 189, 'San Marino'),
(3139, 189, 'Serravalle'),
(3140, 190, 'Agua Grande'),
(3141, 190, 'Cantagalo'),
(3142, 190, 'Lemba'),
(3143, 190, 'Lobata'),
(3144, 190, 'Me-Zochi'),
(3145, 190, 'Pague'),
(3146, 191, 'Al Khobar'),
(3147, 191, 'Aseer'),
(3148, 191, 'Ash Sharqiyah'),
(3149, 191, 'Asir'),
(3150, 191, 'Central Province'),
(3151, 191, 'Eastern Province'),
(3152, 191, 'Ha\'il'),
(3153, 191, 'Jawf'),
(3154, 191, 'Jizan'),
(3155, 191, 'Makkah'),
(3156, 191, 'Najran'),
(3157, 191, 'Qasim'),
(3158, 191, 'Tabuk'),
(3159, 191, 'Western Province'),
(3160, 191, 'al-Bahah'),
(3161, 191, 'al-Hudud-ash-Shamaliyah'),
(3162, 191, 'al-Madinah'),
(3163, 191, 'ar-Riyad'),
(3164, 192, 'Dakar'),
(3165, 192, 'Diourbel'),
(3166, 192, 'Fatick'),
(3167, 192, 'Kaolack'),
(3168, 192, 'Kolda'),
(3169, 192, 'Louga'),
(3170, 192, 'Saint-Louis'),
(3171, 192, 'Tambacounda'),
(3172, 192, 'Thies'),
(3173, 192, 'Ziguinchor'),
(3174, 193, 'Central Serbia'),
(3175, 193, 'Kosovo and Metohija'),
(3176, 193, 'Vojvodina'),
(3177, 194, 'Anse Boileau'),
(3178, 194, 'Anse Royale'),
(3179, 194, 'Cascade'),
(3180, 194, 'Takamaka'),
(3181, 194, 'Victoria'),
(3182, 195, 'Eastern'),
(3183, 195, 'Northern'),
(3184, 195, 'Southern'),
(3185, 195, 'Western'),
(3186, 196, 'Singapore'),
(3187, 197, 'Banskobystricky'),
(3188, 197, 'Bratislavsky'),
(3189, 197, 'Kosicky'),
(3190, 197, 'Nitriansky'),
(3191, 197, 'Presovsky'),
(3192, 197, 'Trenciansky'),
(3193, 197, 'Trnavsky'),
(3194, 197, 'Zilinsky'),
(3195, 198, 'Benedikt'),
(3196, 198, 'Gorenjska'),
(3197, 198, 'Gorishka'),
(3198, 198, 'Jugovzhodna Slovenija'),
(3199, 198, 'Koroshka'),
(3200, 198, 'Notranjsko-krashka'),
(3201, 198, 'Obalno-krashka'),
(3202, 198, 'Obcina Domzale'),
(3203, 198, 'Obcina Vitanje'),
(3204, 198, 'Osrednjeslovenska'),
(3205, 198, 'Podravska'),
(3206, 198, 'Pomurska'),
(3207, 198, 'Savinjska'),
(3208, 198, 'Slovenian Littoral'),
(3209, 198, 'Spodnjeposavska'),
(3210, 198, 'Zasavska'),
(3211, 199, 'Pitcairn'),
(3212, 200, 'Central'),
(3213, 200, 'Choiseul'),
(3214, 200, 'Guadalcanal'),
(3215, 200, 'Isabel'),
(3216, 200, 'Makira and Ulawa'),
(3217, 200, 'Malaita'),
(3218, 200, 'Rennell and Bellona'),
(3219, 200, 'Temotu'),
(3220, 200, 'Western'),
(3221, 201, 'Awdal'),
(3222, 201, 'Bakol'),
(3223, 201, 'Banadir'),
(3224, 201, 'Bari'),
(3225, 201, 'Bay'),
(3226, 201, 'Galgudug'),
(3227, 201, 'Gedo'),
(3228, 201, 'Hiran'),
(3229, 201, 'Jubbada Hose'),
(3230, 201, 'Jubbadha Dexe'),
(3231, 201, 'Mudug'),
(3232, 201, 'Nugal'),
(3233, 201, 'Sanag'),
(3234, 201, 'Shabellaha Dhexe'),
(3235, 201, 'Shabellaha Hose'),
(3236, 201, 'Togdher'),
(3237, 201, 'Woqoyi Galbed'),
(3238, 202, 'Eastern Cape'),
(3239, 202, 'Free State'),
(3240, 202, 'Gauteng'),
(3241, 202, 'Kempton Park'),
(3242, 202, 'Kramerville'),
(3243, 202, 'KwaZulu Natal'),
(3244, 202, 'Limpopo'),
(3245, 202, 'Mpumalanga'),
(3246, 202, 'North West'),
(3247, 202, 'Northern Cape'),
(3248, 202, 'Parow'),
(3249, 202, 'Table View'),
(3250, 202, 'Umtentweni'),
(3251, 202, 'Western Cape'),
(3252, 203, 'South Georgia'),
(3253, 204, 'Central Equatoria'),
(3254, 205, 'A Coruna'),
(3255, 205, 'Alacant'),
(3256, 205, 'Alava'),
(3257, 205, 'Albacete'),
(3258, 205, 'Almeria'),
(3260, 205, 'Asturias'),
(3261, 205, 'Avila'),
(3262, 205, 'Badajoz'),
(3263, 205, 'Balears'),
(3264, 205, 'Barcelona'),
(3267, 205, 'Burgos'),
(3268, 205, 'Caceres'),
(3269, 205, 'Cadiz'),
(3270, 205, 'Cantabria'),
(3271, 205, 'Castello'),
(3273, 205, 'Ceuta'),
(3274, 205, 'Ciudad Real'),
(3281, 205, 'Cordoba'),
(3282, 205, 'Cuenca'),
(3284, 205, 'Girona'),
(3285, 205, 'Granada'),
(3286, 205, 'Guadalajara'),
(3287, 205, 'Guipuzcoa'),
(3288, 205, 'Huelva'),
(3289, 205, 'Huesca'),
(3290, 205, 'Jaen'),
(3291, 205, 'La Rioja'),
(3292, 205, 'Las Palmas'),
(3293, 205, 'Leon'),
(3295, 205, 'Lleida'),
(3296, 205, 'Lugo'),
(3297, 205, 'Madrid'),
(3298, 205, 'Malaga'),
(3299, 205, 'Melilla'),
(3300, 205, 'Murcia'),
(3301, 205, 'Navarra'),
(3302, 205, 'Ourense'),
(3303, 205, 'Pais Vasco'),
(3304, 205, 'Palencia'),
(3305, 205, 'Pontevedra'),
(3306, 205, 'Salamanca'),
(3308, 205, 'Segovia'),
(3309, 205, 'Sevilla'),
(3310, 205, 'Soria'),
(3311, 205, 'Tarragona'),
(3312, 205, 'Santa Cruz de Tenerife'),
(3313, 205, 'Teruel'),
(3314, 205, 'Toledo'),
(3315, 205, 'Valencia'),
(3316, 205, 'Valladolid'),
(3317, 205, 'Vizcaya'),
(3318, 205, 'Zamora'),
(3319, 205, 'Zaragoza'),
(3320, 206, 'Amparai'),
(3321, 206, 'Anuradhapuraya'),
(3322, 206, 'Badulla'),
(3323, 206, 'Boralesgamuwa'),
(3324, 206, 'Colombo'),
(3325, 206, 'Galla'),
(3326, 206, 'Gampaha'),
(3327, 206, 'Hambantota'),
(3328, 206, 'Kalatura'),
(3329, 206, 'Kegalla'),
(3330, 206, 'Kilinochchi'),
(3331, 206, 'Kurunegala'),
(3332, 206, 'Madakalpuwa'),
(3333, 206, 'Maha Nuwara'),
(3334, 206, 'Malwana'),
(3335, 206, 'Mannarama'),
(3336, 206, 'Matale'),
(3337, 206, 'Matara'),
(3338, 206, 'Monaragala'),
(3339, 206, 'Mullaitivu'),
(3340, 206, 'North Eastern Province'),
(3341, 206, 'North Western Province'),
(3342, 206, 'Nuwara Eliya'),
(3343, 206, 'Polonnaruwa'),
(3344, 206, 'Puttalama'),
(3345, 206, 'Ratnapuraya'),
(3346, 206, 'Southern Province'),
(3347, 206, 'Tirikunamalaya'),
(3348, 206, 'Tuscany'),
(3349, 206, 'Vavuniyawa'),
(3350, 206, 'Western Province'),
(3351, 206, 'Yapanaya'),
(3352, 206, 'kadawatha'),
(3353, 207, 'A\'ali-an-Nil'),
(3354, 207, 'Bahr-al-Jabal'),
(3355, 207, 'Central Equatoria'),
(3356, 207, 'Gharb Bahr-al-Ghazal'),
(3357, 207, 'Gharb Darfur'),
(3358, 207, 'Gharb Kurdufan'),
(3359, 207, 'Gharb-al-Istiwa\'iyah'),
(3360, 207, 'Janub Darfur'),
(3361, 207, 'Janub Kurdufan'),
(3362, 207, 'Junqali'),
(3363, 207, 'Kassala'),
(3364, 207, 'Nahr-an-Nil'),
(3365, 207, 'Shamal Bahr-al-Ghazal'),
(3366, 207, 'Shamal Darfur'),
(3367, 207, 'Shamal Kurdufan'),
(3368, 207, 'Sharq-al-Istiwa\'iyah'),
(3369, 207, 'Sinnar'),
(3370, 207, 'Warab'),
(3371, 207, 'Wilayat al Khartum'),
(3372, 207, 'al-Bahr-al-Ahmar'),
(3373, 207, 'al-Buhayrat'),
(3374, 207, 'al-Jazirah'),
(3375, 207, 'al-Khartum'),
(3376, 207, 'al-Qadarif'),
(3377, 207, 'al-Wahdah'),
(3378, 207, 'an-Nil-al-Abyad'),
(3379, 207, 'an-Nil-al-Azraq'),
(3380, 207, 'ash-Shamaliyah'),
(3381, 208, 'Brokopondo'),
(3382, 208, 'Commewijne'),
(3383, 208, 'Coronie'),
(3384, 208, 'Marowijne'),
(3385, 208, 'Nickerie'),
(3386, 208, 'Para'),
(3387, 208, 'Paramaribo'),
(3388, 208, 'Saramacca'),
(3389, 208, 'Wanica'),
(3390, 209, 'Svalbard'),
(3391, 210, 'Hhohho'),
(3392, 210, 'Lubombo'),
(3393, 210, 'Manzini'),
(3394, 210, 'Shiselweni'),
(3395, 211, 'Alvsborgs Lan'),
(3396, 211, 'Angermanland'),
(3397, 211, 'Blekinge'),
(3398, 211, 'Bohuslan'),
(3399, 211, 'Dalarna'),
(3400, 211, 'Gavleborg'),
(3401, 211, 'Gaza'),
(3402, 211, 'Gotland'),
(3403, 211, 'Halland'),
(3404, 211, 'Jamtland'),
(3405, 211, 'Jonkoping'),
(3406, 211, 'Kalmar'),
(3407, 211, 'Kristianstads'),
(3408, 211, 'Kronoberg'),
(3409, 211, 'Norrbotten'),
(3410, 211, 'Orebro'),
(3411, 211, 'Ostergotland'),
(3412, 211, 'Saltsjo-Boo'),
(3413, 211, 'Skane'),
(3414, 211, 'Smaland'),
(3415, 211, 'Sodermanland'),
(3416, 211, 'Stockholm'),
(3417, 211, 'Uppsala'),
(3418, 211, 'Varmland'),
(3419, 211, 'Vasterbotten'),
(3420, 211, 'Vastergotland'),
(3421, 211, 'Vasternorrland'),
(3422, 211, 'Vastmanland'),
(3423, 211, 'Vastra Gotaland'),
(3424, 212, 'Aargau'),
(3425, 212, 'Appenzell Inner-Rhoden'),
(3426, 212, 'Appenzell-Ausser Rhoden'),
(3427, 212, 'Basel-Landschaft'),
(3428, 212, 'Basel-Stadt'),
(3429, 212, 'Bern'),
(3430, 212, 'Canton Ticino'),
(3431, 212, 'Fribourg'),
(3432, 212, 'Geneve'),
(3433, 212, 'Glarus'),
(3434, 212, 'Graubunden'),
(3435, 212, 'Heerbrugg'),
(3436, 212, 'Jura'),
(3437, 212, 'Kanton Aargau'),
(3438, 212, 'Luzern'),
(3439, 212, 'Morbio Inferiore'),
(3440, 212, 'Muhen'),
(3441, 212, 'Neuchatel'),
(3442, 212, 'Nidwalden'),
(3443, 212, 'Obwalden'),
(3444, 212, 'Sankt Gallen'),
(3445, 212, 'Schaffhausen'),
(3446, 212, 'Schwyz'),
(3447, 212, 'Solothurn'),
(3448, 212, 'Thurgau'),
(3449, 212, 'Ticino'),
(3450, 212, 'Uri'),
(3451, 212, 'Valais'),
(3452, 212, 'Vaud'),
(3453, 212, 'Vauffelin'),
(3454, 212, 'Zug'),
(3455, 212, 'Zurich'),
(3456, 213, 'Aleppo'),
(3457, 213, 'Dar\'a'),
(3458, 213, 'Dayr-az-Zawr'),
(3459, 213, 'Dimashq'),
(3460, 213, 'Halab'),
(3461, 213, 'Hamah'),
(3462, 213, 'Hims'),
(3463, 213, 'Idlib'),
(3464, 213, 'Madinat Dimashq'),
(3465, 213, 'Tartus'),
(3466, 213, 'al-Hasakah'),
(3467, 213, 'al-Ladhiqiyah'),
(3468, 213, 'al-Qunaytirah'),
(3469, 213, 'ar-Raqqah'),
(3470, 213, 'as-Suwayda'),
(3471, 214, 'Changhua County'),
(3472, 214, 'Chiayi County'),
(3473, 214, 'Chiayi City'),
(3474, 214, 'Taipei City'),
(3475, 214, 'Hsinchu County'),
(3476, 214, 'Hsinchu City'),
(3477, 214, 'Hualien County'),
(3480, 214, 'Kaohsiung City'),
(3481, 214, 'Keelung City'),
(3482, 214, 'Kinmen County'),
(3483, 214, 'Miaoli County'),
(3484, 214, 'Nantou County'),
(3486, 214, 'Penghu County'),
(3487, 214, 'Pingtung County'),
(3488, 214, 'Taichung City'),
(3492, 214, 'Tainan City'),
(3493, 214, 'New Taipei City'),
(3495, 214, 'Taitung County'),
(3496, 214, 'Taoyuan City'),
(3497, 214, 'Yilan County'),
(3498, 214, 'YunLin County'),
(3500, 215, 'Dushanbe'),
(3501, 215, 'Gorno-Badakhshan'),
(3502, 215, 'Karotegin'),
(3503, 215, 'Khatlon'),
(3504, 215, 'Sughd'),
(3505, 216, 'Arusha'),
(3506, 216, 'Dar es Salaam'),
(3507, 216, 'Dodoma'),
(3508, 216, 'Iringa'),
(3509, 216, 'Kagera'),
(3510, 216, 'Kigoma'),
(3511, 216, 'Kilimanjaro'),
(3512, 216, 'Lindi'),
(3513, 216, 'Mara'),
(3514, 216, 'Mbeya'),
(3515, 216, 'Morogoro'),
(3516, 216, 'Mtwara'),
(3517, 216, 'Mwanza'),
(3518, 216, 'Pwani'),
(3519, 216, 'Rukwa'),
(3520, 216, 'Ruvuma'),
(3521, 216, 'Shinyanga'),
(3522, 216, 'Singida'),
(3523, 216, 'Tabora'),
(3524, 216, 'Tanga'),
(3525, 216, 'Zanzibar and Pemba'),
(3526, 217, 'Amnat Charoen'),
(3527, 217, 'Ang Thong'),
(3528, 217, 'Bangkok'),
(3529, 217, 'Buri Ram'),
(3530, 217, 'Chachoengsao'),
(3531, 217, 'Chai Nat'),
(3532, 217, 'Chaiyaphum'),
(3533, 217, 'Changwat Chaiyaphum'),
(3534, 217, 'Chanthaburi'),
(3535, 217, 'Chiang Mai'),
(3536, 217, 'Chiang Rai'),
(3537, 217, 'Chon Buri'),
(3538, 217, 'Chumphon'),
(3539, 217, 'Kalasin'),
(3540, 217, 'Kamphaeng Phet'),
(3541, 217, 'Kanchanaburi'),
(3542, 217, 'Khon Kaen'),
(3543, 217, 'Krabi'),
(3544, 217, 'Krung Thep'),
(3545, 217, 'Lampang'),
(3546, 217, 'Lamphun'),
(3547, 217, 'Loei'),
(3548, 217, 'Lop Buri'),
(3549, 217, 'Mae Hong Son'),
(3550, 217, 'Maha Sarakham'),
(3551, 217, 'Mukdahan'),
(3552, 217, 'Nakhon Nayok'),
(3553, 217, 'Nakhon Pathom'),
(3554, 217, 'Nakhon Phanom'),
(3555, 217, 'Nakhon Ratchasima'),
(3556, 217, 'Nakhon Sawan'),
(3557, 217, 'Nakhon Si Thammarat'),
(3558, 217, 'Nan'),
(3559, 217, 'Narathiwat'),
(3560, 217, 'Nong Bua Lam Phu'),
(3561, 217, 'Nong Khai'),
(3562, 217, 'Nonthaburi'),
(3563, 217, 'Pathum Thani'),
(3564, 217, 'Pattani'),
(3565, 217, 'Phangnga'),
(3566, 217, 'Phatthalung'),
(3567, 217, 'Phayao'),
(3568, 217, 'Phetchabun'),
(3569, 217, 'Phetchaburi'),
(3570, 217, 'Phichit'),
(3571, 217, 'Phitsanulok'),
(3572, 217, 'Phra Nakhon Si Ayutthaya'),
(3573, 217, 'Phrae'),
(3574, 217, 'Phuket'),
(3575, 217, 'Prachin Buri'),
(3576, 217, 'Prachuap Khiri Khan'),
(3577, 217, 'Ranong'),
(3578, 217, 'Ratchaburi'),
(3579, 217, 'Rayong'),
(3580, 217, 'Roi Et'),
(3581, 217, 'Sa Kaeo'),
(3582, 217, 'Sakon Nakhon'),
(3583, 217, 'Samut Prakan'),
(3584, 217, 'Samut Sakhon'),
(3585, 217, 'Samut Songkhran'),
(3586, 217, 'Saraburi'),
(3587, 217, 'Satun'),
(3588, 217, 'Si Sa Ket'),
(3589, 217, 'Sing Buri'),
(3590, 217, 'Songkhla'),
(3591, 217, 'Sukhothai'),
(3592, 217, 'Suphan Buri'),
(3593, 217, 'Surat Thani'),
(3594, 217, 'Surin'),
(3595, 217, 'Tak'),
(3596, 217, 'Trang'),
(3597, 217, 'Trat'),
(3598, 217, 'Ubon Ratchathani'),
(3599, 217, 'Udon Thani'),
(3600, 217, 'Uthai Thani'),
(3601, 217, 'Uttaradit'),
(3602, 217, 'Yala'),
(3603, 217, 'Yasothon'),
(3604, 218, 'Centre'),
(3605, 218, 'Kara'),
(3606, 218, 'Maritime'),
(3607, 218, 'Plateaux'),
(3608, 218, 'Savanes'),
(3609, 219, 'Atafu'),
(3610, 219, 'Fakaofo'),
(3611, 219, 'Nukunonu'),
(3612, 220, 'Eua'),
(3613, 220, 'Ha\'apai'),
(3614, 220, 'Niuas'),
(3615, 220, 'Tongatapu'),
(3616, 220, 'Vava\'u'),
(3617, 221, 'Arima-Tunapuna-Piarco'),
(3618, 221, 'Caroni'),
(3619, 221, 'Chaguanas'),
(3620, 221, 'Couva-Tabaquite-Talparo'),
(3621, 221, 'Diego Martin'),
(3622, 221, 'Glencoe'),
(3623, 221, 'Penal Debe'),
(3624, 221, 'Point Fortin'),
(3625, 221, 'Port of Spain'),
(3626, 221, 'Princes Town'),
(3627, 221, 'Saint George'),
(3628, 221, 'San Fernando'),
(3629, 221, 'San Juan'),
(3630, 221, 'Sangre Grande'),
(3631, 221, 'Siparia'),
(3632, 221, 'Tobago'),
(3633, 222, 'Aryanah'),
(3634, 222, 'Bajah'),
(3635, 222, 'Bin \'Arus'),
(3636, 222, 'Binzart'),
(3637, 222, 'Gouvernorat de Ariana'),
(3638, 222, 'Gouvernorat de Nabeul'),
(3639, 222, 'Gouvernorat de Sousse'),
(3640, 222, 'Hammamet Yasmine'),
(3641, 222, 'Jundubah'),
(3642, 222, 'Madaniyin'),
(3643, 222, 'Manubah'),
(3644, 222, 'Monastir'),
(3645, 222, 'Nabul'),
(3646, 222, 'Qabis'),
(3647, 222, 'Qafsah'),
(3648, 222, 'Qibili'),
(3649, 222, 'Safaqis'),
(3650, 222, 'Sfax'),
(3651, 222, 'Sidi Bu Zayd'),
(3652, 222, 'Silyanah'),
(3653, 222, 'Susah'),
(3654, 222, 'Tatawin'),
(3655, 222, 'Tawzar'),
(3656, 222, 'Tunis'),
(3657, 222, 'Zaghwan'),
(3658, 222, 'al-Kaf'),
(3659, 222, 'al-Mahdiyah'),
(3660, 222, 'al-Munastir'),
(3661, 222, 'al-Qasrayn'),
(3662, 222, 'al-Qayrawan'),
(3663, 223, 'Adana'),
(3664, 223, 'Adiyaman'),
(3665, 223, 'Afyon'),
(3666, 223, 'Agri'),
(3667, 223, 'Aksaray'),
(3668, 223, 'Amasya'),
(3669, 223, 'Ankara'),
(3670, 223, 'Antalya'),
(3671, 223, 'Ardahan'),
(3672, 223, 'Artvin'),
(3673, 223, 'Aydin'),
(3674, 223, 'Balikesir'),
(3675, 223, 'Bartin'),
(3676, 223, 'Batman'),
(3677, 223, 'Bayburt'),
(3678, 223, 'Bilecik'),
(3679, 223, 'Bingol'),
(3680, 223, 'Bitlis'),
(3681, 223, 'Bolu'),
(3682, 223, 'Burdur'),
(3683, 223, 'Bursa'),
(3684, 223, 'Canakkale'),
(3685, 223, 'Cankiri'),
(3686, 223, 'Corum'),
(3687, 223, 'Denizli'),
(3688, 223, 'Diyarbakir'),
(3689, 223, 'Duzce'),
(3690, 223, 'Edirne'),
(3691, 223, 'Elazig'),
(3692, 223, 'Erzincan'),
(3693, 223, 'Erzurum'),
(3694, 223, 'Eskisehir'),
(3695, 223, 'Gaziantep'),
(3696, 223, 'Giresun'),
(3697, 223, 'Gumushane'),
(3698, 223, 'Hakkari'),
(3699, 223, 'Hatay'),
(3700, 223, 'Icel'),
(3701, 223, 'Igdir'),
(3702, 223, 'Isparta'),
(3703, 223, 'Istanbul'),
(3704, 223, 'Izmir'),
(3705, 223, 'Kahramanmaras'),
(3706, 223, 'Karabuk'),
(3707, 223, 'Karaman'),
(3708, 223, 'Kars'),
(3709, 223, 'Karsiyaka'),
(3710, 223, 'Kastamonu'),
(3711, 223, 'Kayseri'),
(3712, 223, 'Kilis'),
(3713, 223, 'Kirikkale'),
(3714, 223, 'Kirklareli'),
(3715, 223, 'Kirsehir'),
(3716, 223, 'Kocaeli'),
(3717, 223, 'Konya'),
(3718, 223, 'Kutahya'),
(3719, 223, 'Lefkosa'),
(3720, 223, 'Malatya'),
(3721, 223, 'Manisa'),
(3722, 223, 'Mardin'),
(3723, 223, 'Mugla'),
(3724, 223, 'Mus'),
(3725, 223, 'Nevsehir'),
(3726, 223, 'Nigde'),
(3727, 223, 'Ordu'),
(3728, 223, 'Osmaniye'),
(3729, 223, 'Rize'),
(3730, 223, 'Sakarya'),
(3731, 223, 'Samsun'),
(3732, 223, 'Sanliurfa'),
(3733, 223, 'Siirt'),
(3734, 223, 'Sinop'),
(3735, 223, 'Sirnak'),
(3736, 223, 'Sivas'),
(3737, 223, 'Tekirdag'),
(3738, 223, 'Tokat'),
(3739, 223, 'Trabzon'),
(3740, 223, 'Tunceli'),
(3741, 223, 'Usak'),
(3742, 223, 'Van'),
(3743, 223, 'Yalova'),
(3744, 223, 'Yozgat'),
(3745, 223, 'Zonguldak'),
(3746, 224, 'Ahal'),
(3747, 224, 'Asgabat'),
(3748, 224, 'Balkan'),
(3749, 224, 'Dasoguz'),
(3750, 224, 'Lebap'),
(3751, 224, 'Mari'),
(3752, 225, 'Grand Turk'),
(3753, 225, 'South Caicos and East Caicos'),
(3754, 226, 'Funafuti'),
(3755, 226, 'Nanumanga'),
(3756, 226, 'Nanumea'),
(3757, 226, 'Niutao'),
(3758, 226, 'Nui'),
(3759, 226, 'Nukufetau'),
(3760, 226, 'Nukulaelae'),
(3761, 226, 'Vaitupu'),
(3762, 227, 'Central'),
(3763, 227, 'Eastern'),
(3764, 227, 'Northern'),
(3765, 227, 'Western'),
(3766, 228, 'Cherkas\'ka'),
(3767, 228, 'Chernihivs\'ka'),
(3768, 228, 'Chernivets\'ka'),
(3769, 228, 'Crimea'),
(3770, 228, 'Dnipropetrovska'),
(3771, 228, 'Donets\'ka'),
(3772, 228, 'Ivano-Frankivs\'ka'),
(3773, 228, 'Kharkiv'),
(3774, 228, 'Kharkov'),
(3775, 228, 'Khersonska'),
(3776, 228, 'Khmel\'nyts\'ka'),
(3777, 228, 'Kirovohrad'),
(3778, 228, 'Krym'),
(3779, 228, 'Kyyiv'),
(3780, 228, 'Kyyivs\'ka'),
(3781, 228, 'L\'vivs\'ka'),
(3782, 228, 'Luhans\'ka'),
(3783, 228, 'Mykolayivs\'ka'),
(3784, 228, 'Odes\'ka'),
(3785, 228, 'Odessa'),
(3786, 228, 'Poltavs\'ka'),
(3787, 228, 'Rivnens\'ka'),
(3788, 228, 'Sevastopol\''),
(3789, 228, 'Sums\'ka'),
(3790, 228, 'Ternopil\'s\'ka'),
(3791, 228, 'Volyns\'ka'),
(3792, 228, 'Vynnyts\'ka'),
(3793, 228, 'Zakarpats\'ka'),
(3794, 228, 'Zaporizhia'),
(3795, 228, 'Zhytomyrs\'ka'),
(3796, 229, 'Abu Zabi'),
(3797, 229, 'Ajman'),
(3798, 229, 'Dubai'),
(3799, 229, 'Ras al-Khaymah'),
(3800, 229, 'Sharjah'),
(3801, 229, 'Sharjha'),
(3802, 229, 'Umm al Qaywayn'),
(3803, 229, 'al-Fujayrah'),
(3804, 229, 'ash-Shariqah'),
(3805, 230, 'Aberdeen'),
(3806, 230, 'Aberdeenshire'),
(3807, 230, 'Argyll'),
(3808, 230, 'Armagh'),
(3809, 230, 'Bedfordshire'),
(3810, 230, 'Belfast'),
(3811, 230, 'Berkshire'),
(3812, 230, 'Birmingham'),
(3813, 230, 'Brechin'),
(3814, 230, 'Bridgnorth'),
(3815, 230, 'Bristol'),
(3816, 230, 'Buckinghamshire'),
(3817, 230, 'Cambridge'),
(3818, 230, 'Cambridgeshire'),
(3819, 230, 'Channel Islands'),
(3820, 230, 'Cheshire'),
(3821, 230, 'Cleveland'),
(3822, 230, 'Co Fermanagh'),
(3823, 230, 'Conwy'),
(3824, 230, 'Cornwall'),
(3825, 230, 'Coventry'),
(3826, 230, 'Craven Arms'),
(3827, 230, 'Cumbria'),
(3828, 230, 'Denbighshire'),
(3829, 230, 'Derby'),
(3830, 230, 'Derbyshire'),
(3831, 230, 'Devon'),
(3832, 230, 'Dial Code Dungannon'),
(3833, 230, 'Didcot'),
(3834, 230, 'Dorset'),
(3835, 230, 'Dunbartonshire'),
(3836, 230, 'Durham'),
(3837, 230, 'East Dunbartonshire'),
(3838, 230, 'East Lothian'),
(3839, 230, 'East Midlands'),
(3840, 230, 'East Sussex'),
(3841, 230, 'East Yorkshire'),
(3842, 230, 'England'),
(3843, 230, 'Essex'),
(3844, 230, 'Fermanagh'),
(3845, 230, 'Fife'),
(3846, 230, 'Flintshire'),
(3847, 230, 'Fulham'),
(3848, 230, 'Gainsborough'),
(3849, 230, 'Glocestershire'),
(3850, 230, 'Gwent'),
(3851, 230, 'Hampshire'),
(3852, 230, 'Hants'),
(3853, 230, 'Herefordshire'),
(3854, 230, 'Hertfordshire'),
(3855, 230, 'Ireland'),
(3856, 230, 'Isle Of Man'),
(3857, 230, 'Isle of Wight'),
(3858, 230, 'Kenford'),
(3859, 230, 'Kent'),
(3860, 230, 'Kilmarnock'),
(3861, 230, 'Lanarkshire'),
(3862, 230, 'Lancashire'),
(3863, 230, 'Leicestershire'),
(3864, 230, 'Lincolnshire'),
(3865, 230, 'Llanymynech'),
(3866, 230, 'London'),
(3867, 230, 'Ludlow'),
(3868, 230, 'Manchester'),
(3869, 230, 'Mayfair'),
(3870, 230, 'Merseyside'),
(3871, 230, 'Mid Glamorgan'),
(3872, 230, 'Middlesex'),
(3873, 230, 'Mildenhall'),
(3874, 230, 'Monmouthshire'),
(3875, 230, 'Newton Stewart'),
(3876, 230, 'Norfolk'),
(3877, 230, 'North Humberside'),
(3878, 230, 'North Yorkshire'),
(3879, 230, 'Northamptonshire'),
(3880, 230, 'Northants'),
(3881, 230, 'Northern Ireland'),
(3882, 230, 'Northumberland'),
(3883, 230, 'Nottinghamshire'),
(3884, 230, 'Oxford'),
(3885, 230, 'Powys'),
(3886, 230, 'Roos-shire'),
(3887, 230, 'SUSSEX'),
(3888, 230, 'Sark'),
(3889, 230, 'Scotland'),
(3890, 230, 'Scottish Borders'),
(3891, 230, 'Shropshire'),
(3892, 230, 'Somerset'),
(3893, 230, 'South Glamorgan'),
(3894, 230, 'South Wales'),
(3895, 230, 'South Yorkshire'),
(3896, 230, 'Southwell'),
(3897, 230, 'Staffordshire'),
(3898, 230, 'Strabane'),
(3899, 230, 'Suffolk'),
(3900, 230, 'Surrey'),
(3901, 230, 'Sussex'),
(3902, 230, 'Twickenham'),
(3903, 230, 'Tyne and Wear'),
(3904, 230, 'Tyrone'),
(3905, 230, 'Utah'),
(3906, 230, 'Wales'),
(3907, 230, 'Warwickshire'),
(3908, 230, 'West Lothian'),
(3909, 230, 'West Midlands'),
(3910, 230, 'West Sussex'),
(3911, 230, 'West Yorkshire'),
(3912, 230, 'Whissendine'),
(3913, 230, 'Wiltshire'),
(3914, 230, 'Wokingham'),
(3915, 230, 'Worcestershire'),
(3916, 230, 'Wrexham'),
(3917, 230, 'Wurttemberg'),
(3918, 230, 'Yorkshire'),
(3919, 231, 'Alabama'),
(3920, 231, 'Alaska'),
(3921, 231, 'Arizona'),
(3922, 231, 'Arkansas'),
(3923, 231, 'Byram'),
(3924, 231, 'California'),
(3925, 231, 'Cokato'),
(3926, 231, 'Colorado'),
(3927, 231, 'Connecticut'),
(3928, 231, 'Delaware'),
(3929, 231, 'District of Columbia'),
(3930, 231, 'Florida'),
(3931, 231, 'Georgia'),
(3932, 231, 'Hawaii'),
(3933, 231, 'Idaho'),
(3934, 231, 'Illinois'),
(3935, 231, 'Indiana'),
(3936, 231, 'Iowa'),
(3937, 231, 'Kansas'),
(3938, 231, 'Kentucky'),
(3939, 231, 'Louisiana'),
(3940, 231, 'Lowa'),
(3941, 231, 'Maine'),
(3942, 231, 'Maryland'),
(3943, 231, 'Massachusetts'),
(3944, 231, 'Medfield'),
(3945, 231, 'Michigan'),
(3946, 231, 'Minnesota'),
(3947, 231, 'Mississippi'),
(3948, 231, 'Missouri'),
(3949, 231, 'Montana'),
(3950, 231, 'Nebraska'),
(3951, 231, 'Nevada'),
(3952, 231, 'New Hampshire'),
(3953, 231, 'New Jersey'),
(3954, 231, 'New Jersy'),
(3955, 231, 'New Mexico'),
(3956, 231, 'New York'),
(3957, 231, 'North Carolina'),
(3958, 231, 'North Dakota'),
(3959, 231, 'Ohio'),
(3960, 231, 'Oklahoma'),
(3961, 231, 'Ontario'),
(3962, 231, 'Oregon'),
(3963, 231, 'Pennsylvania'),
(3964, 231, 'Ramey'),
(3965, 231, 'Rhode Island'),
(3966, 231, 'South Carolina'),
(3967, 231, 'South Dakota'),
(3968, 231, 'Sublimity'),
(3969, 231, 'Tennessee'),
(3970, 231, 'Texas'),
(3971, 231, 'Trimble'),
(3972, 231, 'Utah'),
(3973, 231, 'Vermont'),
(3974, 231, 'Virginia'),
(3975, 231, 'Washington'),
(3976, 231, 'West Virginia'),
(3977, 231, 'Wisconsin'),
(3978, 231, 'Wyoming'),
(3979, 232, 'United States Minor Outlying I'),
(3980, 233, 'Artigas'),
(3981, 233, 'Canelones'),
(3982, 233, 'Cerro Largo'),
(3983, 233, 'Colonia'),
(3984, 233, 'Durazno'),
(3985, 233, 'FLorida'),
(3986, 233, 'Flores'),
(3987, 233, 'Lavalleja'),
(3988, 233, 'Maldonado'),
(3989, 233, 'Montevideo'),
(3990, 233, 'Paysandu'),
(3991, 233, 'Rio Negro'),
(3992, 233, 'Rivera'),
(3993, 233, 'Rocha'),
(3994, 233, 'Salto'),
(3995, 233, 'San Jose'),
(3996, 233, 'Soriano'),
(3997, 233, 'Tacuarembo'),
(3998, 233, 'Treinta y Tres'),
(3999, 234, 'Andijon'),
(4000, 234, 'Buhoro'),
(4001, 234, 'Buxoro Viloyati'),
(4002, 234, 'Cizah'),
(4003, 234, 'Fargona'),
(4004, 234, 'Horazm'),
(4005, 234, 'Kaskadar'),
(4006, 234, 'Korakalpogiston'),
(4007, 234, 'Namangan'),
(4008, 234, 'Navoi'),
(4009, 234, 'Samarkand'),
(4010, 234, 'Sirdare'),
(4011, 234, 'Surhondar'),
(4012, 234, 'Toskent'),
(4013, 235, 'Malampa'),
(4014, 235, 'Penama'),
(4015, 235, 'Sanma'),
(4016, 235, 'Shefa'),
(4017, 235, 'Tafea'),
(4018, 235, 'Torba'),
(4019, 236, 'Vatican City State (Holy See)'),
(4020, 237, 'Amazonas'),
(4021, 237, 'Anzoategui'),
(4022, 237, 'Apure'),
(4023, 237, 'Aragua'),
(4024, 237, 'Barinas'),
(4025, 237, 'Bolivar'),
(4026, 237, 'Carabobo'),
(4027, 237, 'Cojedes'),
(4028, 237, 'Delta Amacuro'),
(4029, 237, 'Distrito Federal'),
(4030, 237, 'Falcon'),
(4031, 237, 'Guarico'),
(4032, 237, 'Lara'),
(4033, 237, 'Merida'),
(4034, 237, 'Miranda'),
(4035, 237, 'Monagas'),
(4036, 237, 'Nueva Esparta'),
(4037, 237, 'Portuguesa'),
(4038, 237, 'Sucre'),
(4039, 237, 'Tachira'),
(4040, 237, 'Trujillo'),
(4041, 237, 'Vargas'),
(4042, 237, 'Yaracuy'),
(4043, 237, 'Zulia'),
(4044, 238, 'Bac Giang'),
(4045, 238, 'Binh Dinh'),
(4046, 238, 'Binh Duong'),
(4047, 238, 'Da Nang'),
(4048, 238, 'Dong Bang Song Cuu Long'),
(4049, 238, 'Dong Bang Song Hong'),
(4050, 238, 'Dong Nai'),
(4051, 238, 'Dong Nam Bo'),
(4052, 238, 'Duyen Hai Mien Trung'),
(4053, 238, 'Hanoi'),
(4054, 238, 'Hung Yen'),
(4055, 238, 'Khu Bon Cu'),
(4056, 238, 'Long An'),
(4057, 238, 'Mien Nui Va Trung Du'),
(4058, 238, 'Thai Nguyen'),
(4059, 238, 'Thanh Pho Ho Chi Minh'),
(4060, 238, 'Thu Do Ha Noi'),
(4061, 238, 'Tinh Can Tho'),
(4062, 238, 'Tinh Da Nang'),
(4063, 238, 'Tinh Gia Lai'),
(4064, 239, 'Anegada'),
(4065, 239, 'Jost van Dyke'),
(4066, 239, 'Tortola'),
(4067, 240, 'Saint Croix'),
(4068, 240, 'Saint John'),
(4069, 240, 'Saint Thomas'),
(4070, 241, 'Alo'),
(4071, 241, 'Singave'),
(4072, 241, 'Wallis'),
(4073, 242, 'Bu Jaydur'),
(4074, 242, 'Wad-adh-Dhahab'),
(4075, 242, 'al-\'Ayun'),
(4076, 242, 'as-Samarah'),
(4077, 243, '\'Adan'),
(4078, 243, 'Abyan'),
(4079, 243, 'Dhamar'),
(4080, 243, 'Hadramaut'),
(4081, 243, 'Hajjah'),
(4082, 243, 'Hudaydah'),
(4083, 243, 'Ibb'),
(4084, 243, 'Lahij'),
(4085, 243, 'Ma\'rib'),
(4086, 243, 'Madinat San\'a'),
(4087, 243, 'Sa\'dah'),
(4088, 243, 'Sana'),
(4089, 243, 'Shabwah'),
(4090, 243, 'Ta\'izz'),
(4091, 243, 'al-Bayda'),
(4092, 243, 'al-Hudaydah'),
(4093, 243, 'al-Jawf'),
(4094, 243, 'al-Mahrah'),
(4095, 243, 'al-Mahwit'),
(4096, 244, 'Central Serbia'),
(4097, 244, 'Kosovo and Metohija'),
(4098, 244, 'Montenegro'),
(4099, 244, 'Republic of Serbia'),
(4100, 244, 'Serbia'),
(4101, 244, 'Vojvodina'),
(4102, 245, 'Central'),
(4103, 245, 'Copperbelt'),
(4104, 245, 'Eastern'),
(4105, 245, 'Luapala'),
(4106, 245, 'Lusaka'),
(4107, 245, 'North-Western'),
(4108, 245, 'Northern'),
(4109, 245, 'Southern'),
(4110, 245, 'Western'),
(4111, 246, 'Bulawayo'),
(4112, 246, 'Harare'),
(4113, 246, 'Manicaland'),
(4114, 246, 'Mashonaland Central'),
(4115, 246, 'Mashonaland East'),
(4116, 246, 'Mashonaland West'),
(4117, 246, 'Masvingo'),
(4118, 246, 'Matabeleland North'),
(4119, 246, 'Matabeleland South'),
(4120, 246, 'Midlands'),
(4121, 214, 'Lienchiang County');

-- --------------------------------------------------------

--
-- Struktur dari tabel `stock_type`
--

CREATE TABLE `stock_type` (
  `stock_type_id` tinyint(4) NOT NULL,
  `stock_type` varchar(100) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

--
-- Dumping data untuk tabel `stock_type`
--

INSERT INTO `stock_type` (`stock_type_id`, `stock_type`) VALUES
(1, 'defined'),
(2, 'always_ready'),
(3, 'API');

--
-- Indexes for dumped tables
--

--
-- Indeks untuk tabel `business_type`
--
ALTER TABLE `business_type`
  ADD PRIMARY KEY (`business_type_id`);

--
-- Indeks untuk tabel `channel`
--
ALTER TABLE `channel`
  ADD PRIMARY KEY (`channel_id`);

--
-- Indeks untuk tabel `country`
--
ALTER TABLE `country`
  ADD PRIMARY KEY (`country_id`);

--
-- Indeks untuk tabel `group`
--
ALTER TABLE `group`
  ADD PRIMARY KEY (`group_id`),
  ADD KEY `business_type` (`business_type`),
  ADD KEY `membership_type` (`membership_type`);

--
-- Indeks untuk tabel `group_agenda`
--
ALTER TABLE `group_agenda`
  ADD PRIMARY KEY (`group_agenda_id`),
  ADD KEY `group_id` (`group_id`),
  ADD KEY `agenda_type_id` (`agenda_type_id`),
  ADD KEY `loop_id` (`loop_id`);

--
-- Indeks untuk tabel `group_agenda_general_logs`
--
ALTER TABLE `group_agenda_general_logs`
  ADD PRIMARY KEY (`id_logs`),
  ADD KEY `FK_logs_personal` (`member_id`),
  ADD KEY `FK_logs_group_agenda` (`id_group_agenda`),
  ADD KEY `group_id` (`group_id`);

--
-- Indeks untuk tabel `group_agenda_private`
--
ALTER TABLE `group_agenda_private`
  ADD PRIMARY KEY (`group_agenda_private_id`),
  ADD KEY `FK_group_agenda_private_group_agenda` (`id_group_agenda`),
  ADD KEY `FK_group_agenda_private_personal` (`id_member`),
  ADD KEY `group_id` (`group_id`);

--
-- Indeks untuk tabel `group_agenda_repeat_date`
--
ALTER TABLE `group_agenda_repeat_date`
  ADD PRIMARY KEY (`id_group_repeat`),
  ADD KEY `FK_group_agenda_repeat_date_group_agenda` (`group_agenda_id`);

--
-- Indeks untuk tabel `group_agenda_type`
--
ALTER TABLE `group_agenda_type`
  ADD PRIMARY KEY (`agenda_type_id`),
  ADD KEY `FK_group_agenda_type_group` (`group_id`);

--
-- Indeks untuk tabel `group_member`
--
ALTER TABLE `group_member`
  ADD PRIMARY KEY (`group_member_id`),
  ADD KEY `FK_group_member_personal` (`member_id`),
  ADD KEY `FK_group_member_membership_type` (`membership_type`),
  ADD KEY `FK_group_member_group_user_type` (`user_type`),
  ADD KEY `group_id` (`group_id`);

--
-- Indeks untuk tabel `group_member_agenda`
--
ALTER TABLE `group_member_agenda`
  ADD PRIMARY KEY (`group_member_agenda_id`),
  ADD KEY `FK_group_member_agenda_group_member` (`group_member_id`),
  ADD KEY `FK_group_member_agenda_personal` (`member_id`),
  ADD KEY `FK_group_member_agenda_group_agenda` (`group_agenda_id`),
  ADD KEY `group_id` (`group_id`);

--
-- Indeks untuk tabel `group_member_agenda_subscriber`
--
ALTER TABLE `group_member_agenda_subscriber`
  ADD PRIMARY KEY (`group_member_id`),
  ADD KEY `FK_group_member_agenda_subscriber_personal` (`member_id`),
  ADD KEY `FK_group_member_agenda_subscriber_group_agenda` (`agenda_type_id`),
  ADD KEY `group_id` (`group_id`);

--
-- Indeks untuk tabel `group_member_checkin`
--
ALTER TABLE `group_member_checkin`
  ADD PRIMARY KEY (`group_member_checkin_id`),
  ADD KEY `FK_group_member_checkin_personal` (`member_id`),
  ADD KEY `FK_group_member_checkin_group_member` (`group_member_id`),
  ADD KEY `FK_group_member_checkin_group` (`group_id`),
  ADD KEY `id_room` (`id_room`);

--
-- Indeks untuk tabel `group_member_order`
--
ALTER TABLE `group_member_order`
  ADD PRIMARY KEY (`group_member_order`),
  ADD KEY `FK_group_member_order_group_member` (`group_member_id`),
  ADD KEY `FK_group_member_order_personal` (`member_id`),
  ADD KEY `FK_group_member_order_group` (`group_id`);

--
-- Indeks untuk tabel `group_member_order_detail`
--
ALTER TABLE `group_member_order_detail`
  ADD PRIMARY KEY (`group_member_order_detail_id`),
  ADD KEY `FK_group_member_order_detail_group_member_order` (`group_member_order_id`),
  ADD KEY `FK_group_member_order_detail_personal` (`member_id`),
  ADD KEY `FK_group_member_order_detail_group_member` (`group_member_id`),
  ADD KEY `FK_group_member_order_detail_group` (`group_id`),
  ADD KEY `FK_group_member_order_detail_group_product` (`group_product_id`);

--
-- Indeks untuk tabel `group_operation_off`
--
ALTER TABLE `group_operation_off`
  ADD PRIMARY KEY (`operation_id`),
  ADD KEY `FK_group_operation_off_group` (`group_id`);

--
-- Indeks untuk tabel `group_product`
--
ALTER TABLE `group_product`
  ADD PRIMARY KEY (`group_product_id`),
  ADD KEY `FK_group_product_group` (`group_id`),
  ADD KEY `FK_group_product_product_type` (`product_type`),
  ADD KEY `FK_group_product_stock_type` (`stock_type`),
  ADD KEY `FK_group_product_order_type` (`order_type`);

--
-- Indeks untuk tabel `group_user_type`
--
ALTER TABLE `group_user_type`
  ADD PRIMARY KEY (`id_user_type`);

--
-- Indeks untuk tabel `lookup_stok`
--
ALTER TABLE `lookup_stok`
  ADD PRIMARY KEY (`lookup_stok_id`),
  ADD KEY `group_id` (`group_id`);

--
-- Indeks untuk tabel `lookup_token`
--
ALTER TABLE `lookup_token`
  ADD PRIMARY KEY (`id_lookup`),
  ADD KEY `FK_lookup_token_group` (`id_group`);

--
-- Indeks untuk tabel `loop_type`
--
ALTER TABLE `loop_type`
  ADD PRIMARY KEY (`id_loop`);

--
-- Indeks untuk tabel `membership_type`
--
ALTER TABLE `membership_type`
  ADD PRIMARY KEY (`membership_id`);

--
-- Indeks untuk tabel `order_type`
--
ALTER TABLE `order_type`
  ADD PRIMARY KEY (`order_type_id`);

--
-- Indeks untuk tabel `outbox_line`
--
ALTER TABLE `outbox_line`
  ADD PRIMARY KEY (`id_outbox`),
  ADD KEY `group_agenda_id` (`group_agenda_id`),
  ADD KEY `group_agenda_private_id` (`group_agenda_private_id`),
  ADD KEY `member_agenda_id` (`member_agenda_id`),
  ADD KEY `group_id` (`group_id`),
  ADD KEY `group_member_id` (`group_member_id`),
  ADD KEY `member_id` (`member_id`),
  ADD KEY `id_group_repeat` (`id_group_repeat`),
  ADD KEY `id_personal_repeat` (`id_personal_repeat`);

--
-- Indeks untuk tabel `outbox_mail_alibaba`
--
ALTER TABLE `outbox_mail_alibaba`
  ADD PRIMARY KEY (`id_outbox`),
  ADD KEY `group_agenda_id` (`group_agenda_id`),
  ADD KEY `group_agenda_private_id` (`group_agenda_private_id`),
  ADD KEY `member_agenda_id` (`member_agenda_id`),
  ADD KEY `group_id` (`group_id`),
  ADD KEY `group_member_id` (`group_member_id`),
  ADD KEY `member_id` (`member_id`),
  ADD KEY `id_personal_repeat` (`id_personal_repeat`),
  ADD KEY `id_group_repeat` (`id_group_repeat`);

--
-- Indeks untuk tabel `outbox_mail_aws`
--
ALTER TABLE `outbox_mail_aws`
  ADD PRIMARY KEY (`id_outbox`),
  ADD KEY `group_agenda_id` (`group_agenda_id`),
  ADD KEY `group_agenda_private_id` (`group_agenda_private_id`),
  ADD KEY `member_agenda_id` (`member_agenda_id`),
  ADD KEY `group_id` (`group_id`),
  ADD KEY `group_member_id` (`group_member_id`),
  ADD KEY `member_id` (`member_id`),
  ADD KEY `id_personal_repeat` (`id_personal_repeat`),
  ADD KEY `id_group_repeat` (`id_group_repeat`);

--
-- Indeks untuk tabel `outbox_mail_mailchimp`
--
ALTER TABLE `outbox_mail_mailchimp`
  ADD PRIMARY KEY (`id_outbox`),
  ADD KEY `group_agenda_id` (`group_agenda_id`),
  ADD KEY `group_agenda_private_id` (`group_agenda_private_id`),
  ADD KEY `member_agenda_id` (`member_agenda_id`),
  ADD KEY `group_id` (`group_id`),
  ADD KEY `group_member_id` (`group_member_id`),
  ADD KEY `member_id` (`member_id`),
  ADD KEY `id_personal_repeat` (`id_personal_repeat`),
  ADD KEY `id_group_repeat` (`id_group_repeat`);

--
-- Indeks untuk tabel `outbox_telegram`
--
ALTER TABLE `outbox_telegram`
  ADD PRIMARY KEY (`id_outbox`),
  ADD KEY `group_agenda_id` (`group_agenda_id`),
  ADD KEY `group_agenda_private_id` (`group_agenda_private_id`),
  ADD KEY `member_agenda_id` (`member_agenda_id`),
  ADD KEY `group_id` (`group_id`),
  ADD KEY `group_member_id` (`group_member_id`),
  ADD KEY `member_id` (`member_id`),
  ADD KEY `id_personal_repeat` (`id_personal_repeat`),
  ADD KEY `id_group_repeat` (`id_group_repeat`);

--
-- Indeks untuk tabel `outbox_whatsapp`
--
ALTER TABLE `outbox_whatsapp`
  ADD PRIMARY KEY (`id_outbox`),
  ADD KEY `group_agenda_id` (`group_agenda_id`),
  ADD KEY `group_agenda_private_id` (`group_agenda_private_id`),
  ADD KEY `member_agenda_id` (`member_agenda_id`),
  ADD KEY `group_id` (`group_id`),
  ADD KEY `group_member_id` (`group_member_id`),
  ADD KEY `member_id` (`member_id`),
  ADD KEY `id_personal_repeat` (`id_personal_repeat`),
  ADD KEY `id_group_repeat` (`id_group_repeat`);

--
-- Indeks untuk tabel `personal`
--
ALTER TABLE `personal`
  ADD PRIMARY KEY (`member_id`),
  ADD KEY `FK_personal_country` (`country_id`),
  ADD KEY `FK_personal_province` (`province_id`);

--
-- Indeks untuk tabel `personal_agenda`
--
ALTER TABLE `personal_agenda`
  ADD PRIMARY KEY (`member_agenda_id`),
  ADD KEY `FK_personal_agenda_personal` (`member_id`),
  ADD KEY `FK_personal_agenda_loop_type` (`loop_type`);

--
-- Indeks untuk tabel `personal_agenda_repeat_date`
--
ALTER TABLE `personal_agenda_repeat_date`
  ADD PRIMARY KEY (`id_group_repeat`),
  ADD KEY `FK_personal_agenda_repeat_date_personal_agenda` (`member_agenda_id`);

--
-- Indeks untuk tabel `personal_channel`
--
ALTER TABLE `personal_channel`
  ADD PRIMARY KEY (`member_ch_id`),
  ADD KEY `FK_personal_channel_personal` (`member_id`),
  ADD KEY `FK_personal_channel_channel` (`channel_id`);

--
-- Indeks untuk tabel `product_type`
--
ALTER TABLE `product_type`
  ADD PRIMARY KEY (`id_product_type`);

--
-- Indeks untuk tabel `project5_uggroups`
--
ALTER TABLE `project5_uggroups`
  ADD PRIMARY KEY (`GroupID`);

--
-- Indeks untuk tabel `project5_ugmembers`
--
ALTER TABLE `project5_ugmembers`
  ADD PRIMARY KEY (`UserName`(50),`GroupID`);

--
-- Indeks untuk tabel `project5_ugrights`
--
ALTER TABLE `project5_ugrights`
  ADD PRIMARY KEY (`TableName`(50),`GroupID`);

--
-- Indeks untuk tabel `province`
--
ALTER TABLE `province`
  ADD PRIMARY KEY (`province_id`),
  ADD KEY `FK_province_province` (`country_id`);

--
-- Indeks untuk tabel `stock_type`
--
ALTER TABLE `stock_type`
  ADD PRIMARY KEY (`stock_type_id`);

--
-- AUTO_INCREMENT untuk tabel yang dibuang
--

--
-- AUTO_INCREMENT untuk tabel `business_type`
--
ALTER TABLE `business_type`
  MODIFY `business_type_id` tinyint(4) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=6;

--
-- AUTO_INCREMENT untuk tabel `channel`
--
ALTER TABLE `channel`
  MODIFY `channel_id` tinyint(4) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;

--
-- AUTO_INCREMENT untuk tabel `country`
--
ALTER TABLE `country`
  MODIFY `country_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=247;

--
-- AUTO_INCREMENT untuk tabel `group`
--
ALTER TABLE `group`
  MODIFY `group_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;

--
-- AUTO_INCREMENT untuk tabel `group_agenda`
--
ALTER TABLE `group_agenda`
  MODIFY `group_agenda_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=57;

--
-- AUTO_INCREMENT untuk tabel `group_agenda_general_logs`
--
ALTER TABLE `group_agenda_general_logs`
  MODIFY `id_logs` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=94;

--
-- AUTO_INCREMENT untuk tabel `group_agenda_private`
--
ALTER TABLE `group_agenda_private`
  MODIFY `group_agenda_private_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=19;

--
-- AUTO_INCREMENT untuk tabel `group_agenda_repeat_date`
--
ALTER TABLE `group_agenda_repeat_date`
  MODIFY `id_group_repeat` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=11;

--
-- AUTO_INCREMENT untuk tabel `group_agenda_type`
--
ALTER TABLE `group_agenda_type`
  MODIFY `agenda_type_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;

--
-- AUTO_INCREMENT untuk tabel `group_member`
--
ALTER TABLE `group_member`
  MODIFY `group_member_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;

--
-- AUTO_INCREMENT untuk tabel `group_member_agenda`
--
ALTER TABLE `group_member_agenda`
  MODIFY `group_member_agenda_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=51;

--
-- AUTO_INCREMENT untuk tabel `group_member_agenda_subscriber`
--
ALTER TABLE `group_member_agenda_subscriber`
  MODIFY `group_member_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=6;

--
-- AUTO_INCREMENT untuk tabel `group_member_checkin`
--
ALTER TABLE `group_member_checkin`
  MODIFY `group_member_checkin_id` bigint(20) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT untuk tabel `group_member_order`
--
ALTER TABLE `group_member_order`
  MODIFY `group_member_order` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;

--
-- AUTO_INCREMENT untuk tabel `group_member_order_detail`
--
ALTER TABLE `group_member_order_detail`
  MODIFY `group_member_order_detail_id` bigint(20) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT untuk tabel `group_operation_off`
--
ALTER TABLE `group_operation_off`
  MODIFY `operation_id` bigint(20) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT untuk tabel `group_product`
--
ALTER TABLE `group_product`
  MODIFY `group_product_id` bigint(20) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT untuk tabel `group_user_type`
--
ALTER TABLE `group_user_type`
  MODIFY `id_user_type` tinyint(4) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;

--
-- AUTO_INCREMENT untuk tabel `lookup_stok`
--
ALTER TABLE `lookup_stok`
  MODIFY `lookup_stok_id` bigint(20) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT untuk tabel `lookup_token`
--
ALTER TABLE `lookup_token`
  MODIFY `id_lookup` bigint(20) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT untuk tabel `loop_type`
--
ALTER TABLE `loop_type`
  MODIFY `id_loop` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;

--
-- AUTO_INCREMENT untuk tabel `membership_type`
--
ALTER TABLE `membership_type`
  MODIFY `membership_id` tinyint(4) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;

--
-- AUTO_INCREMENT untuk tabel `order_type`
--
ALTER TABLE `order_type`
  MODIFY `order_type_id` tinyint(4) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;

--
-- AUTO_INCREMENT untuk tabel `outbox_line`
--
ALTER TABLE `outbox_line`
  MODIFY `id_outbox` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=130;

--
-- AUTO_INCREMENT untuk tabel `outbox_mail_alibaba`
--
ALTER TABLE `outbox_mail_alibaba`
  MODIFY `id_outbox` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=206;

--
-- AUTO_INCREMENT untuk tabel `outbox_mail_aws`
--
ALTER TABLE `outbox_mail_aws`
  MODIFY `id_outbox` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=211;

--
-- AUTO_INCREMENT untuk tabel `outbox_mail_mailchimp`
--
ALTER TABLE `outbox_mail_mailchimp`
  MODIFY `id_outbox` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=206;

--
-- AUTO_INCREMENT untuk tabel `outbox_telegram`
--
ALTER TABLE `outbox_telegram`
  MODIFY `id_outbox` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=203;

--
-- AUTO_INCREMENT untuk tabel `outbox_whatsapp`
--
ALTER TABLE `outbox_whatsapp`
  MODIFY `id_outbox` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=130;

--
-- AUTO_INCREMENT untuk tabel `personal`
--
ALTER TABLE `personal`
  MODIFY `member_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;

--
-- AUTO_INCREMENT untuk tabel `personal_agenda`
--
ALTER TABLE `personal_agenda`
  MODIFY `member_agenda_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=79;

--
-- AUTO_INCREMENT untuk tabel `personal_agenda_repeat_date`
--
ALTER TABLE `personal_agenda_repeat_date`
  MODIFY `id_group_repeat` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=19;

--
-- AUTO_INCREMENT untuk tabel `personal_channel`
--
ALTER TABLE `personal_channel`
  MODIFY `member_ch_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=11;

--
-- AUTO_INCREMENT untuk tabel `product_type`
--
ALTER TABLE `product_type`
  MODIFY `id_product_type` tinyint(4) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=8;

--
-- AUTO_INCREMENT untuk tabel `project5_uggroups`
--
ALTER TABLE `project5_uggroups`
  MODIFY `GroupID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;

--
-- AUTO_INCREMENT untuk tabel `province`
--
ALTER TABLE `province`
  MODIFY `province_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4122;

--
-- AUTO_INCREMENT untuk tabel `stock_type`
--
ALTER TABLE `stock_type`
  MODIFY `stock_type_id` tinyint(4) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;

--
-- Ketidakleluasaan untuk tabel pelimpahan (Dumped Tables)
--

--
-- Ketidakleluasaan untuk tabel `group`
--
ALTER TABLE `group`
  ADD CONSTRAINT `bussines_type` FOREIGN KEY (`business_type`) REFERENCES `business_type` (`business_type_id`),
  ADD CONSTRAINT `membership_type` FOREIGN KEY (`membership_type`) REFERENCES `membership_type` (`membership_id`);

--
-- Ketidakleluasaan untuk tabel `group_agenda`
--
ALTER TABLE `group_agenda`
  ADD CONSTRAINT `agenda_type_id` FOREIGN KEY (`agenda_type_id`) REFERENCES `group_agenda_type` (`agenda_type_id`),
  ADD CONSTRAINT `group_id` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`),
  ADD CONSTRAINT `loop_id` FOREIGN KEY (`loop_id`) REFERENCES `loop_type` (`id_loop`);

--
-- Ketidakleluasaan untuk tabel `group_agenda_general_logs`
--
ALTER TABLE `group_agenda_general_logs`
  ADD CONSTRAINT `FK_logs_group_agenda` FOREIGN KEY (`id_group_agenda`) REFERENCES `group_agenda` (`group_agenda_id`),
  ADD CONSTRAINT `FK_logs_personal` FOREIGN KEY (`member_id`) REFERENCES `personal` (`member_id`),
  ADD CONSTRAINT `group_agenda_general_logs_ibfk_1` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`);

--
-- Ketidakleluasaan untuk tabel `group_agenda_private`
--
ALTER TABLE `group_agenda_private`
  ADD CONSTRAINT `FK_group_agenda_private_group_agenda` FOREIGN KEY (`id_group_agenda`) REFERENCES `group_agenda` (`group_agenda_id`),
  ADD CONSTRAINT `FK_group_agenda_private_personal` FOREIGN KEY (`id_member`) REFERENCES `personal` (`member_id`),
  ADD CONSTRAINT `group_agenda_private_ibfk_1` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`);

--
-- Ketidakleluasaan untuk tabel `group_agenda_repeat_date`
--
ALTER TABLE `group_agenda_repeat_date`
  ADD CONSTRAINT `FK_group_agenda_repeat_date_group_agenda` FOREIGN KEY (`group_agenda_id`) REFERENCES `group_agenda` (`group_agenda_id`);

--
-- Ketidakleluasaan untuk tabel `group_agenda_type`
--
ALTER TABLE `group_agenda_type`
  ADD CONSTRAINT `FK_group_agenda_type_group` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`);

--
-- Ketidakleluasaan untuk tabel `group_member`
--
ALTER TABLE `group_member`
  ADD CONSTRAINT `FK_group_member_group_user_type` FOREIGN KEY (`user_type`) REFERENCES `group_user_type` (`id_user_type`),
  ADD CONSTRAINT `FK_group_member_membership_type` FOREIGN KEY (`membership_type`) REFERENCES `membership_type` (`membership_id`),
  ADD CONSTRAINT `FK_group_member_personal` FOREIGN KEY (`member_id`) REFERENCES `personal` (`member_id`),
  ADD CONSTRAINT `group_member_ibfk_1` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`);

--
-- Ketidakleluasaan untuk tabel `group_member_agenda`
--
ALTER TABLE `group_member_agenda`
  ADD CONSTRAINT `FK_group_member_agenda_group_agenda` FOREIGN KEY (`group_agenda_id`) REFERENCES `group_agenda` (`group_agenda_id`),
  ADD CONSTRAINT `FK_group_member_agenda_group_member` FOREIGN KEY (`group_member_id`) REFERENCES `group_member` (`group_member_id`),
  ADD CONSTRAINT `FK_group_member_agenda_personal` FOREIGN KEY (`member_id`) REFERENCES `personal` (`member_id`),
  ADD CONSTRAINT `group_member_agenda_ibfk_1` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`);

--
-- Ketidakleluasaan untuk tabel `group_member_agenda_subscriber`
--
ALTER TABLE `group_member_agenda_subscriber`
  ADD CONSTRAINT `FK_group_member_agenda_subscriber_personal` FOREIGN KEY (`member_id`) REFERENCES `personal` (`member_id`),
  ADD CONSTRAINT `group_member_agenda_subscriber_ibfk_1` FOREIGN KEY (`agenda_type_id`) REFERENCES `group_agenda_type` (`agenda_type_id`),
  ADD CONSTRAINT `group_member_agenda_subscriber_ibfk_2` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`);

--
-- Ketidakleluasaan untuk tabel `group_member_checkin`
--
ALTER TABLE `group_member_checkin`
  ADD CONSTRAINT `FK_group_member_checkin_group` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`),
  ADD CONSTRAINT `FK_group_member_checkin_group_member` FOREIGN KEY (`group_member_id`) REFERENCES `group_member` (`group_member_id`),
  ADD CONSTRAINT `FK_group_member_checkin_personal` FOREIGN KEY (`member_id`) REFERENCES `personal` (`member_id`);

--
-- Ketidakleluasaan untuk tabel `group_member_order`
--
ALTER TABLE `group_member_order`
  ADD CONSTRAINT `FK_group_member_order_group` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`),
  ADD CONSTRAINT `FK_group_member_order_group_member` FOREIGN KEY (`group_member_id`) REFERENCES `group_member` (`group_member_id`),
  ADD CONSTRAINT `FK_group_member_order_personal` FOREIGN KEY (`member_id`) REFERENCES `personal` (`member_id`);

--
-- Ketidakleluasaan untuk tabel `group_member_order_detail`
--
ALTER TABLE `group_member_order_detail`
  ADD CONSTRAINT `FK_group_member_order_detail_group` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`),
  ADD CONSTRAINT `FK_group_member_order_detail_group_member` FOREIGN KEY (`group_member_id`) REFERENCES `group_member` (`group_member_id`),
  ADD CONSTRAINT `FK_group_member_order_detail_group_member_order` FOREIGN KEY (`group_member_order_id`) REFERENCES `group_member_order` (`group_member_order`),
  ADD CONSTRAINT `FK_group_member_order_detail_group_product` FOREIGN KEY (`group_product_id`) REFERENCES `group_product` (`group_product_id`),
  ADD CONSTRAINT `FK_group_member_order_detail_personal` FOREIGN KEY (`member_id`) REFERENCES `personal` (`member_id`);

--
-- Ketidakleluasaan untuk tabel `group_operation_off`
--
ALTER TABLE `group_operation_off`
  ADD CONSTRAINT `FK_group_operation_off_group` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`);

--
-- Ketidakleluasaan untuk tabel `group_product`
--
ALTER TABLE `group_product`
  ADD CONSTRAINT `FK_group_product_group` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`),
  ADD CONSTRAINT `FK_group_product_order_type` FOREIGN KEY (`order_type`) REFERENCES `order_type` (`order_type_id`),
  ADD CONSTRAINT `FK_group_product_product_type` FOREIGN KEY (`product_type`) REFERENCES `product_type` (`id_product_type`),
  ADD CONSTRAINT `FK_group_product_stock_type` FOREIGN KEY (`stock_type`) REFERENCES `stock_type` (`stock_type_id`);

--
-- Ketidakleluasaan untuk tabel `lookup_stok`
--
ALTER TABLE `lookup_stok`
  ADD CONSTRAINT `lookup_stok_ibfk_1` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`);

--
-- Ketidakleluasaan untuk tabel `lookup_token`
--
ALTER TABLE `lookup_token`
  ADD CONSTRAINT `FK_lookup_token_group` FOREIGN KEY (`id_group`) REFERENCES `group` (`group_id`);

--
-- Ketidakleluasaan untuk tabel `outbox_line`
--
ALTER TABLE `outbox_line`
  ADD CONSTRAINT `outbox_line_ibfk_1` FOREIGN KEY (`group_agenda_id`) REFERENCES `group_agenda` (`group_agenda_id`),
  ADD CONSTRAINT `outbox_line_ibfk_2` FOREIGN KEY (`group_agenda_private_id`) REFERENCES `group_agenda_private` (`group_agenda_private_id`),
  ADD CONSTRAINT `outbox_line_ibfk_3` FOREIGN KEY (`member_agenda_id`) REFERENCES `personal_agenda` (`member_agenda_id`),
  ADD CONSTRAINT `outbox_line_ibfk_4` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`),
  ADD CONSTRAINT `outbox_line_ibfk_5` FOREIGN KEY (`group_member_id`) REFERENCES `group_member` (`group_member_id`),
  ADD CONSTRAINT `outbox_line_ibfk_6` FOREIGN KEY (`member_id`) REFERENCES `personal` (`member_id`),
  ADD CONSTRAINT `outbox_line_ibfk_7` FOREIGN KEY (`id_group_repeat`) REFERENCES `group_agenda_repeat_date` (`id_group_repeat`),
  ADD CONSTRAINT `outbox_line_ibfk_8` FOREIGN KEY (`id_personal_repeat`) REFERENCES `personal_agenda_repeat_date` (`id_group_repeat`);

--
-- Ketidakleluasaan untuk tabel `outbox_mail_alibaba`
--
ALTER TABLE `outbox_mail_alibaba`
  ADD CONSTRAINT `outbox_mail_alibaba_ibfk_1` FOREIGN KEY (`group_agenda_id`) REFERENCES `group_agenda` (`group_agenda_id`),
  ADD CONSTRAINT `outbox_mail_alibaba_ibfk_2` FOREIGN KEY (`group_agenda_private_id`) REFERENCES `group_agenda_private` (`group_agenda_private_id`),
  ADD CONSTRAINT `outbox_mail_alibaba_ibfk_3` FOREIGN KEY (`member_agenda_id`) REFERENCES `personal_agenda` (`member_agenda_id`),
  ADD CONSTRAINT `outbox_mail_alibaba_ibfk_4` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`),
  ADD CONSTRAINT `outbox_mail_alibaba_ibfk_5` FOREIGN KEY (`group_member_id`) REFERENCES `group_member` (`group_member_id`),
  ADD CONSTRAINT `outbox_mail_alibaba_ibfk_6` FOREIGN KEY (`member_id`) REFERENCES `personal` (`member_id`),
  ADD CONSTRAINT `outbox_mail_alibaba_ibfk_7` FOREIGN KEY (`id_personal_repeat`) REFERENCES `personal_agenda_repeat_date` (`id_group_repeat`),
  ADD CONSTRAINT `outbox_mail_alibaba_ibfk_8` FOREIGN KEY (`id_group_repeat`) REFERENCES `group_agenda_repeat_date` (`id_group_repeat`);

--
-- Ketidakleluasaan untuk tabel `outbox_mail_aws`
--
ALTER TABLE `outbox_mail_aws`
  ADD CONSTRAINT `outbox_mail_aws_ibfk_1` FOREIGN KEY (`group_agenda_id`) REFERENCES `group_agenda` (`group_agenda_id`),
  ADD CONSTRAINT `outbox_mail_aws_ibfk_2` FOREIGN KEY (`group_agenda_private_id`) REFERENCES `group_agenda_private` (`group_agenda_private_id`),
  ADD CONSTRAINT `outbox_mail_aws_ibfk_3` FOREIGN KEY (`member_agenda_id`) REFERENCES `personal_agenda` (`member_agenda_id`),
  ADD CONSTRAINT `outbox_mail_aws_ibfk_4` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`),
  ADD CONSTRAINT `outbox_mail_aws_ibfk_5` FOREIGN KEY (`group_member_id`) REFERENCES `group_member` (`group_member_id`),
  ADD CONSTRAINT `outbox_mail_aws_ibfk_6` FOREIGN KEY (`member_id`) REFERENCES `personal` (`member_id`),
  ADD CONSTRAINT `outbox_mail_aws_ibfk_7` FOREIGN KEY (`id_personal_repeat`) REFERENCES `personal_agenda_repeat_date` (`id_group_repeat`),
  ADD CONSTRAINT `outbox_mail_aws_ibfk_8` FOREIGN KEY (`id_group_repeat`) REFERENCES `group_agenda_repeat_date` (`id_group_repeat`);

--
-- Ketidakleluasaan untuk tabel `outbox_mail_mailchimp`
--
ALTER TABLE `outbox_mail_mailchimp`
  ADD CONSTRAINT `outbox_mail_mailchimp_ibfk_1` FOREIGN KEY (`group_agenda_id`) REFERENCES `group_agenda` (`group_agenda_id`),
  ADD CONSTRAINT `outbox_mail_mailchimp_ibfk_2` FOREIGN KEY (`group_agenda_private_id`) REFERENCES `group_agenda_private` (`group_agenda_private_id`),
  ADD CONSTRAINT `outbox_mail_mailchimp_ibfk_3` FOREIGN KEY (`member_agenda_id`) REFERENCES `personal_agenda` (`member_agenda_id`),
  ADD CONSTRAINT `outbox_mail_mailchimp_ibfk_4` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`),
  ADD CONSTRAINT `outbox_mail_mailchimp_ibfk_5` FOREIGN KEY (`group_member_id`) REFERENCES `group_member` (`group_member_id`),
  ADD CONSTRAINT `outbox_mail_mailchimp_ibfk_6` FOREIGN KEY (`member_id`) REFERENCES `personal` (`member_id`),
  ADD CONSTRAINT `outbox_mail_mailchimp_ibfk_7` FOREIGN KEY (`id_personal_repeat`) REFERENCES `personal_agenda_repeat_date` (`id_group_repeat`),
  ADD CONSTRAINT `outbox_mail_mailchimp_ibfk_8` FOREIGN KEY (`id_group_repeat`) REFERENCES `group_agenda_repeat_date` (`id_group_repeat`);

--
-- Ketidakleluasaan untuk tabel `outbox_telegram`
--
ALTER TABLE `outbox_telegram`
  ADD CONSTRAINT `outbox_telegram_ibfk_1` FOREIGN KEY (`group_agenda_id`) REFERENCES `group_agenda` (`group_agenda_id`),
  ADD CONSTRAINT `outbox_telegram_ibfk_2` FOREIGN KEY (`group_agenda_private_id`) REFERENCES `group_agenda_private` (`group_agenda_private_id`),
  ADD CONSTRAINT `outbox_telegram_ibfk_3` FOREIGN KEY (`member_agenda_id`) REFERENCES `personal_agenda` (`member_agenda_id`),
  ADD CONSTRAINT `outbox_telegram_ibfk_4` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`),
  ADD CONSTRAINT `outbox_telegram_ibfk_5` FOREIGN KEY (`group_member_id`) REFERENCES `group_member` (`group_member_id`),
  ADD CONSTRAINT `outbox_telegram_ibfk_6` FOREIGN KEY (`member_id`) REFERENCES `personal` (`member_id`),
  ADD CONSTRAINT `outbox_telegram_ibfk_7` FOREIGN KEY (`id_personal_repeat`) REFERENCES `personal_agenda_repeat_date` (`id_group_repeat`),
  ADD CONSTRAINT `outbox_telegram_ibfk_8` FOREIGN KEY (`id_group_repeat`) REFERENCES `group_agenda_repeat_date` (`id_group_repeat`);

--
-- Ketidakleluasaan untuk tabel `outbox_whatsapp`
--
ALTER TABLE `outbox_whatsapp`
  ADD CONSTRAINT `outbox_whatsapp_ibfk_1` FOREIGN KEY (`group_agenda_id`) REFERENCES `group_agenda` (`group_agenda_id`),
  ADD CONSTRAINT `outbox_whatsapp_ibfk_2` FOREIGN KEY (`group_agenda_private_id`) REFERENCES `group_agenda_private` (`group_agenda_private_id`),
  ADD CONSTRAINT `outbox_whatsapp_ibfk_3` FOREIGN KEY (`member_agenda_id`) REFERENCES `personal_agenda` (`member_agenda_id`),
  ADD CONSTRAINT `outbox_whatsapp_ibfk_4` FOREIGN KEY (`group_id`) REFERENCES `group` (`group_id`),
  ADD CONSTRAINT `outbox_whatsapp_ibfk_5` FOREIGN KEY (`group_member_id`) REFERENCES `group_member` (`group_member_id`),
  ADD CONSTRAINT `outbox_whatsapp_ibfk_6` FOREIGN KEY (`member_id`) REFERENCES `personal` (`member_id`),
  ADD CONSTRAINT `outbox_whatsapp_ibfk_7` FOREIGN KEY (`id_personal_repeat`) REFERENCES `personal_agenda_repeat_date` (`id_group_repeat`),
  ADD CONSTRAINT `outbox_whatsapp_ibfk_8` FOREIGN KEY (`id_group_repeat`) REFERENCES `group_agenda_repeat_date` (`id_group_repeat`);

--
-- Ketidakleluasaan untuk tabel `personal_agenda`
--
ALTER TABLE `personal_agenda`
  ADD CONSTRAINT `FK_personal_agenda_loop_type` FOREIGN KEY (`loop_type`) REFERENCES `loop_type` (`id_loop`),
  ADD CONSTRAINT `FK_personal_agenda_personal` FOREIGN KEY (`member_id`) REFERENCES `personal` (`member_id`);

--
-- Ketidakleluasaan untuk tabel `personal_agenda_repeat_date`
--
ALTER TABLE `personal_agenda_repeat_date`
  ADD CONSTRAINT `FK_personal_agenda_repeat_date_personal_agenda` FOREIGN KEY (`member_agenda_id`) REFERENCES `personal_agenda` (`member_agenda_id`);

--
-- Ketidakleluasaan untuk tabel `personal_channel`
--
ALTER TABLE `personal_channel`
  ADD CONSTRAINT `FK_personal_channel_channel` FOREIGN KEY (`channel_id`) REFERENCES `channel` (`channel_id`),
  ADD CONSTRAINT `FK_personal_channel_personal` FOREIGN KEY (`member_id`) REFERENCES `personal` (`member_id`);
COMMIT;

/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
