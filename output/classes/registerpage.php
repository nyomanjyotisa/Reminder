...
class RegisterPage extends RunnerPage
{
	...
	protected function activateNewUser()
	{
		...
		if( $this->cipherer->isFieldEncrypted("mail") )
			$strUsername = $this->cipherer->MakeDBValue("mail", $username, "", true);
		else
			$strUsername = make_db_value("mail", $username);

		$sql = "select ".$this->getFieldSQLDecrypt("password")
			." from ". $this->connection->addTableWrappers( "personal" ) 
			." where ".
		...
		$sql = "update ". $this->connection->addTableWrappers( "personal" )
			." set ". $this->connection->addFieldWrappers( "valid" )."=1 "
			." where " . 
			$this->connection->comparisonSQL( 
				$this->getFieldSQLDecrypt("mail"), 
				$strUsername,
	...
	{			
		return projectURL() . GetTableLink( "register" )
			."?a=activate&u=".rawurlencode( base64_encode($this->regValues["mail"]) )
	...
	{
		if( $this->cipherer->isFieldEncrypted("mail") )
			$sEmail = $this->cipherer->MakeDBValue("mail", $strEmail, "", true);	
		else 
			$sEmail = add_db_quotes("mail", $strEmail);
